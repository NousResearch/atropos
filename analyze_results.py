...
def analyze_batch_results():
    ...
    result_files = glob.glob("batch_results_*.json") + glob.glob("padres_test_results_*.json")
    ...
    print(f"- Successful actions: {success_count}/{len(all_experiments)} ({success_count/len(all_experiments)*100:.1f}%)")
    print(f"- Average reward: {reward_sum/len(all_experiments):.2f}")
    print(f"- Task completions: {task_completion_count}/{len(all_experiments)} ({task_completion_count/len(all_experiments)*100:.1f}%)")
