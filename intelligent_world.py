...
from intelligent_agent import IntelligentAgent, Relationship, Goal, MemoryType
...
class IntelligentWorld:
    ...
    def __init__(self, num_agents: int = 50, world_size: Tuple[float, float] = (100, 100),
                 llm_provider: LLMProvider = LLMProvider.NONE, api_key: Optional[str] = None,
        ...
        # LLM integration
        self.llm_manager = LLMManager(llm_provider, api_key, model) if llm_provider != LLMProvider.NONE else None
        ...
        if self.llm_manager:
            print(f"LLM integration: {llm_provider.value} with model {model or 'default'}")
        else:
    ...
    def _create_initial_relationships(self):
        ...
        for _ in range(num_initial_relationships):
            ...
            if agent1.agent_id != agent2.agent_id and agent2.agent_id not in agent1.relationships:
    ...
    async def _execute_world_event(self, event_type: str, valence: str):
        ...
        if event_type == "cultural_festival":
            ...
            for agent in self.agents:
                agent.resources["food"] = max(0, agent.resources["food"] - random.randint(5, 20))
            ...
            for agent in self.agents:
                if agent.cultural_group in [group1, group2]:
                    ...
                    for other in self.agents:
                        if (other.cultural_group != agent.cultural_group and 
                            other.cultural_group in [group1, group2] and
                            other.agent_id in agent.relationships):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_friendships = sum(len([r for r in agent.relationships.values() if r.strength > 0.5]) 
                              for agent in self.agents)
        family_count = len(set(agent.family_id for agent in self.agents if agent.family_id is not None))
        
        # Goal statistics
        total_goals = sum(len(agent.goals) for agent in self.agents)
        completed_goals = sum(len([g for g in agent.goals if g.completed]) for agent in self.agents)
        ...
        return {
            **basic_stats,
            ...
            "goals": {
                ...
                "completion_rate": round(completed_goals / max(1, total_goals) * 100, 1)
            },
            "memory": {
    ...
    def get_agent_insights(self) -> Dict[str, Any]:
        ...
        # Most social agents
        social_scores = [(agent.agent_id, len(agent.relationships)) for agent in self.agents]
        social_scores.sort(key=lambda x: x[1], reverse=True)
        insights["most_social"] = social_scores[:5]
        
        # Most successful (by currency)
        wealth_scores = [(agent.agent_id, agent.resources["currency"]) for agent in self.agents]
        wealth_scores.sort(key=lambda x: x[1], reverse=True)
        insights["most_successful"] = wealth_scores[:5]
        
        # Most goal-oriented
        goal_scores = [(agent.agent_id, len([g for g in agent.goals if g.completed])) for agent in self.agents]
...
async def run_intelligent_simulation(num_agents: int = 50, steps: int = 200,
                                   llm_provider: LLMProvider = LLMProvider.NONE,
                                   api_key: Optional[str] = None, model: Optional[str] = None):
    """Run intelligent society simulation"""
    print(f"🧠 Intelligent Society Simulation")
    print(f"   Agents: {num_agents}")
    print(f"   Steps: {steps}")
    print(f"   Intelligence: {llm_provider.value}")
    print("=" * 50)
    
    world = IntelligentWorld(num_agents, llm_provider=llm_provider, api_key=api_key, model=model)
    ...
    print(f"   Social Density: {final_stats['social']['social_density']} relationships per agent")
