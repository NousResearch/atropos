...
class VR1000Study:
    def __init__(self):
        ...
        self.targets = [
            [-0.4, 0.0, 0.2], [-0.3, 0.1, 0.15], [-0.5, -0.1, 0.25], 
            [-0.2, 0.05, 0.1], [-0.6, 0.0, 0.3], [-0.4, 0.2, 0.2],
            [-0.35, -0.05, 0.18], [-0.45, 0.15, 0.22], [-0.25, -0.15, 0.12],
            [-0.55, 0.1, 0.28], [-0.15, 0.25, 0.08], [-0.65, -0.05, 0.32]
        ]
        
        print("🚀 VR 1000 EXPERIMENT STUDY INITIALIZED")
        print(f"📊 {len(self.targets)} target positions available")
        print(f"🎯 Target: 1,000 experiments with population-level data")
    ...
    def run_single_experiment(self, exp_num):
        ...
        try:
            ...
            if object_positions:
                ...
                distance = np.sqrt(sum((a - t)**2 for a, t in zip(actual_pos, target)))
    ...
    def run_batch(self, start_exp, batch_size):
        ...
        print(f"  📊 Batch: {batch_success}/{batch_size} success ({batch_success/batch_size*100:.1f}%) in {batch_time:.1f}s")
    ...
    def analyze_progress(self, current_results):
        ...
        print(f"\n📈 PROGRESS ANALYSIS:")
        print(f"  Completed: {len(current_results)}/1000 ({len(current_results)/10:.1f}%)")
        ...
        print(f"  Estimated completion: {(1000 - len(current_results)) / rate:.1f} minutes")
    ...
    def run_1000_experiments(self):
        ...
        for batch_start in range(1, 1001, batch_size):
            ...
            recent_success = sum(1 for r in batch_results if r.get('success')) / len(batch_results)
        ...
        print(f"  Average rate: {len(self.results)/(total_time/60):.1f} experiments/minute")
        ...
        if successful:
            ...
            print(f"  Medium accuracy (0.7-0.9): {sum(1 for a in accuracies if 0.7 <= a <= 0.9)}")
        ...
        if successful:
            ...
            summary = {
                ...
                'performance_distribution': {
                    ...
                    'medium_accuracy_count': sum(1 for a in accuracies if 0.7 <= a <= 0.9),
