...
class ResearchDataManager:
    ...
    def __init__(self, project_id: str, bucket_name: str | None = None):
        ...
        try:
            ...
            logger.info(f"Google Cloud Storage client initialized for project '{self.project_id}'.")
    ...
    def _get_or_create_bucket(self, bucket_name: str) -> storage.Bucket:
        ...
        try:
            ...
            logger.error(f"Error checking for bucket '{bucket_name}': {e}", exc_info=True)
            raise
        return bucket
        ...
        try:
            ...
            if blob.exists():
                ...
                if existing_content and not existing_content.endswith('\n'):
                    ...
                    new_content = (existing_content or "") + json_string # Handle case where file exists but is empty
            ...
            logger.info(f"Successfully stored experiment '{experiment_data.get('experiment_id', 'N/A')}' to GCS: gs://{self.bucket_name}/{blob_name}")

        except Exception as e:
            logger.error(f"Error storing experiment data to GCS (experiment_id: {experiment_data.get('experiment_id', 'N/A')}): {e}", exc_info=True)
    ...
    def get_all_experiments_for_paper(self) -> list[dict]:
        ...
        all_experiments = []
        try:
            logger.info(f"Fetching all experiments from GCS bucket '{self.bucket_name}', prefix 'experiments/'.")
            blobs = self.client.list_blobs(self.bucket_name, prefix="experiments/")
            for blob in blobs:
                if blob.name.endswith(".jsonl") and blob.size > 0: # Ensure it's a JSONL and not empty
                    logger.info(f"Processing data from gs://{self.bucket_name}/{blob.name}...")
                    try:
                        jsonl_content = blob.download_as_text()
                        for line_number, line in enumerate(jsonl_content.strip().split('\n')):
                            if line:  # ensure not an empty line after split
                                try:
                                    experiment = json.loads(line)
                                    all_experiments.append(experiment)
                                except json.JSONDecodeError as e:
                                    logger.warning(f"Skipping line {line_number + 1} due to JSON decode error in '{blob.name}': {e} - Line content (first 100 chars): '{line[:100]}...'")
                    except Exception as e_blob:
                        logger.error(f"Error processing blob '{blob.name}': {e_blob}", exc_info=True)
            ...
            logger.info(f"Successfully retrieved {len(all_experiments)} experiments from GCS bucket '{self.bucket_name}'.")
            return all_experiments
        except Exception as e:
            logger.error(f"Error retrieving experiment data from GCS for paper generation: {e}", exc_info=True)
...
if __name__ == '__main__':
    ...
    if not gcp_project_id:
        print("GOOGLE_CLOUD_PROJECT environment variable not set. Please set it to run local tests.")
        # logger.error("GOOGLE_CLOUD_PROJECT environment variable not set. Please set it to run local tests.")
    else:
        logger.info(f"--- Running ResearchDataManager (GCS) local test for project: {gcp_project_id} ---")
        ...
        data_manager = ResearchDataManager(project_id=gcp_project_id) # Uses default bucket
        ...
        test_experiment_1 = {
            ...
            "llm_analysis": {"summary": "Circle experiment looks good.", "reasoning_quality": "high"},
        ...
        test_experiment_2 = {
            ...
            "llm_analysis": {"summary": "Square experiment is interesting.", "reasoning_quality": "medium"},
        ...
        logger.info(f"Check your GCS bucket: gs://{data_manager.bucket_name}/experiments/{datetime.utcnow().strftime('%Y-%m-%d')}.jsonl")
        ...
        if retrieved_experiments:
            logger.info(f"Successfully retrieved {len(retrieved_experiments)} experiments from GCS.")
            ...
            for i, exp in enumerate(retrieved_experiments[:3]): # Print first 3
                logger.info(f"  Retrieved Experiment {i+1}: ID '{exp.get('experiment_id', 'N/A')}', LLM Summary: '{exp.get('llm_analysis', {}).get('summary', 'N/A')}'")
            if len(retrieved_experiments) > 3:
                logger.info(f"  ... and {len(retrieved_experiments) - 3} more experiments.")
        else:
            logger.warning("No experiments retrieved or an error occurred during retrieval from GCS.")
