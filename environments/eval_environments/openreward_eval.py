...
class OpenRewardEval(EvalBase):
    ...
    async def run_item(self, server_manager: ServerManager, task_item: Any) -> Dict[str, Any]:
    ...
    async def __call__(self, server_manager: ServerManager):
        ...
        results = await asyncio.gather(*[self.run_item(server_manager, item) for item in data])
