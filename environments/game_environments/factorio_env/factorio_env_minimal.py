...
class FactorioEnv(BaseEnv):
    ...
    def _build_tools_prompt(self) -> str:
        ...
        def parse_signature(tool: str) -> Tuple[List[Tuple[str, Optional[str], bool]], Optional[str]]:
            ...
            try:
                if client.exists():
                    tree = ast.parse(client.read_text(encoding="utf-8", errors="ignore"))
                                    ...
                                    for a, d in zip(arg_nodes, defaults):
                                        ...
                                        if a.annotation is not None:
                                            try:
                                                ...
                                                ann = None
                                        is_optional = isinstance(d, ast.Constant) and d.value is None
        ...
        for t in sorted(tools):
            ...
            if params:
                ...
                arg_schema = "{}"
            if desc:
                specs_lines.append(f"- {{'name': '{t}', 'description': '{desc}', 'arguments': {arg_schema}}}")
            else:
            ...
            for name, ann, is_opt in params[:3]:  # keep short
                ...
                if "pos" in lname:
                    ...
                    example_args[name] = "Coal" if t == "harvest_resource" else "IronOre"
                elif "prototype" in lname or "entity" in lname:
                    # Use real Prototype examples
                    example_args[name] = "TransportBelt" if "place" in t else "StoneFurnace"
                elif "type" in lname:
            ...
            example_lines.append(
                "<tool_call>{'name': '" + t + "', 'arguments': " + json.dumps(example_args) + "}</tool_call>"
            )
    ...
    async def collect_trajectories(
        ...
        logger.info(f"Starting trajectory collection for task: {task_name}")
        
        # For now, collect single trajectory (group_size=1)
        scored_items = []
        
        for i in range(self.config.group_size):
            try:
                scored_item = await self._run_single_trajectory(
                    task_name, seed + i, f"{episode_id}-{i}"
                )
                if scored_item:
    ...
    async def _run_single_trajectory(
        self, task_name: str, seed: int, trajectory_id: str
        ...
        try:
            ...
            instance = await loop.run_in_executor(
                ...
                lambda: FactorioInstance(
                    ...
                    tcp_port=self.config.factorio_tcp_port,
                    fast=self.config.factorio_fast_mode,
                    num_agents=1
                )
            )
            logger.warning(f"FactorioInstance created successfully")
            
            # Setup task with instance
            task.setup(instance)
            ...
            logger.warning(f"Environment reset - obs type: {type(obs)}, info keys: {info.keys() if info else 'None'}")
            
            # Initialize conversation with task-specific system prompt
            task_goal = task.goal_description if hasattr(task, 'goal_description') else 'Complete the task'
            system_prompt = self._build_system_prompt(task_goal)
            messages.append({"role": "system", "content": system_prompt})
            logger.warning(f"Starting task: {task_name} with goal: {task_goal}")
            ...
            async with self.server.dedicated_server() as server:
                while not done and steps < self.config.max_steps_per_episode:
                    ...
                    if current_tokens > self.config.max_token_length - 500:
                        logger.warning(f"Trajectory {trajectory_id}: Approaching token limit")
                        break
                    ...
                    try:
                        ...
                        action_text = "<tool_call>" + response.choices[0].message.content.strip()
                        logger.info(f"Step {steps+1} - LLM response: {action_text[:150]}...")
                    except Exception as e:
                    ...
                    if code.startswith("__META__:"):
                        ...
                        try:
                            ...
                            total_reward += reward
                            logger.info(f"Step {steps+1} - Result: reward={reward:.2f}, done={done}, info={info}")
                        except Exception as e:
    ...
    def _format_observation(self, obs: Dict, info: Dict) -> str:
        ...
        if isinstance(obs, dict):
            if "inventory" in obs:
                inv = obs["inventory"]
                if inv:
                    inv_str = ", ".join([f"{item['type']}:{item['quantity']}" for item in inv[:8]])
    ...
    def _calculate_score(
        ...
        if steps > 0:
            score += (self.config.max_steps_per_episode / steps) * self.config.efficiency_weight
    ...
    async def evaluate(self, num_items: int = 10) -> Dict[str, Any]:
        ...
        if eval_scores:
            ...
            completion_rate = sum(eval_completions) / len(eval_completions) if eval_completions else 0
            ...
            logger.info(f"Evaluation: avg_score={avg_score:.2f}, completion_rate={completion_rate:.2%}")
