...
class TextarenaEnvConfig(BaseEnvConfig):
    ...
    registry_mode: str = Field(
        default="random", 
        description="Registry mode: random, challenge, specific"
    )
    ...
    registry_max_players: Optional[int] = Field(
        default=None, 
        description="Maximum number of players for selected games"
    )
    registry_min_players: Optional[int] = Field(
        default=None, 
        description="Minimum number of players for selected games"
    )
    ...
    format_reward_enabled: bool = Field(
        default=True,
        description="Apply format rewards for proper response structure"
    )
    format_reward_weight: float = Field(
        default=0.1,
        description="Weight for format reward in combined scoring"
    )
    ...
    format_wrong_order_penalty: float = Field(
        default=0.5,
        description="Penalty multiplier for blocks in wrong order"
    )
    ...
    token_length_penalty_weight: float = Field(
        default=0.1,
        description="Weight for token length penalty"
    )
    token_length_baseline: int = Field(
        default=500,
        description="Baseline token count for neutral penalty"
    )
    token_length_penalty_scale: float = Field(
        default=0.0002,
        description="Scale factor for token length penalty"
    )
    ...
    latro_weight: float = Field(
        default=0.0,
        description="Weight for LaTRo score"
    )
    ...
    registry_include_envs: Optional[List[str]] = Field(
        default=None, description="Specific environments to include in registry selection"
    )
    registry_exclude_envs: Optional[List[str]] = Field(
        default=None, description="Specific environments to exclude from registry selection"
    )
    ...
    textarena_agent_config: TextarenaAgentConfig = Field(default_factory=TextarenaAgentConfig)
...
class TextarenaEpisodeState:
    ...
    def __init__(
        ...
        self.initial_formatted_obs: List[Tuple[int, str, ta.ObservationType]] = initial_obs
        ...
        self.last_env_observations: Optional[List[Tuple[int, str, ta.ObservationType]]] = None
    ...
    def get_current_player_id(self) -> Optional[int]:
        ...
        return self.textarena_env.state.current_player_id if hasattr(self.textarena_env, 'state') else None 