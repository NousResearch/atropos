...
class TextWorldEnvRLPRv2Config(BaseEnvConfig):
    ...
    total_steps: int = 1000  # Collect 1000 episodes
    ...
    output_path: Optional[str] = None  # Path to save episodes (overrides data_path_to_save_groups)
...
class TextWorldEnvRLPRv2(BaseEnv):
    ...
    def _save_winning_episode(self, scored_data_groups, challenge_name, total_reward):
        ...
        # Use output_path if provided, otherwise fall back to data_path_to_save_groups
        save_path = self.config.output_path or self.config.data_path_to_save_groups
        if not save_path:
            return
