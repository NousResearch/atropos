...
class PuzzleGenerator:
    ...
    def _generate_door_sequence_puzzle(
        self,
        maker: GameMaker,
        params: Dict[str, Any],
            ...
            for _ in range(num_branches):
                ...
                for direction in directions:
                    ...
                    if not rooms[branch_from].exits[direction].destination:
                        maker.connect(
                            rooms[branch_from].exits[direction],
                            dead_end.exits[opposite],
                        )
                        # Add a distractor object
                        distractor = maker.new(
                            type="o",
    ...
    def _generate_combination_puzzle(
        self,
        maker: GameMaker,
        params: Dict[str, Any],
        template: Dict[str, Any],
        ...
        for i in range(1, min(nb_rooms, len(directions) + 1)):
            ...
            maker.connect(
                rooms[0].exits[direction], rooms[i].exits[opposite[direction]]
            )

        # Create combination (based on difficulty)
        combo_length = 3 if difficulty == "easy" else 4 if difficulty == "medium" else 5
        combination = "".join(
            [str(self.rng.randint(0, 9)) for _ in range(combo_length)]
        )
        ...
        for i, (digit, clue_name) in enumerate(zip(combination, clue_objects)):
            clue = maker.new(
                type="o",
                name=clue_name,
                desc=f"It has the number {digit} written on it.",
            )
        ...
        safe = maker.new(
            type="c",
            name="safe",
            desc=f"A safe with a {combo_length}-digit combination lock.",
        )
        rooms[0].add(safe)
    ...
    def _generate_weight_puzzle(
        ...
        scale = maker.new(
            type="s", name="scale", desc="A balance scale with two plates."
        )
        ...
        for weight_type in ["heavy", "medium", "light"]:
            if (
                difficulty != "easy" or weight_type != "medium"
            ):  # Easy mode has fewer weights
        ...
        if nb_rooms > 2:
            path = maker.connect(
                rooms[scale_room_idx].exits["north"], rooms[-1].exits["south"]
            )
            door = maker.new_door(
                path,
                name="heavy door",
                desc="This door opens when the scale is balanced.",
            )
            maker.add_fact("locked", door)
    ...
    for puzzle_type in ["door_sequence", "combination_lock", "weight_puzzle"]:
        for difficulty in ["easy", "medium"]:
            ...
            game_file, config = generator.generate(
                difficulty=difficulty, puzzle_type=puzzle_type
            )
