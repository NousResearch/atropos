...
class NavigationGenerator:
    ...
    def generate(self,
        ...
        try:
            ...
            if template["layout"] == "grid":
                ...
                rooms, start_room, end_room = self._generate_branching_maze(maker, params)
            ...
            # Create quest
            quest_commands = self._generate_navigation_quest(start_room, end_room, rooms)
            quest_commands.extend([
                "take golden key",
                f"unlock exit door with golden key"
            ])
            ...
            if params["nb_objects"] > len(template["landmarks"]):
                maker.generate_distractors(params["nb_objects"] - len(template["landmarks"]))
    ...
    def _generate_grid_maze(self, 
                           maker: GameMaker,
                           params: Dict[str, Any]) -> Tuple[List[Any], Any, Any]:
    ...
    def _generate_branching_maze(self,
                                maker: GameMaker,
                                params: Dict[str, Any]) -> Tuple[List[Any], Any, Any]:
    ...
    def _generate_hub_layout(self,
        ...
        for i, direction in enumerate(directions[:num_spokes]):
            ...
            for j in range(rooms_per_spoke):
                room_type = self.rng.choice(self.ROOM_THEMES["corridor" if j < rooms_per_spoke - 1 else "special"])
    ...
    def _generate_mixed_layout(self,
                              maker: GameMaker,
                              params: Dict[str, Any]) -> Tuple[List[Any], Any, Any]:
    ...
    def _place_landmarks(self,
        ...
        landmark_rooms = self.rng.sample(rooms[1:-1], min(num_landmarks, len(rooms) - 2))
