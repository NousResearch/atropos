...
class MixedGenerator:
    ...
    def generate(self,
        ...
        try:
            ...
            if mixed_type == "quest_puzzle":
                return self._generate_quest_puzzle(params, template, difficulty, output_folder, filename_prefix)
            elif mixed_type == "navigation_puzzle":
                return self._generate_navigation_puzzle(params, template, difficulty, output_folder, filename_prefix)
            elif mixed_type == "exploration_quest":
                return self._generate_exploration_quest(params, template, difficulty, output_folder, filename_prefix)
            elif mixed_type == "dungeon_crawler":
                return self._generate_dungeon_crawler(params, template, difficulty, output_folder, filename_prefix)
            else:  # adventure
                return self._generate_adventure(params, template, difficulty, output_folder, filename_prefix)
                
        except Exception as e:
            logger.error(f"Error generating mixed game: {e}")
            return None, {}
    
    def _generate_quest_puzzle(self,
                              params: Dict[str, Any],
        ...
        for i in range(min(3, len(rooms) - 1)):
            if i + 1 < len(rooms):
                ...
                if path:
                    door = maker.new_door(path, name=f"{puzzle_keys[i].split()[0]} door")
    ...
    def _generate_navigation_puzzle(self,
                                   params: Dict[str, Any],
                                   template: Dict[str, Any],
                                   difficulty: str,
                                   output_folder: str,
        ...
        landmarks = ["stone obelisk", "crystal formation", "ancient mural", "glowing orb"]
        landmark_rooms = self.rng.sample(rooms[1:-1], min(len(landmarks), len(rooms) - 2))
        ...
        for color, switch in switches:
            quest_commands.extend([
                "go north",  # Simplified navigation
                f"push {color} switch"
            ])
        quest_commands.extend([
            "take maze treasure",
    ...
    def _generate_exploration_quest(self,
                                   params: Dict[str, Any],
                                   template: Dict[str, Any],
                                   difficulty: str,
                                   output_folder: str,
    ...
    def _generate_dungeon_crawler(self,
                                 params: Dict[str, Any],
                                 template: Dict[str, Any],
                                 difficulty: str,
                                 output_folder: str,
                ...
                for y in range(grid_size):
                    if len(rooms) < nb_rooms:
                        ...
                        if y > 0 and self.rng.random() > 0.3:
                            ...
                            maker.connect(south_room.exits["north"], room.exits["south"])
                ...
                for i in range(section_size):
                    if len(rooms) < nb_rooms:
                        ...
                        if i > 0:
                            ...
                            maker.connect(prev_section_last.exits["north"], room.exits["south"])
        else:
            # Default linear
            for i in range(nb_rooms):
                room = maker.new_room(f"room_{i}")
    ...
    def _find_or_create_path(self,
                            maker: GameMaker,
                            room1: Any,
                            room2: Any) -> Optional[Any]:
