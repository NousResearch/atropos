...
class DiplomacyScorer:
    ...
    def calculate_negotiation_score(
        ...
        communication_score = min(1.0, (len(messages_sent) + len(messages_received)) / 20)
    ...
    def score_negotiation_action(
        ...
        total_score = (
            ...
            self.negotiation_weight * message_quality
        )
    ...
    def _score_message_quality(
        ...
        for to_power in to_powers:
            relationship = power_state.relationships.get(to_power, PowerRelationship.NEUTRAL)
    ...
    def calculate_episode_scores(
        ...
        for power in episode_state.power_assignment:
            ...
            for turn, scores in sorted(turn_scores.items()):
                if power in scores:
                    discounted_turn_score += scores[power] * (self.discount_factor ** turn_count)
                    turn_count += 1
