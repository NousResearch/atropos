...
class DiplomacyMemoryManager:
    ...
    async def retrieve_memories(
        ...
        if self.encoder:
            try:
                ...
                return sorted(self.memories, key=lambda m: m.turn, reverse=True)[:top_k]
        else:
        ...
        distances, indices = self.index.search(
            query_vec, min(top_k * 2, len(self.memories))
        )
    ...
    def get_memories_by_turn_range(
        ...
        return [m for m in self.memories if start_turn <= m.turn <= end_turn]
    ...
    def consolidate_memories(
        ...
        # Get recent memories
        recent_memories = sorted(self.memories, key=lambda m: m.turn, reverse=True)[
            :turn_range
        ]
        ...
        consolidated_details = {
            ...
            "key_events": [
                m.details.get("key_event")
                for m in recent_memories
                if m.details.get("key_event")
            ],
        }
