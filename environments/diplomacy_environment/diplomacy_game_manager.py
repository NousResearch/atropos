                ...
                async with session.get(f"{self.api_url}/health") as response:
                    if response.status != 200:
                        raise ConnectionError(
                            f"AI_Diplomacy API not healthy: {response.status}"
                        )
    ...
    async def initialize_game(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to create game: {await response.text()}"
                    )
    ...
    async def get_game_state(self, game_id: str) -> GameState:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{self.api_url}/games/{game_id}/state") as response:
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to get game state: {await response.text()}"
                    )
    ...
    async def submit_messages(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to submit messages: {await response.text()}"
                    )
    ...
    async def submit_orders(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to submit orders: {await response.text()}"
                    )

    async def execute_turn(
        self, episode_state: DiplomacyEpisodeState
    ) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.api_url}/games/{game_id}/process"
            ) as response:
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to process turn: {await response.text()}"
                    )
    ...
    async def get_legal_orders(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to get legal orders: {await response.text()}"
                    )
    ...
    async def get_power_stats(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status != 200:
                    raise RuntimeError(
                        f"Failed to get power stats: {await response.text()}"
                    )
