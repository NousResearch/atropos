...
class DiplomacyEnvNoThinkingConfig(BaseEnvConfig):
    ...
    powers_config: Dict[str, PowerConfig] = {
        ...
        "ENGLAND": {"type": "llm", "model": "claude-3-5-sonnet-20241022", "is_training": False},
        ...
        "RUSSIA": {"type": "llm", "model": "claude-3-5-sonnet-20241022", "is_training": False},
...
class DiplomacyEnvNoThinking(BaseEnv):
    ...
    async def _start_web_server(self):
        ...
        try:
            logger.info(f"Starting Diplomacy web server on port {self.config.web_server_port}")
            self.web_server_process = subprocess.Popen(
                [sys.executable, "-m", "diplomacy.server.run", "--port", str(self.config.web_server_port)],
            ...
            logger.info(f"Diplomacy web server started at http://localhost:{self.config.web_server_port}")
    ...
    def _get_training_powers(self) -> List[str]:
        ...
        return [
            power for power, config in self.config.powers_config.items()
    ...
    async def collect_trajectory(
        ...
        """
        game_id = item.get("game_id", f"game-{int(time.time())}-{random.randint(1000, 9999)}")
        ...
        all_messages: Dict[str, List[Message]] = {power: [] for power in training_powers}
        ...
        try:
            ...
            game_output_path = os.path.join(self.config.game_logs_dir, f"{game_id}.json") if self.config.save_game_logs else f"/tmp/{game_id}.json"
            
            sys.argv = [
                "lm_game.py",
                "--max_year", str(1900 + self.config.max_game_turns),  # Convert turns to years
                "--models", models_str,
                "--output", game_output_path,
                "--max_tokens", "2000",
                "--num_negotiation_rounds", "1",  # Enable some negotiation
            ]
            ...
            try:
                ...
                if os.path.exists(game_output_path):
                    ...
                    if phases:
                        last_phase = phases[-1]
                        for power in POWERS:
                            game_result["supply_centers"][power] = last_phase.get("state", {}).get("centers", {}).get(power, [])
                else:
            ...
            for power in training_powers:
                ...
                logger.info(f"{power} score: centers={center_score:.2f}, survival={survival_score:.2f}, "
            ...
            self.game_outcomes_buffer.append({
                ...
                "winner": winner,
                "turns": turns_played,
                "final_centers": {p: len(centers) for p, centers in final_centers.items()}
            })
    ...
    async def evaluate(self, *args, **kwargs):
        ...
        for i in range(total_games):
            ...
            if scored_item_tuple and scored_item_tuple[0]:
                ...
                if self.game_outcomes_buffer and self.game_outcomes_buffer[-1]["winner"] in self._get_training_powers():
                    wins += 1
            else:
        ...
        if eval_scores:
            ...
            logger.info(f"Evaluation completed: avg_score={avg_score:.2f}, win_rate={win_rate:.2%}")
    ...
    async def wandb_log(self, wandb_metrics: Optional[Dict[str, float]] = None):
        ...
        if self.game_outcomes_buffer:
            ...
            total_score = sum(
                score for game in self.game_outcomes_buffer 
                for score in game["scores"].values()
            )
            avg_score = total_score / (total_games * len(self._get_training_powers()))
            
            wins = sum(
                1 for game in self.game_outcomes_buffer 
                if game["winner"] in game["training_powers"]
            )
            win_rate = wins / total_games if total_games > 0 else 0
            
            avg_turns = sum(game["turns"] for game in self.game_outcomes_buffer) / total_games
