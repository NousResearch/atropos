    ...
    try:
        ...
        class TestClient(BaseModelClient):
            ...
            async def get_orders(
                self, board_state, power_name, possible_orders, **kwargs
            ):
                return possible_orders[:1] if possible_orders else []

            async def get_conversation_reply(
                self, power_name, conversation_so_far, game_phase, **kwargs
            ):
...
def explore_prompts():
    ...
    prompt_dir = os.path.join(
        os.path.dirname(__file__), "AI_Diplomacy/ai_diplomacy/prompts"
    )
