...
class VerifiersEnv(BaseEnv):
    ...
    def _setup_rubric(self) -> None:
        ...
        if not self.reward_funcs and self.rubric:
            if hasattr(self.rubric, "score_rollout") or hasattr(
                self.rubric, "score_rollouts"
            ):
    ...
    def _normalize_dataset(self, raw_data: Any, dataset_name: str) -> List[VfDataItem]:
        """
    ...
    async def score(
        self, rollout_group_data: List[RolloutData]
    ...
    async def _calculate_reward(self, item: RolloutData) -> float:
        ...
        if not self.reward_funcs or not self.rubric:
            response = (
                item["messages"][-1].get("content", "") if item["messages"] else ""
            )
