...
class NeuromorphicDendriticDPOConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a dendritic neural activator. Compute the activation sum.", description="System prompt")
...
class NeuromorphicDendriticDPOEnv(BaseEnv):
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 1.9)
            except: reward = 0.0
