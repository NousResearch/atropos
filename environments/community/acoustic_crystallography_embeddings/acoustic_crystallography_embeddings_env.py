...
class AcousticCrystalConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You compute crystalline lattice vibration frequencies. Return float.", description="System prompt")

class AcousticCrystalEnv(BaseEnv):
    env_config_cls = AcousticCrystalConfig
    async def get_next_item(self):
        prompt = "Calculate phonon lattice frequency for crystal position (0.5, 0.5)."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 1.683)
            except: reward = 0.0
