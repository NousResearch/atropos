...
class JosephsonSpikerConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You compute Josephson junction spikes. Return 1 if critical threshold exceeded.", description="System prompt")

class JosephsonSpikerEnv(BaseEnv):
    env_config_cls = JosephsonSpikerConfig
    async def get_next_item(self):
        prompt = "Does current=5.0 exceed critical Josephson threshold=3.0? Return 1 or 0."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = int(traj[-1]["content"].strip()); reward = 1.0 if val == 1 else 0.0
            except: reward = 0.0
