...
class MyceliumNutrientConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You compute mycelial nutrient flux. Return the midpoint flux value.", description="System prompt")

class MyceliumNutrientEnv(BaseEnv):
    env_config_cls = MyceliumNutrientConfig
    async def get_next_item(self):
        prompt = "Compute mycelium nutrient flux midpoint for source=100.0 and drain=50.0."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 75.0) / 75.0
            except: reward = 0.0
