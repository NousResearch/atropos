...
class CrystallineLatticeConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You propagate phonon waves in crystalline lattices. Return real FFT component.", description="System prompt")

class CrystallineLatticeEnv(BaseEnv):
    env_config_cls = CrystallineLatticeConfig
    async def get_next_item(self):
        prompt = "Compute phonon propagation FFT[0] for wave [1, 0, -1, 0]."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 0.0) / 2.0
            except: reward = 0.0
