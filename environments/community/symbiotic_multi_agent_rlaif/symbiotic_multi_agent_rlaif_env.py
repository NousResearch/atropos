...
class SymbioticRlaifConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You manage resource trades between symbiotic RL agents. Return exchanged amount.", description="System prompt")

class SymbioticRlaifEnv(BaseEnv):
    env_config_cls = SymbioticRlaifConfig
    async def get_next_item(self):
        prompt = "Agent A needs 50.0 units, Agent B can supply 30.0. Compute how much can be exchanged."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 30.0) / 30.0
            except: reward = 0.0
