...
class SlimeMoldAttentionConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a slime mold routing agent. Return the dominant flow index.", description="System prompt")
...
class SlimeMoldAttentionEnv(BaseEnv):
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = int(traj[-1]["content"].strip()); reward = 1.0 if val == 2 else 0.1
            except: reward = 0.0
