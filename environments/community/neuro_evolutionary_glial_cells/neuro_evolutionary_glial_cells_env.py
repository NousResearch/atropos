...
class GlialCellConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a glial cell optimizer. Return cleaned gradient count.", description="System prompt")

class GlialCellEnv(BaseEnv):
    env_config_cls = GlialCellConfig
    async def get_next_item(self):
        prompt = "Clean gradients [0.1, 0.02, 0.5] by zeroing values below 0.05. How many remain non-zero?"
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = int(traj[-1]["content"].strip()); reward = 1.0 if val == 2 else 0.0
            except: reward = 0.0
