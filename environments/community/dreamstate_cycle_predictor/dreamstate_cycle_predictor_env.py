...
class DreamstateConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a dream stage prediction model. Return dream state likelihood.", description="System prompt")

class DreamstateEnv(BaseEnv):
    env_config_cls = DreamstateConfig

    async def get_next_item(self):
        prompt = "Determine dream probability for motion readings: [0.01, 0.02, 0.01, 0.03]."
