...
class PhonosemanticConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You map words to phonosemantic frequencies. Return frequency for 'h'.", description="System prompt")

class PhonosemanticEnv(BaseEnv):
    env_config_cls = PhonosemanticConfig
    async def get_next_item(self):
        prompt = "Map the first character 'h' in 'hello' to its phonosemantic audio frequency."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 520.0) / 1000.0
            except: reward = 0.0
