...
class QuantumEntangledConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You simulate quantum entanglement between tokens. Return entanglement strength.", description="System prompt")

class QuantumEntangledEnv(BaseEnv):
    env_config_cls = QuantumEntangledConfig
    async def get_next_item(self):
        prompt = "Compute quantum tensor product entanglement for states q=[1,0] and k=[0,1]."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 0.0)
            except: reward = 0.0
