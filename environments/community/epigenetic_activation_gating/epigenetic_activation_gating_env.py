...
class EpigeneticGatingConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You compute epigenetic gate methylation rates. Return methylation value.", description="System prompt")

class EpigeneticGatingEnv(BaseEnv):
    env_config_cls = EpigeneticGatingConfig
    async def get_next_item(self):
        prompt = "Compute methylation gate for input x=10.0 at historical frequency 5.0."
