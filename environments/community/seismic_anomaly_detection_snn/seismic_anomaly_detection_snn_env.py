...
class SeismicSNNConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You detect seismic anomalies. Return 1 if anomaly detected else 0.", description="System prompt")

class SeismicSNNEnv(BaseEnv):
    env_config_cls = SeismicSNNConfig
    async def get_next_item(self):
        prompt = "Detect seismic anomaly in signal [0.1, 1.8, 0.2]. Is max amplitude > 1.5?"
