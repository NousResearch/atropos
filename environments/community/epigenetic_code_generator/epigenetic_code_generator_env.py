...
class EpigeneticCodeConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You generate epigenetic code stubs. Return 'active' or 'silenced'.", description="System prompt")

class EpigeneticCodeEnv(BaseEnv):
    env_config_cls = EpigeneticCodeConfig
    async def get_next_item(self):
        prompt = "Determine epigenetic runtime state for component with methylation_active=True."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            reward = 1.0 if "active" in traj[-1]["content"].lower() else 0.0
