...
class QuantumNASConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a quantum NAS agent selector. Output optimal layout schema.", description="System prompt")

class QuantumNASEnv(BaseEnv):
    env_config_cls = QuantumNASConfig

    async def get_next_item(self):
        prompt = "Search for best hybrid quantum layout under circuit search space constraints."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            ...
            reward = 1.0 if "quantum" in content else 0.2
