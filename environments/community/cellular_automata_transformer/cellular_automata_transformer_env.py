...
class CellularAutomataConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You simulate 1D cellular automata on binary sequences. Return next state.", description="System prompt")

class CellularAutomataEnv(BaseEnv):
    env_config_cls = CellularAutomataConfig
    async def get_next_item(self):
        prompt = "Apply Rule-90 XOR automata step to binary sequence [1, 0, 1, 1, 0]. What is result[2]?"
