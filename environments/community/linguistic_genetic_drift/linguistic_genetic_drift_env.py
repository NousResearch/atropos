...
class LinguisticGeneticDriftConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You simulate linguistic genetic drift. Estimate word frequency after drift.", description="System prompt")
...
class LinguisticGeneticDriftEnv(BaseEnv):
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 if 0.0 <= val <= 1.0 else 0.0
            except: reward = 0.0
