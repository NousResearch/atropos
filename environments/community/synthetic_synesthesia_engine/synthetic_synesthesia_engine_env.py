...
class SynesthesiaConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are a synesthetic sensory translator. Return MIDI notes.", description="System prompt")

class SynesthesiaEnv(BaseEnv):
    env_config_cls = SynesthesiaConfig

    async def get_next_item(self):
        prompt = "Translate RGB color values [255, 128, 64] into MIDI auditory pitches."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            ...
            reward = 1.0 if "63" in content else 0.2
