...
class OrigamiGraphConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You are an origami graph layout solver. Estimate folded projection values.", description="System prompt")

class OrigamiGraphEnv(BaseEnv):
    env_config_cls = OrigamiGraphConfig

    async def get_next_item(self):
        prompt = "Project node coordinates (1.0, 1.0) under a 45 degree fold axis."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            ...
            # Correct fold matrix coordinate projection is around 0.0 for x_folded
            reward = 1.0 if "0." in content else 0.1
