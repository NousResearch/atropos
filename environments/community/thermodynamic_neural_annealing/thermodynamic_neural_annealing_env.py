...
class ThermodynamicAnnealingConfig(BaseEnvConfig):
    system_prompt: Optional[str] = Field("You simulate thermodynamic weight annealing. Return annealed weight value.", description="System prompt")

class ThermodynamicAnnealingEnv(BaseEnv):
    env_config_cls = ThermodynamicAnnealingConfig
    async def get_next_item(self):
        prompt = "Anneal weight 0.5 at temperature 0.1 with Gaussian noise."
    ...
    async def score(self, rollout_group_data: List) -> Optional[ScoredDataGroup]:
        ...
        for traj in rollout_group_data:
            try: val = float(traj[-1]["content"].strip()); reward = 1.0 - abs(val - 0.5)
            except: reward = 0.0
