...
class SolitaireEnv(BaseEnv):
    ...
    async def setup(self):
        ...
        self.train = []
        # TODO: Consider making this path configurable
        csv_file_path = "environments/hack0/solitaire_winning_probability/qa_data.csv"
        try:
                ...
                for row in reader:
                    ...
                    if 'question' in row and 'answer' in row:
                        self.train.append({
                            ...
                            "answer": row["answer"] # Assuming 'answer' in CSV is already in the desired format
                        })
                    else:
                        print(f"Warning: Skipping row due to missing 'question' or 'answer': {row}")
            if not self.train:
                print(f"Warning: No data loaded from {csv_file_path}. Ensure the file exists and has 'question' and 'answer' columns.")
            ...
            self.test.append(
                {
                    ...
                    "gold_answer": item["answer"] # Assuming 'answer' in CSV is the final gold answer string
                    .split("#")[-1]
                    .strip()
                    .replace(",", ""),
                }
            ...
            for item in rollout_group_data:
                ...
                try:
                    if len(item["messages"][-1]["content"].split("<formula>"))<2:
                        reward = -1
                        continue
                    if len(item["messages"][-1]["content"].split("<formula>")[1].split("</formula>"))<1:
                        reward = -1
                        continue
                    # print(item[0][-1]["content"])
                    answer_parsed = aeval(item["messages"][-1]["content"].split("<formula>")[1].split("</formula>")[0])
...
if __name__ == "__main__":
    ...
    # Set OpenAI API key from the config
    os.environ["OPENAI_API_KEY"] = "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"
    
    if len(sys.argv) == 1 or (len(sys.argv) > 1 and sys.argv[1] not in ["serve", "process"]):
