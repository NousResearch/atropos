...
[CRITICAL: Do NOT include any text, explanations, comments, multiple formulas, or intermediate calculation steps within this tag.]
...
Use parentheses liberally to ensure correct order of operations. For example, write (A * B) / (C * D) instead of A * B / C * D if you intend the division to apply to the result of (C * D). Be explicit!
...
class GamePredictor:
    def __init__(self, openai_api_key: str, openai_api_base: str, model: str = "llama-4-maverick-17b-128e-instruct-fp8"):
    ...
    def _extract_formula(self, response_text: str) -> Optional[str]:
        ...
        formula_matches = re.findall(r'<formula>\n(.*?)\n</formula>', response_text, re.DOTALL)
        ...
        lines = [line.strip() for line in last_formula_content.split('\\n') if line.strip()]
    ...
    def _evaluate_formula(self, formula: str) -> float:
        ...
        try:
            ...
            if isinstance(result, (int, float)):
                ...
                raise ValueError(f"Formula '{formula}' evaluated to non-numeric type: {type(result).__name__} ({result})")
        except KeyError as e:
            # Handle cases where symbols are not found (e.g., undefined variables in formula)
             raise ValueError(f"Error evaluating formula '{formula}': Undefined symbol {e}")
    ...
    def compare_probabilities(self, calculated: float, simulated: float) -> Tuple[float, str]:
        """
    ...
    async def predict_game(self, game_func: Callable, n_simulations: int = 100000) -> GameAnalysis:
        """
        ...
        message = {
            "role": "user",
            "content": [
                {"type": "text", "text": self._create_prompt(game_func) + "\n\n" + GUIDELINES},
            ],
        }
        ...
        simulated_prob = await asyncio.to_thread(self.simulate_game, game_func, n_simulations)

        # Calculate difference if both probabilities are available
        prob_diff = abs(calculated_prob - simulated_prob) if calculated_prob is not None else None
    ...
    async def predict_games(self, games: Dict[str, Callable], n_simulations: int = 100000) -> Dict[str, GameAnalysis]:
        """
    ...
    async def generate_qa_csv(self, games: Dict[str, Callable], n_simulations: int, csv_filepath: str):
        """
...
async def main():
    # API credentials
    openai_api_key = "secret_davide_512e497218d84a70a75148232833fff3.rCshDPwZuLdH7xW0wc6pxLQuhSspdPol"
    ...
    await predictor.generate_qa_csv(games, 100000, "environments/hack0/solitaire_winning_probability/qa_data.csv")
    ...
    for game_name, analysis in results.items():
        ...
        if analysis.calculated_probability is not None:
            diff, assessment = predictor.compare_probabilities(analysis.calculated_probability, analysis.simulated_probability)
