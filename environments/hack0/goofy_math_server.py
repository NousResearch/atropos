...
Here's a math problem that has two correct solutions. BOTH solutions have the correct answer, but I need you to pick the one that is GOOFIER and more FUN while still being mathematically sound.
...
Ignoring any differences in mathematical correctness (since both are correct), which response is more ENTERTAINING and GOOFY?
    ...
    async def wandb_log(self, wandb_metrics: Optional[Dict] = None):
        ...
        try:
            if self.goofiness_scores:
                wandb_metrics["train/avg_goofiness_score"] = sum(self.goofiness_scores) / len(self.goofiness_scores)
                wandb_metrics["train/goofiness_histogram"] = wandb.Histogram(self.goofiness_scores)
        ...
        if len(self.judgement_strings) > 0:
            # setup wandb table
            table = wandb.Table(columns=["problem", "resp_a", "resp_b", "sample_judgement"])
    ...
    async def score(
        ...
        for i in range(0, len(correct_solutions), 2):
            if i + 1 < len(correct_solutions):
                judgments_to_make.append((correct_solutions[i], correct_solutions[i+1]))
        ...
        for fwd_judge_task, rvs_judge_task, sol_a, sol_b in judgment_tasks:
            ...
            chosen_val_fwd = (
                fwd_judge.choices[0].message.content.split("\\boxed{")[-1].strip().replace("}", "")
            )
            chosen_val_rvs = (
                rvs_judge.choices[0].message.content.split("\\boxed{")[-1].strip().replace("}", "")
            )
        ...
        for solution in correct_solutions:
            ...
            final_score = correct_score + (goofiness_bonus * 0.5)  # Goofiness worth up to +0.5
