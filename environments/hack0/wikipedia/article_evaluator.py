#!/usr/bin/env python3
"""
ArticleEvaluator: Evaluates the factual accuracy of AI-generated Wikipedia articles 
against reference articles using OpenAI models.

This module compares an AI-generated article line by line against a reference Wikipedia article,
classifying statements as correct, incorrect, or unknown based on the reference content.
"""
...
"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a precision fact-checker that produces only valid YAML."},
    ...
    def calculate_accuracy_score(self, evaluation_data: Dict) -> float:
        ...
        correct_count = sum(1 for item in evaluation.values() 
                           if item.get('accuracy', '') == 'CORRECT')
        incorrect_count = sum(1 for item in evaluation.values() 
    ...
    def calculate_accuracy_statistics(self, evaluation_data: Dict) -> Dict:
        ...
        # Count the number of statements in each category
        correct_count = sum(1 for item in evaluation_data.values() 
                           if item.get('accuracy', '') == 'CORRECT')
        incorrect_count = sum(1 for item in evaluation_data.values() 
                             if item.get('accuracy', '') == 'INCORRECT')
        unknown_count = sum(1 for item in evaluation_data.values() 
