...
from google.cloud.scheduler_v1.types import Job, HttpTarget, OidcToken, RetryConfig
...
class CloudSchedulerManager:
    ...
    def __init__(self, project_id: str, region: str, cloud_run_service_url: str, scheduler_sa_email: str):
        """
        ...
        self.cloud_run_service_url = cloud_run_service_url.rstrip('/') # Ensure no trailing slash
        ...
        try:
            ...
            logger.info(f"ADC detected project: {adc_project_id_detected}. Operations will use configured project: {self.project_id}")
            ...
            logger.info("CloudSchedulerClient initialized successfully with explicitly scoped ADC.")
        except Exception as e:
            logger.error(f"Failed to initialize CloudSchedulerClient: {e}", exc_info=True)
            raise
    ...
    def create_or_update_http_job(
        ...
        if body:
            try:
                ...
                logger.error(f"Error serializing body for job '{job_id}': {e}")
                raise
        ...
        job_obj = Job(
            ...
            attempt_deadline={"seconds": int(attempt_deadline.rstrip('s'))}, # Duration string to seconds
        ...
        try:
            ...
            logger.info(f"Job '{job_id}' already exists. Attempting to update...")
            ...
            logger.info(f"Job '{job_id}' not found. Attempting to create...")
            try:
                created_job = self.client.create_job(parent=self.location_path, job=job_obj)
                ...
                return created_job
                 ...
                 logger.warning(f"Job '{job_id}' was created concurrently. Attempting to get and update.")
                ...
                logger.error(f"API error creating job '{job_id}': {e}", exc_info=True)
                raise
        except GoogleAPICallError as e:
            logger.error(f"API error interacting with job '{job_id}': {e}", exc_info=True)
            raise
        except Exception as e:
            logger.error(f"Unexpected error with job '{job_id}': {e}", exc_info=True)
            raise
    ...
    def delete_job(self, job_id: str):
        ...
        try:
            ...
            logger.warning(f"Job '{job_id}' not found. Nothing to delete.")
            return False
        except GoogleAPICallError as e:
            logger.error(f"API error deleting job '{job_id}': {e}", exc_info=True)
            raise
        except Exception as e:
    ...
    def setup_research_pipeline_jobs(self):
        ...
        run_experiment_schedule = "0 */2 * * *"  # Every 2 hours at the start of the hour
    ...
    def delete_all_research_pipeline_jobs(self):
        ...
        logger.warning("Attempting to delete standard research pipeline Cloud Scheduler jobs...")
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.info(f"Using SCHEDULER_SERVICE_ACCOUNT_EMAIL: {SCHEDULER_SERVICE_ACCOUNT_EMAIL}")

    except KeyError as e:
        logger.error(f"CRITICAL: Environment variable {e} not set. This script requires: "
