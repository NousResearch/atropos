...
def main():
    ...
    parser.add_argument(
        ...
        help="Atropos server URL",
    )
    parser.add_argument(
        "--env-command",
        type=str,
        required=True,
        help="Command to launch environment server",
    )
    parser.add_argument(
        "--min-actors", type=int, default=1, help="Min environment actors"
    )
    parser.add_argument(
        "--max-actors", type=int, default=20, help="Max environment actors"
    )
    parser.add_argument(
        "--target-pressure",
        type=float,
        default=1.0,
        help="Target Rollout Pressure (Queue/BatchSize)",
    )
    parser.add_argument(
        "--poll-interval", type=int, default=10, help="Poll interval in seconds"
    )
    parser.add_argument(
        "--cooldown", type=int, default=10, help="Scaling cooldown in seconds"
    )
    parser.add_argument(
        "--max-step", type=int, default=4, help="Max actors to add/remove at once"
    )
    parser.add_argument(
        "--port-range",
        type=str,
        default="8001:8020",
        help="Port range for local actors (e.g. 8001:8020)",
    )
    parser.add_argument(
        "--gpus-per-actor",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4000,
        help="Min free VRAM (MB) required to scale up",
    )
    ...
    if args.wandb:
        ...
        if wb_info.get("project") and wb_info.get("group"):
            ...
            logger.warning(
                "WandB enabled but server returned no project/group. Logging disabled."
            )
            ...
            if metrics:
                ...
                if target_actors > connected_actors:
                    ...
                    if free_vram < args.vram_threshold:
                        ...
                        actor.set_instance_count(
                            target_actors, gpus_per_actor=args.gpus_per_actor
                        )
                ...
                logger.warning(
                    "Could not fetch metrics. Check if Atropos server is running."
                )
