...
class LocalActor(ScalingStrategy):
    ...
    """
    def __init__(self, command: List[str], cwd: str = ".", port_range: str = "8001:8020"):
        ...
        try:
            ...
            logger.warning(f"Invalid port range '{port_range}'. Using default 8001:8020")
    ...
    def _discover_gpus(self) -> List[int]:
        ...
        try:
            ...
            return [int(line.split(":")[0].split()[-1]) for line in out.strip().split("\n")]
        except:
            logger.warning("No GPUs discovered via nvidia-smi. Running in CPU-only mode.")
            return []
            ...
            try:
                ...
                if search_cmd in cmdline and proc.pid != os.getpid():
                    ...
                    class AdoptedProcess:
                        ...
                        def poll(self):
                            try: p = psutil.Process(self.pid); return None if p.is_running() else 0
                            except: return 0
                        def wait(self, timeout=None):
                            try: return psutil.Process(self.pid).wait(timeout)
                            except: return 0
                        def terminate(self): 
    ...
    def set_instance_count(self, target_count: int, gpus_per_actor: int = 1):
        ...
        if target_count > current_count:
            ...
            for _ in range(to_add):
                ...
                if self.gpu_pool:
                    ...
                    while len(assigned_gpus) < gpus_per_actor and self.available_gpus:
                        ...
                        if self._check_gpu_health(gid):
                            ...
                            logger.critical(f"LocalActor: CORDONING GPU {gid} due to hardware throttle!")
                ...
                logger.info(f"LocalActor: Launched PID {proc.pid} on port {port} (GPUs: {assigned_gpus})")
            ...
            for _ in range(to_remove):
                ...
                try:
                    logger.info(f"LocalActor: Moving PID {pid} to drain mode (SIGUSR1)...")
        ...
        for p in self.processes:
            if p.poll() is None:
                ...
                if time.time() - launch_time < 10:
                    logger.warning(f"LocalActor: PID {pid} died rapidly. Recording failure.")
...
class RemoteActor(ScalingStrategy):
    ...
    def _ssh_exec(self, cmd: str) -> str:
        ...
        return subprocess.check_output(full_cmd, stderr=subprocess.STDOUT).decode().strip()
    ...
    def set_instance_count(self, target_count: int, **kwargs):
        ...
        if target_count > current_count:
            ...
            for _ in range(to_add):
                if not self.free_ports: break
                port = self.free_ports.pop(0)
                cmd_str = " ".join([c.replace("$PORT", str(port)) for c in self.command])
                ...
                logger.debug(f"RemoteActor({self.host}): Launched PID {pid} on port {port}")
            ...
            for _ in range(to_remove):
                ...
                if pid in self.pid_to_port:
                    self.free_ports.append(self.pid_to_port.pop(pid))
                try:
                    logger.info(f"RemoteActor({self.host}): Draining PID {pid} (SIGUSR1)")
    ...
    def cleanup(self):
        ...
        if all_pids:
            ...
            try: self._ssh_exec(f"kill -9 {pids_str}")
            except: pass
        self.remote_pids = []
        self.draining_pids = []
