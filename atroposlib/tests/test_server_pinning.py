...
class TestServerPinning(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        ...
        self.manager = ServerManager(
            configs=[self.config1, self.config2, self.config3], server_class=MockServer
        )
    ...
    async def test_managed_server_pinning_respects_base_url(self):
        ...
        async with self.manager.managed_server(
            base_url=target_url, tokenizer=DummyTokenizer()
        ) as managed:
            self.assertEqual(
    ...
    async def test_managed_server_pinning_fallback(self):
        ...
        async with self.manager.managed_server(
            base_url=fake_url, tokenizer=DummyTokenizer()
        ) as managed:
            self.assertEqual(
                managed.server.config.base_url,
                "http://worker-2:8000",
