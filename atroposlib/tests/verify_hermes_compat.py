...
def test_init():
    ...
    try:
        # Create a config
        config = HermesAgentEnvConfig(tokenizer_name="gpt2", group_size=4)

        # Verify inheritance of new Atropos fields
        print(
            f"Checking inherited fields: reward_mode={config.reward_mode}, track_api_perf={config.track_api_perf}"
        )

        # Mock server configs
        server_configs = [
            APIServerConfig(
                model_name="mock", base_url="http://localhost:8000", api_key="test"
            )
        ]
