"""Tests for TrainerDistillationEnv scaffold parity."""
...
from atroposlib.envs.trainer_distillation_env import TrainerDistillationEnv
...
class _ConcreteTrainerEnv(TrainerDistillationEnv):
...
async def test_attach_teacher_distillation_success():
    env = object.__new__(_ConcreteTrainerEnv)
    ...
    out = await TrainerDistillationEnv._attach_teacher_distillation(env, group)
...
def test_teacher_tokenizer_mismatch_raises(monkeypatch):
    ...
    with pytest.raises(
        ...
        TrainerDistillationEnv._validate_teacher_tokenizer_compatibility(
            env,
...
def test_init_requires_teacher_server_source(monkeypatch):
    from atroposlib.envs import trainer_distillation_env as module
...
def test_resolve_teacher_server_configs_returns_none_when_unset():
    assert (
        _ConcreteTrainerEnv._resolve_teacher_server_configs(
...
def test_resolve_teacher_server_configs_uses_teacher_namespace(monkeypatch):
    ...
    out = _ConcreteTrainerEnv._resolve_teacher_server_configs(
