...
class GPUMemoryManager:
    ...
    def __init__(self, max_agents: int = 2500, device_id: int = 0):
        ...
        logger.info(f"GPU Memory Manager initialized for {max_agents} agents on device {device_id}")
    ...
    def _initialize_memory_pools(self):
        ...
        try:
            ...
            total_memory_mb = sum(pool['size_mb'] for pool in self.memory_pools.values())
    ...
    def allocate_memory_pool(self, pool_type: MemoryPool) -> bool:
        ...
        try:
            ...
            pool['buffer'] = np.zeros(size_bytes // 4, dtype=np.float32)  # 4 bytes per float
            ...
            logger.info(f"Allocated {pool['size_mb']:.2f} MB for {pool_type} pool in {allocation_time:.4f}s")
            return True
    ...
    def transfer_data_to_gpu(self, data: np.ndarray, pool_type: MemoryPool) -> bool:
        ...
        try:
            ...
            if len(pool['buffer']) >= len(data.flatten()):
                ...
                logger.warning(f"Data size ({len(data.flatten())}) exceeds buffer size ({len(pool['buffer'])})")
            ...
            logger.debug(f"Transferred {data_size_mb:.2f} MB to GPU {pool_type} in {transfer_time:.4f}s")
            return True
    ...
    def transfer_data_from_gpu(self, pool_type: MemoryPool, size: int) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            logger.debug(f"Transferred {data_size_mb:.2f} MB from GPU {pool_type} in {transfer_time:.4f}s")
            return data
    ...
    def optimize_memory_layout(self) -> bool:
        ...
        try:
            ...
            if len(allocated_pools) > 0:
                ...
                largest_free_block = max(p['size_mb'] for p in self.memory_pools.values() if not p['allocated']) if any(not p['allocated'] for p in self.memory_pools.values()) else 0
                self.memory_stats.fragmentation_ratio = 1.0 - (largest_free_block / max(total_allocated, 1.0))
            ...
            logger.info(f"Memory optimization completed: {optimizations_applied} optimizations applied")
            return True
    ...
    def get_memory_stats(self) -> MemoryStats:
        ...
        if self.memory_stats.current_usage_mb > 0:
            allocated_count = sum(1 for p in self.memory_pools.values() if p['allocated'])
            total_pools = len(self.memory_pools)
            self.memory_stats.fragmentation_ratio = 1.0 - (allocated_count / total_pools)
    ...
    def get_transfer_performance(self) -> Dict:
        ...
        stats = {
            ...
            'cpu_to_gpu': {
                ...
                'avg_time': np.mean([t['time'] for t in cpu_to_gpu]) if cpu_to_gpu else 0.0,
                'avg_bandwidth_gbps': np.mean([t['bandwidth_gbps'] for t in cpu_to_gpu]) if cpu_to_gpu else 0.0,
                'total_mb': sum(t['size_mb'] for t in cpu_to_gpu)
            },
            'gpu_to_cpu': {
                'count': len(gpu_to_cpu),
                'avg_time': np.mean([t['time'] for t in gpu_to_cpu]) if gpu_to_cpu else 0.0,
                'avg_bandwidth_gbps': np.mean([t['bandwidth_gbps'] for t in gpu_to_cpu]) if gpu_to_cpu else 0.0,
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.info(f"GPU memory cleanup completed: {cleanup_count} pools deallocated")
