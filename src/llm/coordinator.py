...
from dataclasses import dataclass, field, asdict # Added asdict for completeness, though not strictly used in LLMCoordinator serialization yet
import aiohttp
...
class LLMCoordinator:
    ...
    def __init__(self, config):
        ...
        self.total_requests = 0; self.cached_responses = 0; 
        ...
        logger.info(f"LLM Coordinator initialized. Primary Model: {primary_model_name_for_log}, Secondary Model: {secondary_model_name_for_log or 'N/A'}, Rate Limit: {self.rate_limit_per_second}/s, Cache Size: {self.max_cache_size}")
    ...
    def _init_gemini(self):
        ...
        if not GEMINI_AVAILABLE:
            logger.warning("Gemini SDK (google-generativeai) not available. LLMCoordinator will use mock responses for all models.")
            return
        
        # Get API key from environment
        api_key = os.getenv('GEMINI_API_KEY') or os.getenv('GOOGLE_API_KEY')
        if not api_key:
            logger.warning("No Gemini API key (GEMINI_API_KEY/GOOGLE_API_KEY). LLMCoordinator will use mock responses for all models.")
            return
        ...
        try:
            ...
            primary_model_name_from_config = getattr(llm_config_obj, 'model_name', "gemini-pro")
            # Ensure a valid default if config string is empty or not gemini related for primary
            primary_model_name = primary_model_name_from_config if primary_model_name_from_config and "gemini" in primary_model_name_from_config.lower() else "gemini-pro"
            try:
                ...
                logger.error(f"Failed to initialize primary Gemini model '{primary_model_name}': {e_primary}. Primary model will be unavailable.")
                self.gemini_model = None # Ensure it's None if primary fails

            # Initialize secondary model if configured
            secondary_model_name_from_config = getattr(llm_config_obj, 'secondary_model_name', None)
            if secondary_model_name_from_config:
                ...
                secondary_model_name = secondary_model_name_from_config if "gemini" in secondary_model_name_from_config.lower() else None 
                if secondary_model_name: 
                    try:
                        self.gemini_model_secondary = genai.GenerativeModel(secondary_model_name)
                        logger.info(f"Secondary Gemini model initialized: {secondary_model_name}")
                    except Exception as e_secondary:
                        logger.error(f"Failed to initialize secondary Gemini model '{secondary_model_name}': {e_secondary}. Secondary model will be unavailable.")
                        self.gemini_model_secondary = None # Ensure it's None if secondary fails
                else:
                    logger.warning(f"Secondary model name '{secondary_model_name_from_config}' configured but not recognized as a Gemini model type. Secondary model will be unavailable.")
            else:
                logger.info("No secondary Gemini model configured.")

        except Exception as e_configure:
            logger.error(f"Failed during Gemini configuration or model loading: {e_configure}. All Gemini models will be unavailable.", exc_info=True)
            self.gemini_model = None
            self.gemini_model_secondary = None
    ...
    async def stop(self):
        ...
        if self._batch_processor_task:
            ...
            try: await self._batch_processor_task
            except asyncio.CancelledError: logger.debug("Batch processor task cancelled.")
    ...
    async def get_response(
        ...
        if cache_key in self.response_cache and getattr(self.config.llm, 'cache_responses', True):
            self.cached_responses += 1
            self.response_cache.move_to_end(cache_key) # Mark as recently used
            logger.debug(f"LLM Cache hit for agent {agent_id} (prompt hash: {hash(prompt[:100])}, model: {model_preference or 'primary'})")
            return self.response_cache[cache_key]
        
        if not self._running or not self._batch_processor_task or self._batch_processor_task.done():
            # This might happen if start() wasn't called or the processor died.
            logger.error(f"LLMCoordinator batch processor not running for agent {agent_id}. Returning fallback.")
        ...
        logger.debug(f"Agent {agent_id} request queued (pref: {model_preference or 'primary'}). Prompt hash: {hash(prompt[:100])}. Queue size: {self.request_queue.qsize()}")

        try:
            # Wait for the batch processor to fulfill this future
            llm_response_obj = await asyncio.wait_for(response_future, timeout=getattr(self.config.llm, 'request_timeout', 60.0))
            ...
            if llm_response_obj.success:
                if getattr(self.config.llm, 'cache_responses', True):
                    ...
                    if len(self.response_cache) > self.max_cache_size:
                        ...
                        logger.debug(f"LLM Cache evicted. Size: {len(self.response_cache)}")
                return llm_response_obj.response
            else:
                self.failed_requests_after_retries += 1
                logger.error(f"LLM request failed for {agent_id} after {llm_response_obj.retries_attempted} retries (via future, model pref: {model_preference or 'primary'}). Error: {llm_response_obj.error_message}")
                return self._get_fallback_response(agent_id, prompt)
        except asyncio.TimeoutError:
            self.failed_requests_after_retries += 1
            logger.error(f"LLM request timed out for agent {agent_id} (prompt hash: {hash(prompt[:100])}).")
            return self._get_fallback_response(agent_id, prompt)
        except Exception as e_fut:
            self.failed_requests_after_retries += 1
            logger.error(f"Error awaiting LLM response future for agent {agent_id}: {e_fut}", exc_info=True)
            return self._get_fallback_response(agent_id, prompt)
        ...
        try:
            ...
            async with self.throttler:
                ...
                if request.model_preference == "primary" and self.gemini_model:
                    response_text, retries_attempted = await self._call_gemini_api_with_retries(request, self.gemini_model, "primary")
                    success = True
                    error_message = None
                elif request.model_preference == "secondary" and self.gemini_model_secondary:
                    response_text, retries_attempted = await self._call_gemini_api_with_retries(request, self.gemini_model_secondary, "secondary")
                    success = True
                    error_message = None
                else:
                    # Use mock responses
            ...
            logger.warning(f"Gemini API failed for {request.agent_id}: {e}, falling back to mock")
            response_text = await self._mock_llm_response(request)
    ...
    async def _call_gemini_api_with_retries(self, request: LLMRequest, model_instance: Any, model_name_for_log: str) -> Tuple[str, int]:
        llm_config_obj = self.config.llm if hasattr(self.config, 'llm') else self.config
        max_retries = getattr(llm_config_obj, 'api_max_retries', 3)
        base_backoff_seconds = getattr(llm_config_obj, 'api_base_backoff', 1.0)
        max_backoff_seconds = getattr(llm_config_obj, 'api_max_backoff', 16.0)
        
        last_exception = None
        for attempt in range(max_retries + 1):
            try:
                return await self._call_gemini_api(request, model_instance, model_name_for_log), attempt # Return response and attempts count
            except google.api_core.exceptions.ResourceExhausted as e: # Rate limit
                last_exception = e
                logger.warning(f"Gemini API rate limit (ResourceExhausted) for {request.agent_id}, attempt {attempt+1}/{max_retries+1}. Error: {e}")
                self.api_errors_before_retry +=1
            except google.api_core.exceptions.ServiceUnavailable as e: # Server error
                last_exception = e
                logger.warning(f"Gemini API service unavailable for {request.agent_id}, attempt {attempt+1}/{max_retries+1}. Error: {e}")
                self.api_errors_before_retry +=1
            except google.api_core.exceptions.GoogleAPIError as e: # Other Google API errors
                last_exception = e
                logger.warning(f"Gemini API GoogleAPIError for {request.agent_id}, attempt {attempt+1}/{max_retries+1}. Error: {e}")
                self.api_errors_before_retry +=1
            except Exception as e: # Catch other general exceptions during API call
                last_exception = e
                logger.error(f"Generic error in _call_gemini_api for {request.agent_id}, attempt {attempt+1}/{max_retries+1}: {e}", exc_info=True)
                self.api_errors_before_retry +=1
            ...
            if attempt < max_retries:
                backoff_duration = min(base_backoff_seconds * (2 ** attempt), max_backoff_seconds)
                jitter = random.uniform(0, backoff_duration * 0.1) # Add up to 10% jitter
                sleep_duration = backoff_duration + jitter
                logger.info(f"Retrying Gemini API for {request.agent_id} in {sleep_duration:.2f} seconds...")
                await asyncio.sleep(sleep_duration)
            else:
                logger.error(f"Gemini API call failed for {request.agent_id} after {max_retries+1} attempts.")
    ...
    async def _call_gemini_api(self, request: LLMRequest, model_instance: Any, model_name_for_log: str) -> str:
        """Call Gemini API"""
        if not model_instance:
            raise Exception(f"Gemini model ({model_name_for_log}) not initialized for API call attempt.")
        try:
            ...
            if api_response.parts:
                ...
                if api_response.prompt_feedback and api_response.prompt_feedback.block_reason:
                    raise Exception(f"Prompt blocked due to: {api_response.prompt_feedback.block_reason.name}")
                if not api_response.candidates or not api_response.candidates[0].content.parts:
                     # Check if content is blocked in candidate
                    if api_response.candidates and api_response.candidates[0].finish_reason == genai.types.Candidate.FinishReason.SAFETY:
                        # Find the safety rating that caused blocking
                        blocked_rating = next((r for r in api_response.candidates[0].safety_ratings if r.blocked), None)
                        block_reason_detail = blocked_rating.category.name if blocked_rating else "Unknown safety reason"
                        raise Exception(f"Response content blocked due to safety: {block_reason_detail}")
                    raise Exception("No response parts or content generated, possibly due to safety filters or empty response.")
                ...
                raise Exception(f"No response parts generated. Prompt block reason: {block_reason}")
            ...
            logger.debug(f"Detailed error in _call_gemini_api for {request.agent_id}: {type(e).__name__} - {e}")
    ...
    def _get_cache_key(self, prompt: str, temperature: float, model_preference: Optional[str] = None) -> str:
    ...
    async def _batch_processor(self):
        ...
        while self._running:
            try:
                ...
                try:
                    request: LLMRequest = await asyncio.wait_for(self.request_queue.get(), timeout=1.0)
                except asyncio.TimeoutError:
                    continue # No request in queue, loop back to check self._running

                if not request or not request.response_future: # Should not happen if queueing is correct
                    if request: self.request_queue.task_done() # Mark done even if invalid
                    continue

                # Check if the future is already done (e.g., timed out by caller)
                if request.response_future.done():
                    logger.debug(f"Future for agent {request.agent_id} (prompt hash: {hash(request.prompt[:100])}) already done. Skipping processing.")
                    self.request_queue.task_done()
                    continue
                
                logger.debug(f"Processing request for agent {request.agent_id} (prompt hash: {hash(request.prompt[:100])}). Queue size: {self.request_queue.qsize()}")
                ...
                try:
                    ...
                    logger.error(f"Unexpected error in _process_single_request for {request.agent_id}: {e_process}", exc_info=True)
                    ...
                    if not request.response_future.done():
                        ...
                        failed_response = LLMResponse(
                            ...
                            retries_attempted=getattr(llm_response_obj, 'retries_attempted', 0) # if llm_response_obj exists
                        )
                ...
                logger.error(f"Unexpected error in LLM batch processor loop: {e_loop}", exc_info=True)
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "effective_failure_rate": (self.failed_requests_after_retries / max(1, self.total_requests - self.cached_responses)) if (self.total_requests - self.cached_responses) > 0 else 0,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "response_cache": dict(self.response_cache), # OrderedDict to dict for simple serialization
            "total_requests": self.total_requests,
            "cached_responses": self.cached_responses,
            "api_errors_before_retry": self.api_errors_before_retry,
            "failed_requests_after_retries": self.failed_requests_after_retries,
    ...
    def from_dict(cls, data: Dict[str, Any], config: Any) -> "LLMCoordinator":
        ...
        coordinator.failed_requests_after_retries = data.get("failed_requests_after_retries", 0)
        
        logger.info(f"LLMCoordinator state restored. Cache items: {len(coordinator.response_cache)} (max: {coordinator.max_cache_size}), Total Requests: {coordinator.total_requests}")
        return coordinator 