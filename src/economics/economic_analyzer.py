...
class WealthDistribution:
    ...
    wealth_percentiles: Dict[int, float] = field(default_factory=dict)  # 10th, 25th, 50th, 75th, 90th
...
class EconomicAnalyzer:
    ...
    def __init__(self, market_system: MarketSystem, banking_system: BankingSystem):
        ...
        self.inflation_basket = [ResourceType.FOOD, ResourceType.MATERIALS, ResourceType.ENERGY]  # CPI basket
    ...
    def analyze(self, agent_states: Dict[str, Any]) -> EconomicIndicators:
        ...
        indicators.economic_health_score = self._calculate_economic_health_score(indicators)
        ...
        if len(self.historical_indicators) > max_history:
            ...
            self.historical_wealth_distributions = self.historical_wealth_distributions[-max_history:]
        
        logger.debug(f"Economic analysis complete - Health Score: {indicators.economic_health_score:.3f}")
        return indicators
    
    def _analyze_wealth_distribution(self, agent_states: Dict[str, Any]) -> WealthDistribution:
        ...
        for agent_id, agent in agent_states.items():
            ...
            # Subtract debt
            agent_loans = [loan for loan in self.banking_system.loans.values() 
                          if loan.borrower_id == agent_id and loan.status.value == "active"]
            debt = sum(loan.remaining_balance for loan in agent_loans)
            wealth -= debt
            
            agent_wealth.append(max(0.0, wealth))  # Wealth can't be negative for distribution analysis
        ...
        distribution.wealth_concentration_top_10 = top_10_wealth / distribution.total_wealth if distribution.total_wealth > 0 else 0
        distribution.wealth_concentration_top_1 = top_1_wealth / distribution.total_wealth if distribution.total_wealth > 0 else 0
        ...
        distribution.poor_agents = sum(1 for w in agent_wealth if w <= bottom_25_threshold)
        distribution.wealthy_agents = sum(1 for w in agent_wealth if w >= top_25_threshold)
        distribution.middle_class_agents = n - distribution.poor_agents - distribution.wealthy_agents
    ...
    def _calculate_inflation_rate(self) -> float:
        ...
        for resource_type in self.inflation_basket:
            current_prices[resource_type] = self.market_system.get_market_price(resource_type)
        
        # Compare with prices from previous period
        if resource_type in self.price_history and len(self.price_history[resource_type]) >= 2:
    ...
    def _calculate_trade_balance(self) -> float:
        ...
        for market in self.market_system.markets.values():
            ...
            recent_volume = sum(txn.total_cost for txn in market.recent_transactions[-10:])
    ...
    def _calculate_financial_stability(self) -> float:
        ...
        default_factor = max(0.0, 1.0 - default_rate * 5)  # 20% default rate = 0 stability
    ...
    def _calculate_market_liquidity(self) -> float:
        ...
        for market in self.market_system.markets.values():
            ...
            activity_factor = min(1.0, recent_activity / 10.0)  # 10 transactions = full activity
    ...
    def _calculate_economic_health_score(self, indicators: EconomicIndicators) -> float:
        ...
        scores['gdp_growth'] = max(0.0, min(1.0, (indicators.gdp_growth_rate + 0.5) / 1.0))
    ...
    def _update_price_history(self):
        ...
        for resource_type, market in self.market_system.markets.items():
            ...
            self.price_history[resource_type] = [
                (t, p) for t, p in self.price_history[resource_type] 
                if t > cutoff_time
            ]
    ...
    def get_economic_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        recent_indicators = self.historical_indicators[-days:] if len(self.historical_indicators) >= days else self.historical_indicators
        ...
        trends['gdp_trend'] = (gdp_values[-1] - gdp_values[0]) / len(gdp_values) if len(gdp_values) > 1 else 0
        
        # Unemployment trend
        unemployment_values = [ind.unemployment_rate for ind in recent_indicators]
        trends['unemployment_trend'] = (unemployment_values[-1] - unemployment_values[0]) / len(unemployment_values) if len(unemployment_values) > 1 else 0
        
        # Inflation trend
        inflation_values = [ind.inflation_rate for ind in recent_indicators]
        trends['inflation_trend'] = (inflation_values[-1] - inflation_values[0]) / len(inflation_values) if len(inflation_values) > 1 else 0
        
        # Inequality trend
        gini_values = [ind.gini_coefficient for ind in recent_indicators]
        trends['inequality_trend'] = (gini_values[-1] - gini_values[0]) / len(gini_values) if len(gini_values) > 1 else 0
        
        # Economic health trend
        health_values = [ind.economic_health_score for ind in recent_indicators]
        trends['health_trend'] = (health_values[-1] - health_values[0]) / len(health_values) if len(health_values) > 1 else 0
    ...
    def get_policy_recommendations(self, indicators: EconomicIndicators) -> List[str]:
        ...
        if indicators.gdp_growth_rate < 0.0:
            recommendations.append("Stimulate economic activity through public investment")
