...
@app.command()
def run(
    agents: int = typer.Option(50, "--agents", "-a", help="Number of agents to simulate"),
    ...
    console.print("🚀 [bold green]Starting 2,500-Agent LLM Society Simulation[/bold green]")
...
@app.command()
def demo(
    scenario: str = typer.Option("basic", "--scenario", "-s", help="Demo scenario to run"),
    debug: bool = typer.Option(False, "--debug", "-d", help="Enable debug logging"),
):
    """Run a demo scenario"""
    setup_logging(debug)
...
@app.command()
def benchmark(
    agents: int = typer.Option(100, "--agents", "-a", help="Number of agents for benchmark"),
    duration: int = typer.Option(60, "--duration", "-d", help="Benchmark duration in seconds"),
):
    """Run performance benchmarks"""
    console.print(f"⚡ [bold yellow]Running performance benchmark[/bold yellow]")
    console.print(f"📊 Agents: {agents}")
