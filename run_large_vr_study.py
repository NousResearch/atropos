...
def run_large_vr_study():
    ...
    if physics_data:
        ...
        print(f"   Average reward: {sum(p.get('reward', 0) for p in physics_data) / len(physics_data):.3f}")
        print(f"   Task completion rate: {sum(1 for p in physics_data if p.get('done')) / len(physics_data) * 100:.1f}%")
        
        # Position consistency analysis
        positions = [p.get('object_positions', [{}])[0].get('position', [0,0,0]) for p in physics_data if p.get('object_positions')]
        if positions:
            avg_x = sum(pos[0] for pos in positions) / len(positions)
            avg_y = sum(pos[1] for pos in positions) / len(positions)
            avg_z = sum(pos[2] for pos in positions) / len(positions)
            print(f"   Average red cube position: [{avg_x:.6f}, {avg_y:.6f}, {avg_z:.6f}]")
