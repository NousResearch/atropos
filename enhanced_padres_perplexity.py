...
from run_single_padres_test import PadresTest # This class uses Anthropic, will need changes if we fully remove Anthropic
...
class SimplePadresResearch:
    def __init__(self):
        ...
        self.padres = PadresTest(use_llm=False) # Set use_llm=False if PadresTest only uses LLM for its own analysis
        ...
        self.gemini_api_key = os.getenv('GEMINI_API_KEY') # Expect GEMINI_API_KEY in .env or environment

        if not self.gemini_api_key:
            logger.warning("GEMINI_API_KEY not found in environment. Gemini calls will fail.")
            ...
            try:
                ...
                logger.info("Gemini client configured and model initialized (gemini-1.5-flash-latest).")
            except Exception as e:
                logger.error(f"Failed to configure Gemini client or model: {e}", exc_info=True)
    ...
    def call_llm_for_text(self, prompt_text: str) -> str:
        ...
        logger.info(f"Sending prompt to Gemini (model: gemini-1.5-flash-latest). Prompt length: {len(prompt_text)}")
        try:
            ...
            if response.parts:
                ...
                logger.warning(f"Gemini response structure not as expected. Full response: {response}")
                ...
                if hasattr(response, 'candidates') and response.candidates:
                    for candidate in response.candidates:
                        if hasattr(candidate, 'content') and hasattr(candidate.content, 'parts') and candidate.content.parts:
                            ...
                            if generated_text: break # Take first available text
            if not generated_text:
                 logger.warning("No text found in Gemini response parts or direct .text attribute.")       
    ...
    def run_research_experiment(self):
        ...
        observation_for_llm = padres_result_raw.get('observation', "No observation provided by Padres API.")
        ...
        if isinstance(padres_result_raw.get('status'), dict) and \
           padres_result_raw['status'].get('api_status','').endswith("ERROR"):
            logger.warning(f"Padres API call resulted in an error state: {padres_result_raw['status']}. Observation for LLM will be this error.")
            observation_for_llm = f"Padres API Error: {padres_result_raw['status'].get('error_message', json.dumps(padres_result_raw['status']))}"
             ...
             observation_for_llm = json.dumps(padres_result_raw) # Fallback if 'observation' key is missing
        ...
        if isinstance(action_result, dict):
            ...
            elif action_result.get('done') == True and isinstance(action_result.get('reward'), (int, float)) and action_result.get('reward', 0) > 0: # Fallback logic
                is_padres_success = True
            # Check if any top-level status indicates an error explicitly
            elif isinstance(padres_result_raw.get('status'), dict) and padres_result_raw['status'].get('api_status','').endswith("ERROR"):
                 is_padres_success = False
        ...
        logger.info(f"Single experiment (Padres -> Gemini) processed. Experiment ID: {current_timestamp}, Padres Success: {is_padres_success}")
...
if __name__ == '__main__':
    ...
    #     print("Error: PERPLEXITY_API_KEY not found in environment. Please set it in your .env file.")
        ...
        if os.getenv("PERPLEXITY_API_KEY"):
            ...
            logger.warning("PERPLEXITY_API_KEY not set, skipping separate Perplexity test.") 