...
class ReportGenerator:
    ...
    def __init__(self, mcp_manager, config=None):
        ...
        self.default_gdocs_mcp = self.config.get("default_gdocs_mcp_server_name", "gdocs_main") # Assumes a gdocs_main server
        self.default_slack_mcp = self.config.get("default_slack_mcp_server_name", "slack_main") # Assumes a slack_main server
        self.default_llm_mcp = self.config.get("default_llm_mcp_server_name", "gemini_pro_main") # For formatting
        self.default_data_source_mcp_map = {
            "bigquery": self.config.get("default_bigquery_mcp_server_name", "bq_main"),
            "firestore": self.config.get("default_firestore_mcp_server_name", "firestore_main"),
            # Add other data source types and their default MCPs here
        }
        logging.info(f"ReportGenerator initialized. GDocs: {self.default_gdocs_mcp}, Slack: {self.default_slack_mcp}, LLM Formatter: {self.default_llm_mcp}")
    ...
    async def generate_report(self, report_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not report_data_collection or report_data_collection.get("error"):
                logging.error(f"Failed to fetch report data for '{report_name}': {report_data_collection.get('error')}")
                return {
                    "status": "error", 
            ...
            report_content_parts = await self._format_report_content(report_data_collection.get("data", {}), report_config)
            if not report_content_parts or report_content_parts.get("error"):
                logging.error(f"Failed to format report content for '{report_name}': {report_content_parts.get('error')}")
                return {
                    "status": "error", 
                    "report_name": report_name,
            ...
            gdoc_info = {}
            if report_config.get("output_gdoc", True): # Default to creating GDoc unless specified false
                gdoc_info = await self._create_or_update_gdoc(report_content_parts.get("formatted_content", {}), report_config)
                if not gdoc_info or gdoc_info.get("error"):
                    logging.error(f"Failed to create/update GDoc for '{report_name}': {gdoc_info.get('error')}")
                    # Non-fatal, can still send notifications if content exists
            
            notification_info = {}
            if report_config.get("send_notifications", True): # Default to sending notifications
                notification_info = await self._send_notifications(gdoc_info, report_content_parts.get("formatted_content", {}), report_config)
                if not notification_info or notification_info.get("error"):
                    logging.error(f"Failed to send notifications for '{report_name}': {notification_info.get('error')}")
                    # Non-fatal
            
            return {
                "status": "success",
                "report_name": report_name,
                "message": f"Report '{report_name}' generated.",
                "gdoc_info": gdoc_info,
                "notification_info": notification_info,
                "data_summary": report_data_collection.get("summary", "No data summary."),
                "content_summary": report_content_parts.get("summary", "No content summary.")
            }
        except Exception as e:
            logging.exception(f"Critical error in generate_report for '{report_name}': {e}")
            return {
                "status": "error",
                "report_name": report_name,
                "error": str(e)
    ...
    async def _fetch_report_data(self, report_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not data_sources_config:
            logging.warning(f"No data sources specified for report: {report_name}. Report will be based on static content or template only.")
            return {"data": {}, "summary": "No dynamic data sources specified."}

        logging.info(f"Fetching data for report: {report_name} from {len(data_sources_config)} sources")
        ...
        for i, source_conf in enumerate(data_sources_config):
            ...
            mcp_server_name = source_conf.get("mcp_server_name") # Specific MCP server for this source
            tool_name = source_conf.get("tool_name")
            parameters = source_conf.get("parameters", {})

            if not source_type:
                logging.warning(f"Skipping data source '{source_name}' due to missing 'type'.")
            ...
            if not mcp_server_name:
                logging.error(f"Cannot determine MCP server for source '{source_name}' of type '{source_type}'. Skipping.")
                source_summaries.append(f"{source_name}: skipped (no MCP server for type '{source_type}')")
                aggregated_data[source_name] = {"error": f"No MCP server configured for type '{source_type}'"}
                continue
            
            if not tool_name:
                 logging.error(f"Skipping data source '{source_name}': 'tool_name' is required for MCP interaction.")
                 source_summaries.append(f"{source_name}: skipped (missing tool_name)")
                 aggregated_data[source_name] = {"error": "Missing tool_name"}
                 continue

            logging.info(f"  Fetching data from '{source_name}' (type: {source_type}) using {mcp_server_name}.{tool_name}")
            try:
                ...
                if isinstance(data, dict) and data.get("error"):
                    logging.error(f"Error fetching data from '{source_name}': {data.get('error')}")
                    aggregated_data[source_name] = data # Store error response
                    source_summaries.append(f"{source_name}: error ({data.get('error')})")
                else:
                    aggregated_data[source_name] = data
                    source_summaries.append(f"{source_name}: success ({type(data)})")
            except Exception as e:
                logging.exception(f"Exception fetching data from '{source_name}': {e}")
                aggregated_data[source_name] = {"error": str(e)}
                source_summaries.append(f"{source_name}: exception ({str(e)})")
        
        return {"data": aggregated_data, "summary": f"Fetched data from {len(data_sources_config)} sources. Status: {source_summaries}"}

    async def _format_report_content(self, report_data: Dict[str, Any], 
                                  report_config: Dict[str, Any]) -> Dict[str, Any]:
        """Format the report data into structured content, potentially using an LLM via MCP."""
        report_name = report_config.get("name", "Unnamed Report")
        template_id = report_config.get("template_id", "default_summary") # e.g., "detailed_analysis_v1", "weekly_brief"
        static_content = report_config.get("static_content", {}) # Predefined sections, text
        use_llm_formatter = report_config.get("use_llm_formatter", True)

        logging.info(f"Formatting report content for: '{report_name}' using template/strategy: '{template_id}'. LLM Formatter: {use_llm_formatter}")
        ...
        if use_llm_formatter and report_data:
            ...
            Adapt the output based on the template goal '{template_id}'. For example, if it's a 'weekly_brief', keep it concise.
            ...
            try:
                llm_response = await self.mcp_manager.call_tool(
                    ...
                    parameters={"prompt": prompt, "max_tokens": 2048} # Adjust as needed
                )

                if llm_response and not (isinstance(llm_response, dict) and llm_response.get("error")):
                    content_str = llm_response.get("text") if isinstance(llm_response, dict) else str(llm_response)
                    ...
                    try:
                        ...
                        return {"formatted_content": formatted_content, "summary": f"Content formatted using LLM ({self.default_llm_mcp})."}
                    except json.JSONDecodeError as jde:
                        logging.error(f"LLM formatter returned invalid JSON for '{report_name}': {jde}. Raw: {content_str[:500]}")
                        # Fallback: use raw LLM output as a single content block
                        return {
                            "formatted_content": {"title": report_name, "sections": [{"heading": "LLM Output (Raw)", "content": content_str}]},
                            "summary": "LLM content formatting failed (JSON parse error), used raw output.",
                            "warning": "LLM response was not valid JSON."
                        }
                else:
                    err_msg = llm_response.get('error', 'Unknown LLM error') if isinstance(llm_response, dict) else "Unknown LLM error"
                    logging.error(f"Error from LLM formatter for '{report_name}': {err_msg}")
                    return {"error": f"LLM formatter MCP error: {err_msg}"}
            except Exception as e:
                logging.exception(f"Exception using LLM formatter for '{report_name}': {e}")
            ...
            logging.info(f"Using basic formatting (no LLM or no dynamic data) for '{report_name}'.")
            return {
                "formatted_content": {
                    "title": static_content.get("title", report_name),
                    "sections": static_content.get("sections", [
                        {"heading": "Data Overview", "content": json.dumps(report_data, indent=2, default=str) if report_data else "No dynamic data available."}
                    ])
                },
                "summary": "Content formatted using basic/static structure."
            }

    async def _create_or_update_gdoc(self, report_content: Dict[str, Any], 
                                 report_config: Dict[str, Any]) -> Dict[str, Any]:
        """Create or update a Google Doc with the report content via MCP."""
        report_name = report_config.get("name", "Unnamed Report")
        gdoc_title = report_content.get("title", report_name) # Use title from formatted content
        gdoc_template_id = report_config.get("gdoc_template_id") # Optional: for using a GDoc template
        target_folder_id = report_config.get("gdoc_folder_id") # Optional: to save in specific GDrive folder

        logging.info(f"Creating/updating Google Doc for report: '{report_name}' with title '{gdoc_title}'.")
        ...
        tool_to_call = "create_doc_from_content" # Assuming a tool that takes structured content
        ...
        try:
            ...
            if gdoc_response and gdoc_response.get("status", "").startswith("success") and gdoc_response.get("document_id"):
                logging.info(f"Successfully created/updated GDoc for '{report_name}': ID {gdoc_response.get("document_id")}")
                return {
                    "status": "success",
                    "doc_id": gdoc_response.get("document_id"),
                    "doc_url": gdoc_response.get("url", f"https://docs.google.com/document/d/{gdoc_response.get('document_id')}/edit"),
                    "operation": "created" if tool_to_call == "create_doc_from_content" else "updated"
                }
            else:
                error_msg = gdoc_response.get("error", "Unknown GDocs MCP error") if isinstance(gdoc_response, dict) else "Unknown GDocs MCP error"
                logging.error(f"Error from GDocs MCP for '{report_name}': {error_msg}")
                return {"error": f"GDocs MCP error: {error_msg}"}
        except Exception as e:
            logging.exception(f"Exception creating/updating GDoc for '{report_name}': {e}")
            return {"error": f"GDocs MCP exception: {str(e)}"}

    async def _send_notifications(self, gdoc_info: Dict[str, Any], report_content: Dict[str, Any], 
                             report_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        notification_channels = report_config.get("notification_channels", ["slack"]) # Default to slack
        slack_channel_id = report_config.get("slack_channel_id", "#general") # Example channel

        if not notification_channels:
            logging.info(f"No notification channels configured for report '{report_name}'. Skipping notifications.")
            return {"status": "skipped", "message": "No notification channels configured."}

        logging.info(f"Sending notifications for report: '{report_name}' via {notification_channels}")
        ...
        if "slack" in notification_channels and self.default_slack_mcp:
            ...
            for section in report_content.get("sections", [])[:2]: # First 2 sections
                ...
                content_preview = str(section.get("content"))[:100] + "..." if len(str(section.get("content"))) > 100 else str(section.get("content"))
                if heading: summary_points.append(f"*{heading}*: {content_preview}")
            
            message_text = f"New Report Generated: *{report_title}*\n" \
                           f"GDoc Link: {doc_url}\n\n" \
            ...
            try:
                slack_response = await self.mcp_manager.call_tool(
                    ...
                    parameters={"channel_id": slack_channel_id, "text_content": message_text}
                )
                if slack_response and slack_response.get("status", "").startswith("success"):
                    logging.info(f"Successfully sent Slack notification for '{report_name}' to {slack_channel_id}")
                    results["slack"] = {"status": "success", "channel": slack_channel_id, "ts": slack_response.get("ts")}
                else:
                    err_msg = slack_response.get("error", "Unknown Slack MCP error") if isinstance(slack_response, dict) else "Unknown Slack MCP error"
                    logging.error(f"Error from Slack MCP for '{report_name}': {err_msg}")
                    results["slack"] = {"status": "error", "error": err_msg}
            except Exception as e:
                logging.exception(f"Exception sending Slack notification for '{report_name}': {e}")
                results["slack"] = {"status": "error", "error": f"Slack MCP exception: {str(e)}"}
        elif "slack" in notification_channels:
            results["slack"] = {"status": "skipped", "error": "Slack MCP server not configured."}
            logging.warning("Slack notification requested but Slack MCP server not configured.")
...
async def run_rg_test_with_mcp():
    ...
    for mcp_name in ["gdocs_main", "slack_main", "bq_main", "firestore_main", "gemini_pro_main"]:
        mock_status = pipeline_config.get('mcp_server_configurations', {}).get(mcp_name, {}).get('config', {}).get('mock_mode')
    ...
    rg_config_from_pipeline.setdefault("default_llm_mcp_server_name", "gemini_pro_main") # Or claude_main
    rg_config_from_pipeline.setdefault("default_bigquery_mcp_server_name", "bq_main")
    rg_config_from_pipeline.setdefault("default_firestore_mcp_server_name", "firestore_main")
        ...
        "data_sources": [
            {
                ...
                "tool_name": "run_query",
                "parameters": {"query": "SELECT status, COUNT(*) as count FROM mock_dataset.trial_results WHERE week = CURRENT_WEEK() GROUP BY status"}
            }
        ],
        ...
        "data_sources": [
            {
                ...
                "parameters": {"collection_id": "assistant_outputs_test", "document_id": "exp_001_completed_insights_example"} 
    ...
    report_task3 = {
        ...
        "static_content": {
            ...
            "sections": [
                {"heading": "Meeting Schedule", "content": "Q3 planning meetings will be held next week..."},
                {"heading": "Key Topics", "content": "- Review Q2 results\n- Define Q3 OKRs\n- Resource allocation"}
            ]
        },
    ...
    for server_instance in mcp_manager.mcp_servers.values():
        if hasattr(server_instance, "close_client") and asyncio.iscoroutinefunction(server_instance.close_client):
