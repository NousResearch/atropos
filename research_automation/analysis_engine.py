...
class AnalysisEngine:
    ...
    def __init__(self, mcp_manager, config=None):
        ...
        self.bq_project_id = self.config.get("bigquery_project_id") # For context if needed, MCP handles actual project
        self.bq_dataset_id = self.config.get("bigquery_dataset_id") # For context
        self.bq_analysis_results_table = self.config.get("bigquery_analysis_results_table", "analysis_outputs")

        logging.info(f"AnalysisEngine initialized with BigQuery MCP: {self.default_bigquery_mcp}")
        if not self.bq_analysis_results_table:
            logging.warning("BigQuery analysis results table ID is not configured.")

    async def perform_deep_analysis(self, analysis_config: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not query_result_data or query_result_data.get("error"):
                logging.error(f"Failed to run analysis query: {query_result_data.get('error', 'No data returned')}")
                return {
                    "status": "error", 
            ...
            if isinstance(query_result_data, list):
                ...
                logging.warning(f"Unexpected format from _run_analysis_query for {analysis_name}: {type(query_result_data)}")
                # Proceed with empty data if format is unrecognized but no explicit error

            processed_results = await self._process_analysis_data(data_for_processing, analysis_config)
            if not processed_results or processed_results.get("error"):
                logging.error(f"Failed to process analysis data: {processed_results.get('error', 'Processing failed')}")
                return {
                    "status": "error", 
                    "analysis_name": analysis_name,
            ...
            storage_info = await self._store_analysis_results(processed_results, analysis_config)
            if not storage_info or storage_info.get("error"):
                logging.error(f"Failed to store analysis results: {storage_info.get('error', 'Storage failed')}")
                # Non-fatal for returning results, but log it.
            
            return {
                "status": "success",
                "analysis_name": analysis_name,
                "summary": f"Analysis '{analysis_name}' completed successfully.",
                "results": processed_results,
                "storage_info": storage_info
            }
        except Exception as e:
            logging.exception(f"Critical error in perform_deep_analysis for '{analysis_name}': {e}")
            return {
                "status": "error",
                "analysis_name": analysis_name,
                "error": str(e)
    ...
    async def _run_analysis_query(self, analysis_config: Dict[str, Any]) -> Any:
        ...
        logging.info(f"Running analysis query for '{analysis_config.get("name")}': {query[:100]}...")
        ...
        try:
            ...
            bq_response = await self.mcp_manager.call_tool(
                ...
                tool_name="run_query",
                parameters={"query": query, "to_dataframe": False} # Fetch as list of dicts
            )
            
            if isinstance(bq_response, dict) and "error" in bq_response:
                logging.error(f"Error from BigQuery MCP during query execution: {bq_response['error']}")
    ...
    async def _process_analysis_data(self, query_rows: List[Dict[str, Any]], analysis_config: Dict[str, Any]) -> Dict[str, Any]:
        """Process the raw data (list of dicts) from BigQuery into useful insights."""
        analysis_name = analysis_config.get("name", "Unnamed Analysis")
        logging.info(f"Processing analysis data for '{analysis_name}'. Received {len(query_rows)} rows.")

        # If no data, return a specific structure
        if not query_rows:
            logging.warning(f"No data rows received for processing for analysis '{analysis_name}'.")
            return {
                "metrics": {},
                "charts_data": [],
        ...
        try:
            ...
            logging.exception(f"Error converting query rows to DataFrame for '{analysis_name}': {e}")
            return {"error": f"Could not process query data into DataFrame: {str(e)}"}

        if df.empty:
            logging.warning(f"DataFrame is empty after converting query rows for '{analysis_name}'.")
            return {
                "metrics": {},
                "charts_data": [],
                "findings": ["Query data resulted in an empty dataset after processing."],
        ...
        try:
            ...
            if score_col:
                ...
                results["metrics"]["avg_overall_score"] = round(avg_overall_score, 3) if not pd.isna(avg_overall_score) else None
                results["metrics"]["num_valid_scores"] = int(df[score_col].count())
                results["findings"].append(f"Average score across {df.shape[0]} entries: {results['metrics']['avg_overall_score']:.2f}" if results['metrics']['avg_overall_score'] is not None else "Average score could not be calculated.")
            ...
            value_col = analysis_config.get("value_column_name", score_col if score_col else "value") # default to score_col if available

            if time_col in df.columns and value_col in df.columns and value_col != time_col:
                df[time_col] = pd.to_datetime(df[time_col], errors='coerce')
                df[value_col] = pd.to_numeric(df[value_col], errors='coerce')
                df_sorted = df.dropna(subset=[time_col, value_col]).sort_values(by=time_col)
                if not df_sorted.empty:
                    results["charts_data"].append({
                        ...
                        "data_points": df_sorted[[time_col, value_col]].to_dict(orient='records')
                    })
                    results["findings"].append(f"Trend analysis performed on '{value_col}' over '{time_col}'.")
            
            results["summary"] = f"Processed {df.shape[0]} data points. Found {len(results['findings'])} findings."
            if not results["findings"]:
                 results["findings"].append("Basic descriptive analysis performed. No specific pre-canned findings generated.")

        except Exception as e:
            logging.exception(f"Error during pandas processing for '{analysis_name}': {e}")
            results["error"] = f"Error during data processing: {str(e)}"
            results["findings"].append(f"Error during data processing: {str(e)}. Some results might be incomplete.")

        return results
    
    async def _store_analysis_results(self, processed_results: Dict[str, Any], 
                                   analysis_config: Dict[str, Any]) -> Dict[str, Any]:
        """Store the analysis results (summary, not raw data) in BigQuery via MCP."""
        analysis_name = analysis_config.get("name", "Unnamed Analysis")
        analysis_id = analysis_config.get("analysis_id", f"{analysis_name.replace(' ', '_').lower()}_{uuid.uuid4().hex[:8]}")
        output_table_id = self.bq_analysis_results_table

        if not output_table_id:
            logging.error("BigQuery analysis results table ID is not configured. Cannot store results.")
            return {"error": "BQ analysis results table ID not configured."}
        
        logging.info(f"Storing analysis results for '{analysis_name}' (ID: {analysis_id}) in BQ table: {output_table_id}")
        ...
        row_to_insert = {
            ...
            "results_summary_json": json.dumps(processed_results), # Store the whole results dict as JSON string
            "analysis_config_json": json.dumps(analysis_config) # Store the config for reproducibility
        }
        ...
        try:
            ...
            if insert_response and insert_response.get("status") == "success" and insert_response.get("inserted_count", 0) > 0:
                logging.info(f"Successfully stored analysis results for '{analysis_id}' in BigQuery table '{output_table_id}'.")
                return {
                    "status": "success",
                    "table_id": output_table_id,
                ...
                error_msg = insert_response.get("error", "Unknown error during BQ insert") if isinstance(insert_response, dict) else "Unknown BQ insert error"
                logging.error(f"Failed to store analysis results in BigQuery: {error_msg}")
...
async def run_ae_test_with_mcp():
    ...
    print(f"BigQuery MCP mock_mode: {pipeline_config.get('mcp_server_configurations', {}).get('bq_main', {}).get('config', {}).get('mock_mode')}")
    ...
    print(f"Results for '{general_task.get('name')}':\n{json.dumps(results1, indent=2, default=str)}")
