...
class ResearchAssistant:
    ...
    def __init__(self, mcp_manager, config=None):
        ...
        self.default_llm_mcp = self.config.get("default_llm_mcp_server_name", "gemini_pro_main")
        self.default_bigquery_mcp = self.config.get("default_bigquery_mcp_server_name", "bq_main")
        self.default_firestore_mcp = self.config.get("default_firestore_mcp_server_name", "firestore_main")
        self.default_perplexity_mcp = self.config.get("default_perplexity_mcp_server_name", "perplexity_main")
        ...
        self.firestore_insights_collection = self.config.get("firestore_insights_collection", "research_assistant_outputs")
        
        self.low_score_threshold = self.config.get("low_score_threshold", 0.7) # Example specific config

        logging.info(f"ResearchAssistant initialized with LLM: {self.default_llm_mcp}, BQ: {self.default_bigquery_mcp}, Firestore: {self.default_firestore_mcp}")
        if not all([self.bq_project_id, self.bq_dataset_id, self.bq_trial_results_table]):
            logging.warning("BigQuery connection details (project, dataset, table) are not fully configured.")

    async def analyze_experimental_results(self, experiment_ids: List[str]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not results_data or results_data.get("error"):
                logging.error(f"Failed to fetch experiment results: {results_data.get('error', 'No data returned')}")
                return {"status": "error", "error": f"Failed to fetch experiment results: {results_data.get('error', 'No data returned')}"}

            insights = await self._generate_insights(results_data, experiment_ids)
            if not insights or insights.get("error"):
                 logging.error(f"Failed to generate insights: {insights.get('error', 'No insights returned')}")
                 return {"status": "error", "error": f"Failed to generate insights: {insights.get('error', 'No insights returned')}"}

            recommendations = await self._generate_recommendations(insights, experiment_ids)
            if not recommendations or recommendations.get("error"):
                logging.error(f"Failed to generate recommendations: {recommendations.get('error', 'No recommendations returned')}")
            ...
            storage_success = await self._store_insights(insights, recommendations, experiment_ids)
            if not storage_success or storage_success.get("error"):
                logging.error(f"Failed to store insights: {storage_success.get('error', 'Storage failed')}")
            ...
            logging.exception(f"Critical error in analyze_experimental_results for {experiment_ids}: {e}")
            return {
                "status": "error",
                "error": str(e)
            }
    
    async def _fetch_experiment_results(self, experiment_ids: List[str]) -> Dict[str, Any]:
        """Fetch experiment results from BigQuery via MCP."""
        logging.info(f"Fetching results for experiments: {experiment_ids}")
        if not all([self.bq_project_id, self.bq_dataset_id, self.bq_trial_results_table]):
            return {"error": "BigQuery connection details (project, dataset, table) are not configured."}

        experiment_ids_quoted = [f"'{eid}'" for eid in experiment_ids]
        query = f"""
            SELECT *
        ...
        try:
            bq_response = await self.mcp_manager.call_tool(
                ...
                tool_name="run_query",
                parameters={"query": query, "to_dataframe": False} # Assuming returns list of dicts
            )
            ...
            if bq_response and "error" not in bq_response:
                ...
                elif isinstance(bq_response, dict) and "rows" in bq_response: # More standard BQ response
                    return {"experiment_data": bq_response["rows"]}
                else: # Unexpected format
                    logging.warning(f"Unexpected BigQuery response format: {bq_response}")
                    return {"experiment_data": []} # Return empty if format is weird but no explicit error
            else:
                logging.error(f"Error from BigQuery MCP: {bq_response.get('error', 'Unknown error')}")
                return {"error": f"BigQuery MCP error: {bq_response.get('error', 'Unknown error')}"}
        except Exception as e:
            logging.exception(f"Exception fetching experiment results from BigQuery: {e}")
            return {"error": f"Exception querying BigQuery: {str(e)}"}

    async def _generate_insights(self, results_data: Dict[str, Any], 
        ...
        try:
            llm_response = await self.mcp_manager.call_tool(
                ...
                parameters={"prompt": prompt, "max_tokens": 1024} # Adjust max_tokens as needed
            )
            ...
            if llm_response and not (isinstance(llm_response, dict) and llm_response.get("error")):
                ...
                try:
                    ...
                    elif isinstance(llm_response, str): # Direct string response (like Claude mock)
                        insights_content = llm_response
                    else:
                        logging.error(f"Unexpected LLM response type for insights: {type(llm_response)}")
                    ...
                    logging.error(f"Failed to parse JSON from LLM for insights: {jde}. Response was: {llm_response[:500]}")
                    # Fallback: return the raw text if JSON parsing fails, wrapped in a standard structure
                    return {"insights_generated": [{"insight_type": "raw_text", "description": str(llm_response)}], "warning": "LLM response was not valid JSON."}
                except Exception as e:
                    logging.exception(f"Error processing LLM response for insights: {e}")
                    return {"error": f"Error processing LLM response for insights: {str(e)}"}
    ...
    async def _generate_recommendations(self, insights_data: Dict[str, Any], 
                                      experiment_ids: List[str]) -> Dict[str, Any]:
        """Generate recommendations for new experiments via MCP based on insights."""
        logging.info(f"Generating recommendations based on insights for experiments: {experiment_ids}")
            ...
            if llm_response and not (isinstance(llm_response, dict) and llm_response.get("error")):
                try:
                    if isinstance(llm_response, dict) and "text" in llm_response:
                        ...
                        logging.error(f"Unexpected LLM response type for recommendations: {type(llm_response)}")
                        return {"error": "Unexpected LLM response type for recommendations."}

                    if recommendations_content.strip().startswith("```json"):
                        recommendations_content = recommendations_content.strip()[7:-3].strip()
                    elif recommendations_content.strip().startswith("```"):
                        recommendations_content = recommendations_content.strip()[3:-3].strip()
                    ...
                    logging.error(f"Failed to parse JSON from LLM for recommendations: {jde}. Response was: {llm_response[:500]}")
                    return {"recommendations_generated": [{"recommendation_type": "raw_text", "description": str(llm_response)}], "warning": "LLM response was not valid JSON."}
                except Exception as e:
                    logging.exception(f"Error processing LLM response for recommendations: {e}")
                    return {"error": f"Error processing LLM response for recommendations: {str(e)}"}
            else:
                err_msg = llm_response.get('error', 'Unknown LLM error') if isinstance(llm_response, dict) else "Unknown LLM error"
    ...
    async def _store_insights(self, insights_data: Optional[Dict[str, Any]], 
        ...
        primary_experiment_id = experiment_ids[0] if experiment_ids else "batch_analysis"
        ...
        logging.info(f"Storing insights and recommendations in Firestore under doc ID: {insight_doc_id} in collection {self.firestore_insights_collection}")

        data_to_store = {
            "assistant_run_id": insight_doc_id,
            "analyzed_experiment_ids": experiment_ids,
            "insights": insights_data.get("insights_generated", []),
            "recommendations": recommendations_data.get("recommendations_generated", []),
        ...
        try:
            ...
            if fs_response and fs_response.get("status", "").startswith("success"):
                logging.info(f"Successfully stored insights in Firestore: {insight_doc_id}")
                return {
                    "status": "success", 
                    "document_id": insight_doc_id, 
                    "collection_id": self.firestore_insights_collection,
                ...
                error_msg = fs_response.get("error", "Unknown Firestore error") if isinstance(fs_response, dict) else "Unknown Firestore error"
...
async def main_test():
    ...
    print(f"Claude mock_mode: {pipeline_config.get('mcp_server_configurations', {}).get('claude_main', {}).get('config', {}).get('mock_mode')}")
    print(f"BigQuery mock_mode: {pipeline_config.get('mcp_server_configurations', {}).get('bq_main', {}).get('config', {}).get('mock_mode')}")
    print(f"Firestore mock_mode: {pipeline_config.get('mcp_server_configurations', {}).get('firestore_main', {}).get('config', {}).get('mock_mode')}")
    ...
    if not ra_config:
        print("Warning: 'research_assistant_config' not found in pipeline_config.json. RA may not work correctly.")
    ...
    assistant_run_result = await assistant.analyze_experimental_results([test_experiment_id_completed])
    print(f"Research Assistant run result:\n{json.dumps(assistant_run_result, indent=2)}")
    ...
    no_data_run_result = await assistant.analyze_experimental_results([test_experiment_id_no_data])
    print(f"Research Assistant run result (no data):\n{json.dumps(no_data_run_result, indent=2)}")
    ...
    print(f"Research Assistant run result (empty list):\n{json.dumps(empty_list_run_result, indent=2)}")
