...
class PointEHandler:
    def __init__(self, device="cuda" if torch.cuda.is_available() else "cpu"):
        ...
        self.loop = asyncio.get_event_loop() if hasattr(asyncio, '_get_running_loop') else None
    ...
    async def load_models(self):
        ...
        try:
            ...
            await self.loop.run_in_executor(None, self.base_model.load_state_dict, base_model_state_dict)
            ...
            await self.loop.run_in_executor(None, self.upsampler_model.load_state_dict, upsampler_model_state_dict)
            ...
            logger.info("PointEHandler: Enabling fallback mode with procedural generation")
            self.fallback_mode = True
            self.models_loaded = True  # Consider fallback as "loaded"
            return True

    async def generate_point_cloud_from_text(self, prompt: str, output_path: str = "point_cloud.ply"):
    ...
    async def _generate_real_point_cloud(self, prompt: str, output_path: str):
        ...
        try:
            ...
            def _blocking_save(pc_data, out_path):
                ...
                colors_cpu = None
                if pc_data.channels is not None and all(k in pc_data.channels for k in ['R','G','B']):
                    r = pc_data.channels['R'].cpu().numpy()
                    g = pc_data.channels['G'].cpu().numpy() 
                    b = pc_data.channels['B'].cpu().numpy()
                    colors_cpu = (np.stack([r.squeeze(),g.squeeze(),b.squeeze()],axis=-1)*255).astype(np.uint8)
            ...
            pc, fig = await self.loop.run_in_executor(None, _blocking_sample_and_process, self.sampler, prompt)
            if pc is None or fig is None:
                logger.error(f"PointEHandler: Failed to generate point cloud samples for prompt: {prompt}")
                return None, None

            save_success = await self.loop.run_in_executor(None, _blocking_save, pc, output_path)
            if not save_success:
                logger.error(f"PointEHandler: Failed to save point cloud to {output_path}")
                return pc, fig
            
            return pc, fig
            
        except Exception as e:
            logger.error(f"PointEHandler: Error in real Point-E generation for '{prompt}': {e}")
    ...
    async def _generate_fallback_point_cloud(self, prompt: str, output_path: str):
        ...
        try:
            logger.info(f"PointEHandler: Generating procedural point cloud for: {prompt}")
    ...
    def _generate_cube_points(self, color: Tuple[int, int, int], num_points: int = 2048) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _generate_sphere_points(self, color: Tuple[int, int, int], num_points: int = 2048) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _generate_cylinder_points(self, color: Tuple[int, int, int], num_points: int = 2048) -> Tuple[np.ndarray, np.ndarray]:
    ...
    def _generate_abstract_points(self, color: Tuple[int, int, int], num_points: int = 2048) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if handler.models_loaded:
        ...
        for prompt in test_prompts:
            ...
            try:
                ...
                point_cloud, figure = await handler.generate_point_cloud_from_text(prompt, output_file)
                if point_cloud:
