...
class PointEGenerator:
    ...
    def __init__(self, config):
        ...
        logger.info(f"Point-E Generator initialized (trimesh available: {TRIMESH_AVAILABLE})")
    ...
    async def generate_asset(self, request: AssetRequest) -> Optional[Generated3DAsset]:
        ...
        try:
            ...
            logger.info(f"Generated asset '{asset.description}' in {generation_time:.2f}s")
            return asset
    ...
    async def _generate_asset_mock(self, request: AssetRequest) -> Generated3DAsset:
        ...
        if primitive:
            ...
            return self.primitive_assets.get("cube", list(self.primitive_assets.values())[0])
    
    def _choose_primitive_for_description(self, description: str) -> Optional[Generated3DAsset]:
        """Choose appropriate primitive based on text description"""
        desc_lower = description.lower()
        
        # Simple keyword matching
        if any(word in desc_lower for word in ["chair", "table", "box", "chest", "crate"]):
        ...
        elif any(word in desc_lower for word in ["stick", "rod", "pole", "tool", "handle"]):
