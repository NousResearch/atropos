...
from evolution.synthetic_users import SyntheticUser, VisualCue, create_realistic_population, calculate_overall_fitness # For potential direct use or if evolver needs it
from evolution.visual_cue_evolver import evolve_visual_cues_multi_objective, initialize_user_population_cache, USER_POPULATION_CACHE
...
async def run_single_segment_evolution_async(
    ...
    print(f"--- Starting evolution for segment: {segment_name} ({len(segment_users)} users) ---")
    ...
    try:
        ...
        if hasattr(asyncio, 'to_thread'): # Python 3.9+
            ...
            logbook_data = log_output # log_output is already serializable list of dicts
        else:
            print(f"Warning: asyncio.to_thread not available. Running segment {segment_name} synchronously.")
        ...
        if pareto_front_cues:
            for cue in pareto_front_cues:
                serialized_cues.append(cue.__dict__ if hasattr(cue, '__dict__') else str(cue))
...
async def main_segmented_runs():
    ...
    full_user_population = create_realistic_population(num_users=200) # Reduced for quicker test
    ...
    segments: Dict[str, Callable[[SyntheticUser], bool]] = {
        ...
        'vr_experts_europe': lambda u: u.vr_experience_level == 'expert' and u.cultural_region == 'europe',
        'accessibility_focus_reaction': lambda u: u.reaction_time_multiplier > 1.3,
        'east_asia_novice': lambda u: u.cultural_region == 'east_asia' and u.vr_experience_level == 'novice'
    }
    ...
    for segment_name, filter_func in segments.items():
        ...
        if not segment_users:
            print(f"Skipping segment '{segment_name}' due to no users matching criteria.")
        ...
        result = await run_single_segment_evolution_async(segment_name, segment_users, num_generations_segment, population_size_segment)
