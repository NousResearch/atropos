...
def format_cue_params_for_csharp(cue_data: Dict[str, Any], class_name_prefix="") -> str:
    ...
    for key, value in cue_data.items():
        if isinstance(value, float):
            lines.append(f"        public static readonly float {class_name_prefix}{key.capitalize()} = {value:.3f}f;")
        elif isinstance(value, int):
            lines.append(f"        public static readonly int {class_name_prefix}{key.capitalize()} = {value};")
        elif isinstance(value, str): # For parameters like animation_name if you add them
            lines.append(f"        public static readonly string {class_name_prefix}{key.capitalize()} = \"{value}\";")
...
def generate_unity_affordance_library(discoveries: Dict[str, Dict[str, Any]]) -> str:
    ...
    for i, (cue_name, cue_data) in enumerate(discoveries.items()):
        ...
        class_name_suffix = "".join(filter(str.isalnum, cue_name.title().replace("_", "").replace(" ", "")))
        enum_entry_name = class_name_suffix
        cue_enum_entries.append(enum_entry_name)

        params_code = format_cue_params_for_csharp(cue_data, class_name_prefix=f"{class_name_suffix}_")
...
if __name__ == '__main__':
    ...
    mock_discoveries_for_unity = {
        "UniversalBreathingCue": {
            "glow": 0.35, "pulse_hz": 2.3, "edge": 0.42, "color_hue": 210.0,
            "color_saturation": 0.8, "color_value": 0.9, "particle_density": 0.1,
            "particle_speed": 0.5, "animation_type": 2, # 2 for breathe
            "size_change_amplitude": 0.15, "blur_amount": 0.1
        },
        "YouthHighEnergyCue": {
            "glow": 0.75, "pulse_hz": 4.1, "edge": 0.3, "color_hue": 60.0, 
            "color_saturation": 0.9, "color_value": 1.0, "particle_density": 0.8, 
            "particle_speed": 1.5, "animation_type": 1, # 1 for pulse
            "size_change_amplitude": 0.05, "blur_amount": 0.05
        },
        "SeniorClearContrastCue": {
            "glow": 0.2, "pulse_hz": 1.2, "edge": 0.9, "color_hue": 240.0, 
            "color_saturation": 0.7, "color_value": 0.8, "particle_density": 0.0,
            "particle_speed": 0.2, "animation_type": 0, # 0 for static
            "size_change_amplitude": 0.0, "blur_amount": 0.0
        }
    }
