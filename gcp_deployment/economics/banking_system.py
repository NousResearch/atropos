...
class BankAccount:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BankAccount":
        ...
        data["transactions"] = [BankTransaction.from_dict(txn_data) for txn_data in data.get("transactions", [])]
...
class CreditScoreCalculator:
    ...
    def _calculate_payment_history_score(self, account: BankAccount, loans: List[Loan]) -> float:
    ...
    def _calculate_debt_utilization_score(self, account: BankAccount, loans: List[Loan]) -> float:
    ...
    def _calculate_new_credit_score(self, account: BankAccount, loans: List[Loan]) -> float:
...
class BankingSystem:
    ...
    def __init__(self, database_handler: Optional[Any] = None, current_step_getter: Optional[callable] = None):
        ...
        self.agent_accounts: Dict[str, List[str]] = defaultdict(list)  # agent_id -> account_ids
        ...
        self.get_current_step = current_step_getter # Function to get current simulation step
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "agent_accounts": {agent_id: list(acc_ids) for agent_id, acc_ids in self.agent_accounts.items()}
            # Config-like attributes (fees, rates, etc.) are not serialized as they are part of initial setup
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any], database_handler: Optional[Any] = None, current_step_getter: Optional[callable] = None) -> "BankingSystem":
        ...
        system.accounts = {acc_id: BankAccount.from_dict(acc_data) 
                           for acc_id, acc_data in data.get("accounts", {}).items()}
        system.loans = {loan_id: Loan.from_dict(loan_data) 
        ...
        for agent_id, acc_ids_list in agent_accounts_data.items():
            system.agent_accounts[agent_id] = list(acc_ids_list) # Ensure it's a list of strings
            
        logger.info(f"BankingSystem state restored with {len(system.accounts)} accounts, {len(system.loans)} loans.")
        return system
    
    async def create_account(self, agent_id: str, account_type: AccountType = AccountType.CHECKING, 
                          initial_deposit: float = 0.0, current_step: Optional[int] = None) -> Optional[BankAccount]:
        ...
        try:
            ...
            logger.info(f"Created {account_type.value} account {account_id} for agent {agent_id} with ${initial_deposit:.2f}")
            return account
        except Exception as e:
            logger.error(f"Error creating account for agent {agent_id}: {e}", exc_info=True)
            return None
    
    async def process_transaction(self, account_id: str, transaction_type: TransactionType, 
                              amount: float, description: str = "", counterparty_id: Optional[str] = None, current_step: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if transaction_type in [TransactionType.DEPOSIT, TransactionType.SALARY, TransactionType.INTEREST_PAYMENT]:
                new_balance = account.balance + amount
            elif transaction_type in [TransactionType.WITHDRAWAL, TransactionType.FEE, TransactionType.LOAN_PAYMENT]:
                new_balance = account.balance - amount
            else:
            ...
            if new_balance < -account.overdraft_limit:
                logger.warning(f"Transaction exceeds overdraft limit for account {account_id}")
                return False
            ...
            account.available_balance = new_balance  # Simplified - in reality would account for holds
            ...
            step_to_log = current_step if current_step is not None else (self.get_current_step() if self.get_current_step else -1)
            if self.db_handler:
                try: await self.db_handler.save_banking_transaction(transaction.to_dict(), step_to_log, account.owner_id)
                except Exception as e_db: logger.error(f"DB Error saving transaction {transaction.transaction_id} for {account_id}: {e_db}", exc_info=True)
            
            if fee_txn:
                # Save fee transaction to DB
                if self.db_handler:
                    try: await self.db_handler.save_banking_transaction(fee_txn.to_dict(), step_to_log, account.owner_id)
                    except Exception as e_db_fee: logger.error(f"DB Error saving fee transaction for {account_id}: {e_db_fee}", exc_info=True)
            
            logger.debug(f"Processed {transaction_type.value}: ${amount:.2f} for account {account_id}")
            return True
        except Exception as e:
            logger.error(f"Error processing transaction for account {account_id} ({transaction_type.value}, {amount}): {e}", exc_info=True)
            return False
    
    async def transfer_funds(self, from_account_id: str, to_account_id: str, amount: float, 
                              description: str = "Transfer", current_step: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if not await self.process_transaction(from_account_id, TransactionType.TRANSFER, -amount, 
                                          f"Transfer to {to_account_id}: {description}", to_account.owner_id, current_step=current_step):
                return False
            
            # Process deposit to destination account
            if not await self.process_transaction(to_account_id, TransactionType.TRANSFER, amount,
                                          f"Transfer from {from_account_id}: {description}", from_account.owner_id, current_step=current_step):
                # Reverse the withdrawal if deposit fails
                await self.process_transaction(from_account_id, TransactionType.TRANSFER, amount,
                                       f"Reversal - Transfer to {to_account_id} failed", current_step=current_step)
                return False
            
            logger.info(f"Transferred ${amount:.2f} from {from_account_id} to {to_account_id}")
            return True
        except Exception as e:
            logger.error(f"Error transferring funds from {from_account_id} to {to_account_id}: {e}", exc_info=True)
            return False
    
    async def apply_for_loan(self, agent_id: str, loan_type: LoanType, amount: float, 
                              purpose: str, term_months: int = 60, current_step: Optional[int] = None) -> Optional[Loan]:
        ...
        try:
            ...
            credit_adjustment = (750 - account.credit_score) / 1000.0  # Lower score = higher rate
            ...
            logger.error(f"Error applying for loan for agent {agent_id} ({loan_type.value}, {amount}): {e}", exc_info=True)
            return None
    
    async def approve_loan(self, loan_id: str, current_step: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # Disburse loan funds
            step = current_step if current_step is not None else (self.get_current_step() if self.get_current_step else -1)
            if await self.process_transaction(primary_account_id, TransactionType.DEPOSIT, net_disbursement,
                                      f"Loan disbursement - {loan.loan_id}", current_step=step):
                
                # Update loan status
                loan.status = LoanStatus.ACTIVE
                loan.first_payment_date = time.time() + (30 * 24 * 3600)  # First payment in 30 days
                ...
                if origination_fee > 0:
                    await self.process_transaction(primary_account_id, TransactionType.FEE, origination_fee,
                                           f"Loan origination fee - {loan.loan_id}", current_step=step)
                
                logger.info(f"Disbursed loan {loan_id}: ${net_disbursement:.2f} (after ${origination_fee:.2f} fee)")
                return True
            
            return False
        except Exception as e:
            logger.error(f"Error approving/disbursing loan {loan_id}: {e}", exc_info=True)
            return False
    
    async def process_loan_payment(self, loan_id: str, payment_amount: float, current_step: Optional[int] = None) -> bool:
        ...
        try:
            ...
            # Process payment
            step = current_step if current_step is not None else (self.get_current_step() if self.get_current_step else -1)
            if await self.process_transaction(primary_account_id, TransactionType.LOAN_PAYMENT, payment_amount,
                                      f"Loan payment - {loan_id}", current_step=step):
                
                # Calculate interest and principal portions
                interest_portion = loan.remaining_balance * (loan.interest_rate / 12.0)
                principal_portion = min(payment_amount - interest_portion, loan.remaining_balance)
            ...
            logger.error(f"Error processing loan payment for {loan_id}: {e}", exc_info=True)
            return False
    
    def _assess_loan_application(self, account: BankAccount, loan: Loan) -> Dict[str, Any]:
        """Assess loan application and calculate approval probability"""
        
        # Get agent's existing loans
        agent_loans = [l for l in self.loans.values() if l.borrower_id == account.owner_id and l.loan_id != loan.loan_id]
        
        # Update credit score
        account.credit_score = self.credit_calculator.calculate_credit_score(account, agent_loans)
        ...
        probability = base_probability * credit_factor * income_factor * debt_factor * collateral_factor
    ...
    def _estimate_income_factor(self, account: BankAccount) -> float:
        ...
        recent_deposits = [txn for txn in account.transactions[-30:] 
    ...
    async def process_monthly_interest(self, current_step: Optional[int] = None):
        ...
        try:
            ...
            step = current_step if current_step is not None else (self.get_current_step() if self.get_current_step else -1)
            for account_id, account in list(self.accounts.items()): # Use list copy for safe iteration if modifying dict
                if account.balance > 0 and account.interest_rate > 0 and account.is_active:
                    monthly_interest = account.balance * (account.interest_rate / 12.0)
                    if monthly_interest > 0.001: # Process only if interest is meaningful
                        await self.process_transaction(account_id, TransactionType.INTEREST_PAYMENT, monthly_interest, "Monthly interest earned", current_step=step)
        except Exception as e:
    ...
    def get_agent_financial_summary(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            accounts = [self.accounts[acc_id] for acc_id in agent_account_ids if acc_id in self.accounts]
            
            # Get all loans
            agent_loans = [loan for loan in self.loans.values() if loan.borrower_id == agent_id]
            
            # Calculate totals
            total_balance = sum(acc.balance for acc in accounts)
            total_debt = sum(loan.remaining_balance for loan in agent_loans if loan.status == LoanStatus.ACTIVE)
            ...
            for account in accounts:
                all_transactions.extend(account.transactions[-10:])  # Last 10 transactions per account
            ...
            logger.error(f"Error getting financial summary for agent {agent_id}: {e}", exc_info=True)
            return {"error": f"Could not retrieve financial summary for agent {agent_id}"}
    
    def get_banking_statistics(self) -> Dict[str, Any]:
        """Get comprehensive banking system statistics"""
        
        try:
            total_deposits = sum(acc.balance for acc in self.accounts.values() if acc.balance > 0)
            total_loans = sum(loan.remaining_balance for loan in self.loans.values() 
                             if loan.status == LoanStatus.ACTIVE)
            ...
            credit_scores = [acc.credit_score for acc in self.accounts.values() if acc.credit_score > 0]
            avg_credit_score = sum(credit_scores) / len(credit_scores) if credit_scores else 0
            
            # Default rate
            defaulted_loans = [loan for loan in self.loans.values() if loan.status == LoanStatus.DEFAULTED]
    ...
    def get_concise_account_summary_for_llm(self, agent_id: str) -> Optional[str]:
        ...
        try:
            ...
            if financial_summary.get("account_details"):
                primary_account = financial_summary["account_details"][0] # Use first account as primary for summary
                ...
                parts.append(f"Balance: {financial_summary.get('total_balance', 0.0):.2f}") # Fallback to total balance

            # Loan information
            active_loans = []
            if financial_summary.get("loan_details"):
                for loan_detail in financial_summary["loan_details"]:
                    if loan_detail.get("status") == LoanStatus.ACTIVE.value: # Ensure comparison with enum value if needed
                        active_loans.append(loan_detail)
            
            if active_loans:
                num_active_loans = len(active_loans)
                total_active_debt = sum(loan.get("remaining_balance", 0.0) for loan in active_loans)
                parts.append(f"Active Loans: {num_active_loans}, Total Debt: {total_active_debt:.2f}")
            else:
            ...
            logger.error(f"Error getting concise account summary for {agent_id}: {e}", exc_info=True)
