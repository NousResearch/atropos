...
class Market:
    ...
    def __init__(self, resource_type: ResourceType, base_price: float = 1.0):
        ...
        logger.debug(f"Created market for {resource_type.value} with base price {base_price}")
    
    def add_order(self, order: TradeOrder) -> bool:
        """Add trade order to the market"""
        try:
            if order.resource_type != self.resource_type:
                logger.warning(f"Order resource type {order.resource_type} doesn't match market {self.resource_type}")
                return False
            
            # Add to appropriate order book
            if order.order_type == TradeOrderType.BUY:
                self.buy_orders.append(order)
                self.buy_orders.sort(key=lambda x: (-x.price_per_unit, x.created_time))  # Highest price first
                self.total_demand += order.quantity
            else:
                self.sell_orders.append(order)
                self.sell_orders.sort(key=lambda x: (x.price_per_unit, x.created_time))  # Lowest price first
                self.total_supply += order.quantity
            
            logger.debug(f"Added {order.order_type.value} order for {order.quantity} {self.resource_type.value} at {order.price_per_unit}")
            return True
        except Exception as e:
            logger.error(f"Error adding order to market {self.resource_type.value}: {e}", exc_info=True)
            return False
        ...
        try:
            while self.buy_orders and self.sell_orders:
                ...
                if buy_order.price_per_unit >= sell_order.price_per_unit:
                    ...
                    trade_price = (buy_order.price_per_unit + sell_order.price_per_unit) / 2.0
                    ...
                    logger.debug(f"Executed trade in {self.resource_type.value} market: {trade_quantity} units at {trade_price}")
                else:
                    # No more matching orders
                    break
        except Exception as e:
            logger.error(f"Error processing trades in market {self.resource_type.value}: {e}", exc_info=True)
        return transactions
    ...
    def calculate_market_price(self) -> float:
        ...
        try:
            ...
            logger.error(f"Error calculating market price for {self.resource_type.value}: {e}", exc_info=True)
            return self.current_price
    ...
    def update_market_price(self):
        ...
        try:
            ...
            logger.error(f"Error updating market price for {self.resource_type.value}: {e}", exc_info=True)
    
    def get_market_summary(self) -> Dict[str, Any]:
        """Get comprehensive market summary"""
        try:
            best_bid = self.buy_orders[0].price_per_unit if self.buy_orders else 0.0
            best_ask = self.sell_orders[0].price_per_unit if self.sell_orders else float('inf')
            ...
            logger.error(f"Error getting market summary for {self.resource_type.value}: {e}", exc_info=True)
            return {
                "resource_type": self.resource_type.value,
                "current_price": self.current_price,
                "base_price": self.base_price,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Market":
        ...
        market.buy_orders = [TradeOrder.from_dict(order_data) for order_data in data.get("buy_orders", [])]
        market.sell_orders = [TradeOrder.from_dict(order_data) for order_data in data.get("sell_orders", [])]
        ...
        market.recent_transactions = [Transaction.from_dict(txn_data) for txn_data in data.get("recent_transactions", [])]
        return market
...
class MarketSystem:
    ...
    def __init__(self):
        ...
        self.trade_networks: Dict[str, Set[str]] = defaultdict(set)  # Agent trading relationships
    ...
    def submit_order(self, agent_id: str, resource_type: ResourceType, order_type: TradeOrderType,
                    quantity: float, price_per_unit: float, **kwargs) -> Optional[str]:
        ...
        try:
            ...
            if market.add_order(order):
                logger.info(f"Agent {agent_id} submitted {order_type.value} order {order_id}: {quantity} {resource_type.value} at {price_per_unit}")
                return order_id
            
            return None
        except Exception as e:
            logger.error(f"Error submitting order for agent {agent_id} ({resource_type.value}): {e}", exc_info=True)
            return None
    ...
    def process_all_markets(self) -> Dict[ResourceType, List[Transaction]]:
        ...
        try:
            for resource_type, market in self.markets.items():
                if resource_type == ResourceType.CURRENCY: continue # No trading currency against itself
                # Process trades
    ...
    def get_market_price(self, resource_type: ResourceType) -> float:
        ...
        try:
            ...
            logger.error(f"Error getting market price for {resource_type.value}: {e}", exc_info=True)
            return 0.0
    
    def get_price_quote(self, resource_type: ResourceType, quantity: float, 
                       order_type: TradeOrderType) -> Dict[str, float]:
        ...
        try:
            ...
            if order_type == TradeOrderType.BUY:
                ...
                if available_quantity > 0:
                    ...
                    return {
                        ...
                        "market_impact": abs(avg_price - market.current_price) / market.current_price
                    }
            ...
            logger.error(f"Error getting price quote for {resource_type.value}: {e}", exc_info=True)
            return {"error": f"Error generating quote for {resource_type.value}"}
    
    def get_agent_trading_history(self, agent_id: str, days: int = 7) -> List[Transaction]:
        ...
        try:
            ...
            logger.error(f"Error getting agent trading history for {agent_id}: {e}", exc_info=True)
            return []
    ...
    def calculate_agent_trade_reputation(self, agent_id: str) -> Dict[str, float]:
        ...
        try:
            ...
            base_reputation = 0.5
            volume_bonus = min(0.3, total_volume / 1000.0)  # Max 0.3 bonus for high volume
            diversity_bonus = min(0.2, unique_partners / 10.0)  # Max 0.2 bonus for trading diversity
            ...
            return {
                ...
                "reliability": min(1.0, trade_count / 30.0)  # Based on trading frequency
            }
        except Exception as e:
            logger.error(f"Error calculating agent trade reputation for {agent_id}: {e}", exc_info=True)
            return {"reputation": 0.5, "trade_volume": 0.0, "reliability": 0.5}
    ...
    def get_market_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_connections = sum(len(connections) for connections in self.trade_networks.values())
            avg_connections = total_connections / max(1, active_traders)
            
            # Recent activity (last 24 hours)
            recent_cutoff = time.time() - 86400
            recent_transactions = [txn for txn in self.all_transactions if txn.timestamp > recent_cutoff]
    ...
    def _calculate_market_efficiency(self) -> float:
        ...
        try:
            ...
            for market in self.markets.values():
                ...
                best_bid = market.buy_orders[0].price_per_unit if market.buy_orders else 0.0
                best_ask = market.sell_orders[0].price_per_unit if market.sell_orders else float('inf')
                ...
                if best_ask == float('inf') or best_bid == 0.0:
                    ...
                    liquidity_efficiency = min(1.0, total_orders / 10.0)  # Good liquidity = 10+ orders
    ...
    def get_resource_market_summary(self, resource_type: ResourceType) -> Optional[str]:
        ...
        """
        if resource_type not in self.markets:
            logger.warning(f"Market for {resource_type.value} not found when requesting summary.")
            return None
        ...
        try:
            ...
            ask_str = f"{float(ask):.2f}" if ask != "N/A" and ask != float('inf') else "N/A"
            demand_qty_str = f"{float(demand_qty):.0f}" if demand_qty != "N/A" else "N/A"
            supply_qty_str = f"{float(supply_qty):.0f}" if supply_qty != "N/A" else "N/A"
        except ValueError:
             # Fallback if float conversion fails for some reason
            price_str, bid_str, ask_str, demand_qty_str, supply_qty_str = "N/A", "N/A", "N/A", "N/A", "N/A"
    ...
    def get_detailed_resource_market_info(self, resource_type: ResourceType) -> Optional[str]:
        """
        ...
        """
        if resource_type not in self.markets:
            logger.warning(f"Market for {resource_type.value} not found when requesting detailed info.")
        ...
        try:
            ...
            ask_str = f"{float(summary_data.get('best_ask', float('inf'))):.2f}" if summary_data.get('best_ask', float('inf')) != float('inf') else "N/A"
            ...
            return (f"{resource_type.value.capitalize()} Market: Price: {price_str}, "
            ...
            logger.error(f"Error formatting detailed market info for {resource_type.value}: {e}")
            return f"{resource_type.value.capitalize()}: Error retrieving market details."
    ...
    def get_general_market_overview(self) -> Optional[str]:
        ...
        overview_parts = []
        for resource_type, market in self.markets.items():
            if resource_type == ResourceType.CURRENCY: # Usually no need to list currency market price against itself
                continue
            price = market.current_price
            overview_parts.append(f"{resource_type.value.capitalize()} (Price: {price:.2f})")
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "markets": {res_type.value: market.to_dict() for res_type, market in self.markets.items()},
            "all_transactions": [txn.to_dict() for txn in self.all_transactions],
            "trade_networks": {agent_id: list(partners) for agent_id, partners in self.trade_networks.items()}
        }

    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "MarketSystem":
        system = cls()
        system.markets = {ResourceType(res_val): Market.from_dict(market_data) 
                          for res_val, market_data in data.get("markets", {}).items()}
        system.all_transactions = [Transaction.from_dict(txn_data) for txn_data in data.get("all_transactions", [])]
        system.trade_networks = defaultdict(set, {agent_id: set(partners) 
        ...
        base_prices = { ResourceType.FOOD:1.0, ResourceType.MATERIALS:2.0, ResourceType.ENERGY:1.5, ResourceType.LUXURY:10.0, ResourceType.TOOLS:5.0, ResourceType.KNOWLEDGE:3.0, ResourceType.SERVICES:2.5, ResourceType.CURRENCY:1.0 }
        for rt_enum in ResourceType:
            if rt_enum not in system.markets:
                logger.info(f"MarketSystem.from_dict: Initializing missing market for {rt_enum.value} from defaults.")
