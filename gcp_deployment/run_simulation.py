...
Examples:
  ...
  python run_simulation.py --load previous.json --continue 1000
    ...
    parser.add_argument('--world-size', type=int, nargs=2, default=[100, 100],
                       metavar=('WIDTH', 'HEIGHT'),
                       help='World dimensions (default: 100 100)')
    
    # LLM configuration
    parser.add_argument('--llm', choices=['none', 'openai', 'anthropic', 'mock'],
                       default='none',
                       help='LLM provider for agent intelligence (default: none)')
    parser.add_argument('--model', type=str,
                       help='Specific model to use (e.g., gpt-3.5-turbo)')
    ...
    parser.add_argument('--load', type=str,
                       help='Load simulation from file')
    parser.add_argument('--continue', dest='continue_steps', type=int,
...
def load_simulation(filepath: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Error: Simulation file '{filepath}' not found")
        return None
    except json.JSONDecodeError as e:
...
def run_benchmark():
    ...
    print("-" * 50)
    for r in results:
        print(f"{r['label']:<12} {r['agents']:<7} {r['steps']:<6} {r['time']:<8.2f} {r['sps']:<8.1f}")
...
def run_test():
    ...
    print(f"✅ Economic activity: {total_currency} total currency, {total_trades} active traders")
...
def main():
    ...
    if args.load:
        ...
        print(f"📁 Loaded simulation: {sim_data['metadata']['agents']} agents, {sim_data['metadata']['steps']} steps")
    ...
    if args.llm != 'none':
        ...
        if args.llm in ['openai', 'anthropic'] and not args.api_key:
            ...
            print(f"   Set --api-key or use environment variable")
            print(f"   Falling back to rule-based agents")
            llm_provider = LLMProvider.NONE
    
    # Run the simulation
    print(f"🚀 Starting Society Simulation")
    print(f"   Agents: {args.agents}")
    print(f"   Steps: {args.steps}")
    ...
    if args.optimized:
        ...
        world = asyncio.run(run_optimized_simulation(
        ...
        world = asyncio.run(run_intelligent_simulation(
    ...
    if hasattr(world, 'get_agent_insights'):
        ...
        print(f"   Social Density: {social_connections / args.agents:.1f} connections per agent")
        print(f"   Economic Activity: {final_stats['total_resources']['currency']:.0f} total currency")
        print(f"   Cultural Diversity: {len(final_stats['cultural_groups'])} active groups")
        ...
        social_connections = final_stats.get('total_connections', 0)
        print(f"   Social Density: {social_connections / args.agents:.1f} connections per agent")
        print(f"   Economic Activity: {final_stats['total_resources']['currency']} total currency")
        print(f"   Cultural Diversity: {len(final_stats['cultural_groups'])} active groups")
