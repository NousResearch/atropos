...
class FunSearchManager:
    ...
    async def initialize(self):
        ...
        try:
            ...
            if not os.path.exists(self.funsearch_path):
                ...
                print("Please clone FunSearch: git clone https://github.com/deepmind/funsearch.git")
                return False
    ...
    async def discover_vr_functions(
        ...
        try:
            ...
            validated_functions = await self._validate_functions(evolution_result, training_data)
            
            # Store discoveries
            discovery_id = await self._store_discoveries(validated_functions, domain)
            
            print(f"✅ Function discovery completed: {discovery_id}")
            print(f"   Best function fitness: {validated_functions['best_fitness']:.4f}")
            ...
            for result in individual_results:
                ...
                for var in input_vars:
                    if var == "gpu_util":
                        ...
                        complexity = {"main_menu": 1, "gameplay_scene": 2, "stress_test": 3}.get(scene, 1)
    ...
    async def _run_evolution(
        ...
        for generation in range(self.generations):
            ...
            self.current_population = await self._evolve_population(self.current_population, fitness_scores)
            
            # Early stopping if converged
            if generation > 20 and abs(best_fitness_history[-1] - best_fitness_history[-10]) < 0.001:
    ...
    async def _validate_functions(
        ...
        top_functions = sorted(population, key=lambda f: f.get("fitness", 0), reverse=True)[:10]
    ...
    async def _store_discoveries(
        self, 
        validated_functions: Dict[str, Any], 
        domain: str
    ) -> str:
...
"""
    ...
    def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        discoveries = [d for d in os.listdir(discovery_dir) if d.startswith("funsearch_")]
