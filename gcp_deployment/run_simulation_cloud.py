    ...
    def run(self, n_steps):
        ...
        for i in range(n_steps):
            ...
            if i % 20 == 0 or i == n_steps - 1:
                ...
                print(f"Step {i:4d}: SPS: {sps:6.1f}, Energy: {metrics['avg_energy']:.2f}, "
        ...
        return {
            ...
            'summary': {
                'avg_happiness': np.mean([a.happiness for a in self.agents]),
                'total_wealth': sum([a.wealth for a in self.agents]),
                'social_connections': len([a for a in self.agents if len(a.connections) > 0])
            }
        }
...
def main():
    ...
    if args.benchmark:
        ...
        print("-" * 50)
        for r in results:
            print(f"{r['name']:<15} {r['agents']:<8} {r['steps']:<8} "
        ...
        print(f"\n🎯 Simulation Complete!")
        print(f"   Social Density: {results['summary']['social_connections']/args.agents:.1f} connections per agent")
        print(f"   Economic Activity: {int(results['summary']['total_wealth'])} total currency")
