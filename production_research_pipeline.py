...
from enhanced_padres_perplexity import SimplePadresResearch # Needs ANTHROPIC_API_KEY, PERPLEXITY_API_KEY from env
...
class Production24x7Pipeline:
    def __init__(self):
        ...
        if not os.getenv('ANTHROPIC_API_KEY'):
            logger.warning("ANTHROPIC_API_KEY environment variable not set. Claude functionality may be affected.")
        if not os.getenv('PERPLEXITY_API_KEY'):
            logger.warning("PERPLEXITY_API_KEY environment variable not set. Perplexity functionality may be affected.")
        ...
        try:
            ...
            logger.error(f"Error during Production24x7Pipeline initialization: {e}", exc_info=True)
    ...
    def run_experiment_batch(self, batch_size: int = 10):
        ...
        master_research_query = "Latest spatial reasoning AI research 2024 2025 LLM physics simulation"
        logger.info(f"Performing batch-level Perplexity search for query: {master_research_query}")
        batch_perplexity_result = self.researcher.search_perplexity(master_research_query)
        if "Error:" in str(batch_perplexity_result): # Check if perplexity call resulted in an error string
            logger.warning(f"Batch-level Perplexity search failed: {batch_perplexity_result}. This will be noted in stored data.")
        ...
        for i in range(batch_size):
            ...
            try:
                ...
                padres_data_for_raw = experiment_core_data.get('padres_api_response', {})
                try:
                ...
                perplexity_research_str = str(batch_perplexity_result) if batch_perplexity_result else ''
                ...
                logger.info(f"Complete data for BigQuery (experiment_id: {final_data_for_bq.get('experiment_id')}): {json.dumps(final_data_for_bq, default=str)}")
                ...
                batch_results_summary.append({
                    ...
                    "status": "success",
                    "experiment_id": experiment_core_data.get('experiment_id', 'N/A'),
                })
                logger.info(f"Experiment {experiment_number} processed and data storage initiated.")
                
            except Exception as e:
                logger.error(f"❌ Experiment {experiment_number} failed during pipeline processing: {e}", exc_info=True)
                batch_results_summary.append({
                    "experiment_number": experiment_number,
                    "status": "error",
                    "error_message": str(e)
        ...
        logger.info(f"Experiment batch completed. Processed {len(batch_results_summary)} experiments.")
        return {
    ...
    def analyze_recent_data(self):
        ...
        try:
            ...
            trends = self.data_manager.get_success_rate_trends() # Assumes this looks at a relevant period, e.g., 30 days
            ...
            return {
                "message": "Failed to analyze recent data.",
                "error": str(e)
            }
...
if __name__ == '__main__':
    ...
    logger.info("Running Production24x7Pipeline locally (with batch Perplexity call)...")
    if not os.getenv("GOOGLE_CLOUD_PROJECT"):
        ...
        logger.info(f"Experiment Batch Results: {json.dumps(experiment_results, indent=2)}")
