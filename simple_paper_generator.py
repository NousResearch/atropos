...
def analyze_discoveries(results: List[Dict]) -> Dict[str, Any]:
    ...
    insights = {
        ...
        "total_discoveries": sum(len(r.get("best_cues_pareto_front", [])) for r in results if r.get("status") == "completed"),
        "segment_analysis": {},
    ...
    if "young_gamers" in insights["segment_analysis"] and "seniors_low_va" in insights["segment_analysis"]:
        ...
        if young_glow > senior_glow + 0.1:
            insights["key_findings"].append(f"Young gamers prefer higher glow intensity ({young_glow:.2f}) compared to seniors ({senior_glow:.2f})")
        ...
        if young_pulse > senior_pulse + 0.5:
            insights["key_findings"].append(f"Young gamers prefer faster pulse rates ({young_pulse:.2f}Hz) vs seniors ({senior_pulse:.2f}Hz)")
    ...
    if all_cues:
        insights["convergence_patterns"] = {
            ...
            "preferred_animation": max(set(c.get('animation_type', 0) for c in all_cues), key=lambda x: sum(1 for c in all_cues if c.get('animation_type', 0) == x))
        }
...
def main():
    ...
    # Print summary
    print("\n=== PAPER SUMMARY ===")
    print(f"Title: Evolutionary Discovery of Demographic-Specific VR Affordances")
    print(f"Discoveries: {insights['total_discoveries']} optimal visual cue configurations")
