...
class ExperimentOrchestrator:
    ...
    async def fetch_experiment_config(self, experiment_id: str) -> dict:
        ...
        """
        print(
            "Fetching experiment config for "{experiment_id}' using MCP ({self.default_firestore_mcp})."
        )
        try:
            ...
            if config_data and not config_data.get("error"):
                ...
                raise ValueError(
                    "Experiment config "{experiment_id}' not found or error fetching: {config_data.get('error')}"
                )
        except Exception as e:
            print("Error fetching experiment config "{experiment_id}': {e}")
    ...
    async def run_experiment_batch(self, batch_config: dict):
        ...
        for exp_id in experiment_ids_to_run:
            try:
                print("Starting experiment "{exp_id}' from batch '{batch_name}'.")
                config = await self.fetch_experiment_config(exp_id)
                await self.run_single_experiment(config)
                print("Successfully completed experiment "{exp_id}'.")
            except Exception as e:
                print(
                    "Failed to run experiment "{exp_id}' from batch '{batch_name}': {e}"
                )
                # Log error, potentially mark experiment as failed
        print("Experiment batch "{batch_name}' finished.")
    ...
    async def run_single_experiment(self, experiment_config: dict):
        ...
        for i in range(num_trials):
            ...
            try:
                ...
                print("    Formatted prompt (first 80 chars): "{prompt[:80]}...'")
                ...
                if (
                    not isinstance(action_response_dict, dict)
                    or action_response_dict.get("error")
                    or action_response_dict.get("status") != "success"
                ):
                ...
                print(
                    f"    Executing action in Padres env ({self.default_padres_mcp})."
                )
                execution_result = await self.mcp_manager.call_tool(
                    ...
                    parameters={
                        ...
                        "action_name": action_from_llm,
                    },
                )
                if not isinstance(execution_result, dict) or execution_result.get(
                    "error"
                ):
                    raise RuntimeError(
                        f"Padres action execution failed: {execution_result}"
                    )
                ...
                trial_data = {
                    ...
                    "final_score_for_trial": (
                        execution_result.get("score_delta")
                        if isinstance(execution_result, dict)
                        else execution_result.get("score")
                    ),
    ...
    class MockMCPManagerForOrchestrator:
        ...
        async def call_tool(
            ...
            if server_name == "firestore_main" and tool_name == "get_document":
                ...
                return {
                    "text": "Mock LLM action for prompt: "{parameters.get('prompt_or_messages', '')[:20]}...'",
                    "status": "success",
                }
            ...
            return {
                "error": "Mock tool "{tool_name}' not implemented for server '{server_name}'."
            }
