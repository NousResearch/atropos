        ...
        for server_instance in mcp_manager.mcp_servers.values():
            if hasattr(server_instance, "close_client") and asyncio.iscoroutinefunction(server_instance.close_client):
                try:
                    ...
                    print(f"Closed client for MCP server {server_instance.server_name} in Celery task.")
                except Exception as e_close:
                    print(f"Error closing client for server {server_instance.server_name} in Celery task: {e_close}")
            elif hasattr(server_instance, "close_client"):
                try:
                    server_instance.close_client()
                    print(f"Closed client for MCP server {server_instance.server_name} (sync) in Celery task.")
                except Exception as e_close:
                    print(f"Error closing client for server {server_instance.server_name} (sync) in Celery task: {e_close}")
...
def run_experiment_task(experiment_id: str, pipeline_config_dict: dict) -> dict:
    ...
    print(f"Celery task: run_experiment_task started for experiment_id: {experiment_id}")
    ...
    try:
        ...
        if not experiment_config_dict_fetched or experiment_config_dict_fetched.get("error"):
            error_msg = f"Failed to fetch experiment config for {experiment_id}: {experiment_config_dict_fetched.get('error', 'Not found')}"
            print(error_msg)
            return {"status": "error", "experiment_id": experiment_id, "error": error_msg}

        result = loop.run_until_complete(
            orchestrator.run_single_experiment(experiment_config_dict_fetched)
        )
        print(f"Celery task: run_experiment_task completed for experiment_id: {experiment_id}. Result: {json.dumps(result, indent=2)}")
        return result
        ...
        print(f"Celery task: run_experiment_task finished for experiment_id: {experiment_id}")


@celery_app.task(name="pipeline_tasks.run_experiment_batch")
def run_experiment_batch_task(batch_config_dict: dict, pipeline_config_dict: dict) -> dict:
    """
    ...
    print(f"Celery task: run_experiment_batch_task started for batch: '{batch_name}' with IDs: {experiment_ids}")
    loop = asyncio.get_event_loop()
    orchestrator = None
    mcp_manager = None
    try:
        if not experiment_ids:
            return {"status": "error", "batch_name": batch_name, "error": "No experiment_ids provided in batch_config."}
        ...
        print(f"Celery task: run_experiment_batch_task completed for batch '{batch_name}'. Result: {json.dumps(result, indent=2)}")
        return result
    except FileNotFoundError as fnf:
        print(f"Error in run_experiment_batch_task: {fnf}")
        return {"status": "error", "batch_name": batch_name, "error": str(fnf)}
    except Exception as e:
        print(f"Error in run_experiment_batch_task for batch '{batch_name}': {e}")
        return {"status": "error", "batch_name": batch_name, "error": str(e)}
    finally:
        if mcp_manager:
            loop.run_until_complete(_close_mcp_manager_clients(mcp_manager))
        print(f"Celery task: run_experiment_batch_task finished for batch: '{batch_name}'")
...
print("Celery tasks (updated for direct pipeline_config_dict) defined in backend_services.celery_tasks")
...
def analyze_experiment_results_task(experiment_ids: List[str], pipeline_config_dict: dict) -> dict:
    """Celery task to analyze experimental results using ResearchAssistant."""
    print(f"Celery task: analyze_experiment_results_task started for experiment_ids: {experiment_ids}")
    loop = asyncio.get_event_loop()
    mcp_manager = None
    try:
        # pipeline_config = _get_pipeline_config(config_path_str) # No longer using path
        ...
        print(f"Celery task: analyze_experiment_results_task completed for {experiment_ids}. Result: {json.dumps(result, indent=2)}")
        return result
    except FileNotFoundError as fnf:
        print(f"Error in analyze_experiment_results_task: {fnf}")
        return {"status": "error", "inputs": {"experiment_ids": experiment_ids}, "error": str(fnf)}
    except Exception as e:
        print(f"Error in analyze_experiment_results_task for {experiment_ids}: {e}")
        return {"status": "error", "inputs": {"experiment_ids": experiment_ids}, "error": str(e)}
    finally:
        if mcp_manager:
            loop.run_until_complete(_close_mcp_manager_clients(mcp_manager))
        print(f"Celery task: analyze_experiment_results_task finished for {experiment_ids}")


@celery_app.task(name="pipeline_tasks.perform_deep_analysis")
def perform_deep_analysis_task(analysis_config: dict, pipeline_config_dict: dict) -> dict:
    """Celery task to perform deep analysis using AnalysisEngine."""
    analysis_name = analysis_config.get("name", "Unnamed Analysis")
    print(f"Celery task: perform_deep_analysis_task started for analysis: '{analysis_name}'")
    loop = asyncio.get_event_loop()
    mcp_manager = None
    try:
        # pipeline_config = _get_pipeline_config(config_path_str) # No longer using path
        ...
        result = loop.run_until_complete(
            engine.perform_deep_analysis(analysis_config)
        )
        print(f"Celery task: perform_deep_analysis_task completed for '{analysis_name}'. Result: {json.dumps(result, indent=2)}")
        return result
    except FileNotFoundError as fnf:
        print(f"Error in perform_deep_analysis_task: {fnf}")
        return {"status": "error", "inputs": {"analysis_config_name": analysis_name}, "error": str(fnf)}
    except Exception as e:
        print(f"Error in perform_deep_analysis_task for '{analysis_name}': {e}")
        return {"status": "error", "inputs": {"analysis_config_name": analysis_name}, "error": str(e)}
    finally:
        if mcp_manager:
            loop.run_until_complete(_close_mcp_manager_clients(mcp_manager))
        print(f"Celery task: perform_deep_analysis_task finished for '{analysis_name}'")
...
def generate_report_task(report_config: dict, pipeline_config_dict: dict) -> dict:
    ...
    print(f"Celery task: generate_report_task started for report: '{report_name}'")
    loop = asyncio.get_event_loop()
    mcp_manager = None
    try:
        # pipeline_config = _get_pipeline_config(config_path_str) # No longer using path
        ...
        result = loop.run_until_complete(
            reporter.generate_report(report_config)
        )
        print(f"Celery task: generate_report_task completed for '{report_name}'. Result: {json.dumps(result, indent=2)}")
        return result
    except FileNotFoundError as fnf:
        print(f"Error in generate_report_task: {fnf}")
        return {"status": "error", "inputs": {"report_config_name": report_name}, "error": str(fnf)}
    except Exception as e:
        print(f"Error in generate_report_task for '{report_name}': {e}")
        return {"status": "error", "inputs": {"report_config_name": report_name}, "error": str(e)}
    finally:
        if mcp_manager:
            loop.run_until_complete(_close_mcp_manager_clients(mcp_manager))
        print(f"Celery task: generate_report_task finished for '{report_name}'") 