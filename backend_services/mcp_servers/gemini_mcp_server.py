...
class GeminiMCPServer(BaseMCPServer):
    ...
    def _validate_config(self):
        ...
        if "model_name" not in self.config:
            raise ValueError(f"Missing 'model_name' in config for GeminiMCPServer: {self.server_name}")
        ...
        if isinstance(config_api_key, str) and config_api_key.startswith("ENV_"):
            ...
            print(f"  GeminiMCPServer '{self.server_name}': Config requests API key from env var '{env_var_name_in_config}'.")
            loaded_env_api_key = os.environ.get(env_var_name_in_config)
            if loaded_env_api_key:
                self.config["api_key"] = loaded_env_api_key # Replace placeholder with actual key
                self.configured_api_key_source = f"environment ({env_var_name_in_config}) via config"
            elif os.environ.get("GOOGLE_API_KEY"): # Fallback to standard GOOGLE_API_KEY if specific ENV_ one not found
                self.config["api_key"] = os.environ.get("GOOGLE_API_KEY")
                self.configured_api_key_source = "environment (GOOGLE_API_KEY fallback)"
                print(f"  GeminiMCPServer '{self.server_name}': '{env_var_name_in_config}' not set, using GOOGLE_API_KEY fallback.")
            else:
                print(f"Warning: GeminiMCPServer '{self.server_name}': API key specified as ENV_ variable '{env_var_name_in_config}' in config, but it's not set, and GOOGLE_API_KEY is also not set.")
        ...
        elif os.environ.get("GOOGLE_API_KEY"): # No api_key in config, check standard env var
            self.config["api_key"] = os.environ.get("GOOGLE_API_KEY") # Store for SDK configure()
            self.configured_api_key_source = "environment (GOOGLE_API_KEY)"
        else:
            print(f"Warning: GeminiMCPServer '{self.server_name}': No API key found in config or GOOGLE_API_KEY env var. SDK initialization may fail.")
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if api_key_to_use:
                ...
                print(f"Gemini SDK configured for '{self.server_name}' using API key from: {self.configured_api_key_source or 'unknown'}. ")
            elif self.configured_api_key_source and "GOOGLE_API_KEY" in self.configured_api_key_source : # Relies on env GOOGLE_API_KEY set previously
                 print(f"Gemini SDK for '{self.server_name}' will use GOOGLE_API_KEY from environment (source: {self.configured_api_key_source}).")
                ...
                print(f"Attempting to initialize Gemini model for '{self.server_name}' without explicit API key; SDK will search GOOGLE_API_KEY or fail.")
                ...
                for setting in raw_safety_settings:
                    if isinstance(setting, dict) and "category" in setting and "threshold" in setting:
                        try:
                            ...
                            safety_settings_sdk.append(SafetySetting(category=category_enum, threshold=threshold_enum))
                        except KeyError as ke:
                            print(f"Warning: Invalid HarmCategory or HarmBlockThreshold string in safety_settings for {self.server_name}: {ke}. Setting will be skipped.")
                    else:
                        print(f"Warning: Invalid safety_setting format for {self.server_name}, skipping: {setting}")
            ...
            print(f"Gemini GenerativeModel '{model_name}' initialized for server '{self.server_name}'.")
            
            # Test with a count_tokens call
            test_token_count = self.model.count_tokens("test initialization")
            print(f"  Successful test: count_tokens('test initialization') = {test_token_count.total_tokens} for {self.server_name}")

        except Exception as e:
            print(f"CRITICAL: Error initializing Gemini client/model for {self.server_name}: {e}")
            self.model = None
            # Depending on requirements, you might want to raise an exception here to halt startup
            # if this MCP server is critical.
            # raise

    def _generate_content(self, prompt_or_messages: Any, generation_config_override: Optional[Dict] = None, safety_settings_override: Optional[List[Dict]] = None, stream: bool = False) -> Dict:
        """
        ...
        """
        if not self.model:
            return {"error": f"Gemini model not initialized for {self.server_name}. Cannot generate content.", "status": "failure"}

        print(f"  {self.server_name}._generate_content (model: {self.model.model_name}) called. Stream: {stream}")
        if isinstance(prompt_or_messages, str):
            print(f"    Prompt (first 50 chars): '{prompt_or_messages[:50]}...'")
        else: # Assuming list of parts or messages
            print(f"    Content/Messages (type: {type(prompt_or_messages)}, count: {len(prompt_or_messages) if isinstance(prompt_or_messages, list) else 'N/A'})")

        gen_config_sdk_override: Optional[GenerationConfig] = None
        if isinstance(generation_config_override, dict):
            gen_config_sdk_override = genai.types.GenerationConfig(**generation_config_override)
        ...
        if isinstance(safety_settings_override, list):
            ...
            for setting in safety_settings_override:
                 if isinstance(setting, dict) and "category" in setting and "threshold" in setting:
                    try:
                        safety_sdk_override.append(SafetySetting(category=HarmCategory[setting["category"]], threshold=HarmBlockThreshold[setting["threshold"]]))
                    except KeyError: pass # Skip invalid

        try:
            if stream:
                # For now, MCP tool simulates stream by concatenating. True streaming would need a different return type.
                print("    Streaming requested. SDK will stream, this method will concatenate for a single response.")
                ...
                for chunk in response_chunks:
                    if chunk.parts:
                        full_text.append(chunk.text) # chunk.text directly for convenience
                    final_response_candidate = chunk.candidates[0] if chunk.candidates else None # Keep the last candidate info
                
                response_text = "".join(full_text)
                finish_reason = final_response_candidate.finish_reason.name if final_response_candidate and final_response_candidate.finish_reason else "STREAM_END_UNKNOWN"
                safety_ratings = str(final_response_candidate.safety_ratings) if final_response_candidate and final_response_candidate.safety_ratings else "N/A"
            else:
                ...
                try:
                    ...
                    if response.prompt_feedback and response.prompt_feedback.block_reason:
                        return {"text": "", "status": "blocked", "error": "Content generation blocked by safety filter", "finish_reason": "SAFETY", "prompt_feedback": str(response.prompt_feedback), "candidates_info": str(response.candidates)}
                    return {"text": "", "status": "failure", "error": "No content in response or value error", "details": str(ve), "prompt_feedback": str(response.prompt_feedback), "candidates_info": str(response.candidates)}
                ...
                safety_ratings = str(response.candidates[0].safety_ratings) if response.candidates and response.candidates[0].safety_ratings else "N/A"
            ...
            print(f"Error during Gemini content generation for {self.server_name}: {type(e).__name__} - {e}")
            return {"text": "", "status": "failure", "error": f"Gemini API call failed for {self.server_name}", "details": str(e)}
    ...
    def _count_tokens(self, content: Any) -> Dict:
        ...
        """
        if not self.model:
            return {"error": f"Gemini model not initialized for {self.server_name}. Cannot count tokens.", "total_tokens": -1}
        try:
            token_count_response = self.model.count_tokens(content)
            return {"total_tokens": token_count_response.total_tokens, "status": "success"}
        except Exception as e:
            print(f"Error counting tokens with Gemini for {self.server_name}: {e}")
            return {"error": "Token counting failed", "details": str(e), "total_tokens": -1}
    ...
    def call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        if tool_name == "generate_content":
            ...
            if prompt_or_messages is None:
                raise ValueError(f"Missing 'prompt_or_messages' for 'generate_content' tool in {self.server_name}.")
            ...
            if content is None:
                raise ValueError(f"Missing 'content' for 'count_tokens' tool in {self.server_name}.")
...
if __name__ == "__main__":
    ...
    if not gemini_api_key_from_env:
        print("  WARNING: GOOGLE_API_KEY environment variable not set. Test may fail if API key is strictly required by SDK.")
    ...
    sample_gemini_config = {
        ...
        "generation_config": {"temperature": 0.2, "max_output_tokens": 50, "top_p": 0.8, "top_k": 30},
        "safety_settings": [
            {"category": "HARM_CATEGORY_HARASSMENT", "threshold": "BLOCK_MEDIUM_AND_ABOVE"},
            {"category": "HARM_CATEGORY_HATE_SPEECH", "threshold": "BLOCK_MEDIUM_AND_ABOVE"}
        ]
    }
    if config_api_key: # Only add to config if it was set as a dummy (because env var was missing)
        sample_gemini_config["api_key"] = config_api_key

    gemini_server = None
    try:
        gemini_server = GeminiMCPServer(server_name="gemini_pro_sdk_test", config=sample_gemini_config)

        if gemini_server and gemini_server.model:
            print(f"\n--- Test: Simple Text Prompt ---")
            gen_params_text = {"prompt_or_messages": "What are the first 5 prime numbers? Call should succeed even with dummy key if SDK handles it gracefully for some ops like count, or if it truly allows init without key if env var also missing."}
            response_text = gemini_server.call_tool("generate_content", gen_params_text)
            print(f"Generate Content (text) Response:\n{json.dumps(response_text, indent=2)}")

            print(f"\n--- Test: Chat-like Messages ---")
            gen_params_chat = {
                "prompt_or_messages": [
                    {"role": "user", "parts": [{"text": "Hi Gemini, can you write a short poem about code? Call should succeed even with dummy key if SDK handles it gracefully for some ops like count, or if it truly allows init without key if env var also missing."}]},
                ],
            ...
            print(f"Generate Content (chat) Response :\n{json.dumps(response_chat, indent=2)}")
            
            print(f"\n--- Test: Token Counting ---")
            count_params_str = {"content": "How many tokens are in this sentence? This should work with a configured model even if API key is dummy/missing for the count itself."}
            ...
            count_params_chat_content =  [{ "role": "user", "parts": [{"text": "Hello!"}] }]
            token_count_chat = gemini_server.call_tool("count_tokens", {"content": count_params_chat_content})
            print(f"Token count (chat content): {json.dumps(token_count_chat, indent=2)}")
