...
class BigQueryMCPServer(BaseMCPServer):
    ...
    def _validate_config(self):
        ...
        if "project_id" not in self.config:
            raise ValueError(f"Missing 'project_id' in config for BigQueryMCPServer: {self.server_name}")
    ...
    def _initialize_client(self):
        ...
        try:
            self.client = bigquery.Client(project=self.project_id, location=self.default_location)
            # Test connection by listing datasets (lightweight call)
            print(f"BigQuery client initialized for project '{self.project_id}' in location '{self.client.location}' for server '{self.server_name}'. Attempting to list datasets (max 1)...")
            datasets = list(self.client.list_datasets(max_results=1))
            if datasets:
                print(f"  Successfully listed at least one dataset: {datasets[0].dataset_id}")
            else:
                print("  No datasets found or not permitted to list, but client initialized.")
        except Exception as e:
            print(f"CRITICAL: Error initializing BigQuery client for {self.server_name}: {e}")
    ...
    def _run_query(self, query: str, query_parameters: list = None, to_dataframe: bool = False, target_dataset_id: str = None, target_table_id: str = None, write_disposition: str = "WRITE_EMPTY", create_disposition: str = "CREATE_IF_NEEDED") -> any:
        """
        Executes a SQL query in BigQuery. Can also write results to a destination table.
        """
        if not self.client:
            return {"error": f"BigQuery client not initialized for {self.server_name}. Query cannot run."}
        ...
        if query_parameters:
            ...
            for p in query_parameters:
                if isinstance(p, dict) and all(k in p for k in ["name", "type", "value"]):
                    bq_params.append(bigquery.ScalarQueryParameter(p["name"], p["type"].upper(), p["value"]))
                else:
        ...
        if target_table_id:
            if not target_dataset_id and not self.default_dataset_id:
                return {"error": "target_dataset_id or default_dataset_id required when target_table_id is specified."}
            dest_dataset_ref = self.client.dataset(target_dataset_id or self.default_dataset_id) 
            job_config.destination = dest_dataset_ref.table(target_table_id)
            job_config.write_disposition = bigquery.WriteDisposition.WRITE_TRUNCATE if write_disposition.upper() == "WRITE_TRUNCATE" else bigquery.WriteDisposition.WRITE_APPEND if write_disposition.upper() == "WRITE_APPEND" else bigquery.WriteDisposition.WRITE_EMPTY
            job_config.create_disposition = bigquery.CreateDisposition.CREATE_NEVER if create_disposition.upper() == "CREATE_NEVER" else bigquery.CreateDisposition.CREATE_IF_NEEDED
            print(f"    Query results will be written to: {job_config.destination}. Write: {job_config.write_disposition}, Create: {job_config.create_disposition}")
        ...
        try:
            ...
            print(f"    Query job {query_job.job_id} finished. Rows affected: {query_job.num_dml_affected_rows}, Rows returned (if not DDL/DML): {results_iterator.total_rows if results_iterator else 'N/A'}")

            if query_job.destination: # If results were written to a table
                dest_table = self.client.get_table(query_job.destination)
                return {"status": "success", "destination_table": str(dest_table.reference), "rows_written": dest_table.num_rows}
            ...
            return {"error": f"BigQuery query execution failed for {self.server_name}", "details": str(e)}

    def _insert_rows(self, table_id: str, rows: list[dict], dataset_id: str = None, skip_invalid_rows: bool = False, ignore_unknown_values: bool = False) -> dict:
        """
        Inserts rows into a BigQuery table.
        """
        if not self.client:
            return {"error": f"BigQuery client not initialized for {self.server_name}. Cannot insert rows."}

        target_dataset_str = dataset_id or self.default_dataset_id
        if not target_dataset_str:
            return {"error": f"Dataset ID must be provided either in server config ('dataset_id') or call for insert_rows in {self.server_name}."}
        
        full_table_id = f"{self.project_id}.{target_dataset_str}.{table_id}"
        print(f"  {self.server_name}._insert_rows called for table '{full_table_id}'. Num rows: {len(rows)}.")
        if not rows:
            return {"status": "success", "message": "No rows provided to insert.", "inserted_count": 0, "errors": []}
        ...
        try:
            ...
            job_config.skip_leading_rows = 0 # Assuming rows is a list of dicts, not CSV data
            ...
            errors = self.client.insert_rows_json(table_ref, rows, job_config=None) # job_config applies to load jobs, not insert_rows_json directly in this way
            ...
            if not errors: # If errors is an empty list
                print(f"    Successfully inserted {len(rows)} rows into {full_table_id}.")
                return {"status": "success", "inserted_count": len(rows), "errors": []}
            else:
                print(f"    Encountered errors while inserting rows to {full_table_id}: {errors}")
            ...
            print(f"BigQuery table '{full_table_id}' not found during insert for {self.server_name}: {nf_error}")
            return {"error": f"BigQuery table '{full_table_id}' not found", "details": str(nf_error)}
        except Exception as e:
            print(f"Error inserting rows with BigQuery for {self.server_name}: {e}")
            return {"error": f"BigQuery insert_rows failed for {self.server_name}", "details": str(e)}
    ...
    def call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        if tool_name == "run_query":
            ...
            if not query:
                raise ValueError(f"Missing 'query' parameter for 'run_query' tool in {self.server_name}.")
            return self._run_query(
                ...
                create_disposition=parameters.get("create_disposition", "CREATE_IF_NEEDED")
            )
            ...
            if not table_id or rows_to_insert is None: # rows can be an empty list
                raise ValueError(f"Missing 'table_id' or 'rows' parameter for 'insert_rows' tool in {self.server_name}.")
...
if __name__ == "__main__":
    print("Testing BigQueryMCPServer with actual client (requires GCP auth & project)...")
    ...
    if not sample_bq_config_real["project_id"]:
        print("SKIPPING real BigQueryMCPServer test: GOOGLE_CLOUD_PROJECT env var not set.")
    else:
        print(f"Using GCP Project: {sample_bq_config_real['project_id']} for test.")
        bq_server_real = BigQueryMCPServer(server_name="bq_real_test", config=sample_bq_config_real)

        if bq_server_real.client: # Only proceed if client initialized
            test_table_id = "mcp_test_table_inserts"
            # Test insert_rows
            rows_data = [
                {"name": "alpha", "value": 10, "timestamp": datetime.datetime.utcnow().isoformat()},
                {"name": "beta", "value": 25, "timestamp": datetime.datetime.utcnow().isoformat()},
                {"name": "gamma", "value": 15, "invalid_extra_field": True} # Test ignore_unknown_values
            ]
            insert_params_real = {
            ...
            insert_status_real = bq_server_real.call_tool("insert_rows", insert_params_real)
            ...
            if insert_status_real.get("status") == "success":
                ...
                query_results_real = bq_server_real.call_tool("run_query", query_params_real)
                ...
                ddl_result = bq_server_real.call_tool("run_query", query_to_table_params)
...
if __name__ == "__main__":
    ...
    try:
        bq_server = BigQueryMCPServer(server_name="bq_main_test", config=sample_bq_config)

        # Test run_query
        query_params = {
            "query": "SELECT name, SUM(number) as total_count FROM `my_dataset.my_table` WHERE score > @min_score GROUP BY name;",
            "query_parameters": [{"name": "min_score", "parameterType": {"type": "FLOAT64"}, "parameterValue": {"value": 0.75}}] # Example BQ param format
        }
