...
from google.cloud.pubsub_v1.publisher.client import Client as PublisherClient # For type hinting
...
class PubSubMCPServer(BaseMCPServer):
    ...
    def _validate_config(self):
        ...
        if "project_id" not in self.config:
            raise ValueError(f"Missing 'project_id' in config for PubSubMCPServer: {self.server_name}")
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            print(f"Pub/Sub Publisher client initialized for project '{self.project_id}' for server '{self.server_name}'.")
            ...
            topics = list(self.publisher_client.list_topics(project=f"projects/{self.project_id}", page_size=1))
            if topics:
                print(f"  Successfully listed at least one Pub/Sub topic: {topics[0].name}")
            else:
                print("  No Pub/Sub topics found in project or not permitted to list, but client initialized.")
        except Exception as e:
            print(f"CRITICAL: Error initializing Pub/Sub Publisher client for {self.server_name}: {e}")
    ...
    def _publish_message(self, topic_id: str, message_data: Dict[str, Any], attributes: Optional[Dict[str, str]] = None) -> Dict[str, Any]:
        """
        ...
        if not self.publisher_client:
            return {"status": "failure", "error": f"Pub/Sub Publisher client not initialized for {self.server_name}."}
        ...
        try:
            ...
            return {"status": "failure", "error": "Message data not JSON serializable", "details": str(e)}
        
        print(f"    Message Data (first 100 bytes as string): {data_bytes.decode('utf-8', errors='ignore')[:100]}...")
        str_attributes = {k: str(v) for k, v in attributes.items()} if attributes else None
        if str_attributes:
            print(f"    Attributes: {str_attributes}")

        try:
            future = self.publisher_client.publish(topic_path, data_bytes, **(str_attributes if str_attributes else {}))
            message_id = future.result() # Wait for publish to complete (blocks)
            print(f"    Message published successfully to {topic_path}. Message ID: {message_id}")
            return {"status": "success", "topic_path": topic_path, "message_id": message_id}
        except NotFound:
            error_msg = f"Pub/Sub topic '{topic_path}' not found."
    ...
    def call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        if tool_name == "publish_message":
            ...
            if not topic_id:
                raise ValueError(f"Missing 'topic_id' for 'publish_message' tool in {self.server_name}.")
            if not isinstance(message_data, dict):
                 raise ValueError(f"'message_data' must be a dictionary for 'publish_message' tool in {self.server_name}.")
...
if __name__ == "__main__":
    ...
    if not gcp_project_id_env:
        print(f"SKIPPING real PubSubMCPServer test: GOOGLE_CLOUD_PROJECT env var not set.")
    else:
        print(f"Using GCP Project: {gcp_project_id_env} for Pub/Sub tests.")
        sample_ps_config_real = {
            "project_id": gcp_project_id_env
        }
        ps_server_real = PubSubMCPServer(server_name="pubsub_sdk_test", config=sample_ps_config_real)

        if ps_server_real.publisher_client:
            print(f"Attempting to publish to topic: {test_topic_id} (ensure it exists or auto-create is enabled if supported by test setup)")
            publish_params = {
                "topic_id": test_topic_id, 
                "message_data": {"experiment_id": "exp_sdk_test_123", "status": "completed", "score": 0.99, "timestamp": datetime.datetime.utcnow().isoformat()},
                "attributes": {"source_system": "mcp_server_test", "priority": "medium", "attempt_num": "1"}
            }
            ...
            print(f"Publish Result (real client):\n{json.dumps(publish_result, indent=2)}")
            assert publish_result.get("status") == "success" or "Topic not found" in publish_result.get("error", "") # Topic might not exist for test
            ...
            bad_data_result = ps_server_real.call_tool("publish_message", bad_data_params)
            print(f"Publish Result (bad data):\n{json.dumps(bad_data_result, indent=2)}")
            assert bad_data_result.get("status") == "failure" and "not JSON serializable" in bad_data_result.get("error", "")
            ...
            nx_topic_result = ps_server_real.call_tool("publish_message", non_existent_topic_params)
            print(f"Publish Result (non-existent topic):\n{json.dumps(nx_topic_result, indent=2)}")
            assert nx_topic_result.get("status") == "failure" and "Topic not found" in nx_topic_result.get("error", "")
