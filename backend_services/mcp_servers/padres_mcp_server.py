...
class PadresMCPServer(BaseMCPServer):
    ...
    def __init__(self, server_name: str, config: dict = None):
        ...
        self.service_base_url = self.config.get("service_base_url") # No default, must be provided unless mock_mode

        super().__init__(server_name, config) # Initialize server_name and basic config handling
        ...
        if self.mock_mode:
            ...
            if not self.service_base_url:
                raise ValueError(f"'service_base_url' must be provided for PadresMCPServer '{self.server_name}' when not in mock_mode.")
            if "://" not in self.service_base_url: # Basic check
                 raise ValueError(f"'service_base_url' ('{self.service_base_url}') appears invalid for PadresMCPServer '{self.server_name}'.")
            ...
            if not self.http_client and not self.mock_mode: # if client not already set by super call
                self._initialize_client() 

            if self.http_client or self.mock_mode: # Successfully initialized or in mock mode
                 print(f"PadresMCPServer '{self.server_name}' initialized. Mode: {'mock' if self.mock_mode else 'live'}. Target: {self.service_base_url if not self.mock_mode else 'N/A'}")
            else: # Failed to initialize client in live mode
                 print(f"PadresMCPServer '{self.server_name}' FAILED to initialize for live mode. Check service_base_url and connectivity.")
    ...
    def _initialize_client(self):
        ...
        if self.mock_mode: # No client needed for mock mode
            ...
            print(f"PadresMCPServer '{self.server_name}': Client initialization skipped (mock mode).")
            return

        if not self.service_base_url:
            print(f"Warning: service_base_url not set for PadresMCPServer '{self.server_name}'. Client not initialized.")
            self.http_client = None
            return
        ...
        try:
            ...
            print(f"HTTP AsyncClient initialized for PadresMCPServer '{self.server_name}' targeting URL '{self.service_base_url}'.")
        except Exception as e:
            print(f"CRITICAL: Error initializing HTTP AsyncClient for PadresMCPServer {self.server_name}: {e}")
            self.http_client = None

    async def _api_call(self, method: str, endpoint: str, json_payload: Optional[Dict] = None, params: Optional[Dict] = None) -> Dict[str, Any]:
        """Helper for making API calls."""
        if not self.http_client:
            return {"error": f"Padres client not initialized or in mock mode for {self.server_name}.", "status": "failure"}
        
        url = f"{self.service_base_url.rstrip('/')}/{endpoint.lstrip('/')}"
        print(f"  {self.server_name}: Calling {method} {url} with payload: {json_payload} and params: {params}")

        try:
            response = await self.http_client.request(method, endpoint, json=json_payload, params=params)
            response.raise_for_status()
            return response.json()
        except httpx.HTTPStatusError as e:
            error_detail = "No response body"
            try: error_detail = e.response.json() if e.response.content else e.response.text
            except: pass # Keep error_detail as text if JSON parsing fails
            print(f"HTTP error for {self.server_name} calling {url}: {e.response.status_code} - {error_detail}")
            return {"error": "Padres service HTTP error", "status_code": e.response.status_code, "details": error_detail, "status": "failure"}
        except httpx.RequestError as e:
            print(f"Request error for {self.server_name} calling {url}: {e}")
            return {"error": "Padres service request error", "details": str(e), "status": "failure"}
        except Exception as e:
            print(f"Generic error for {self.server_name} calling {url}: {type(e).__name__} - {e}")
            return {"error": "Generic error during Padres API call", "details": str(e), "status": "failure"}

    # --- Methods mapped to new Padres API endpoints ---

    async def _padres_setup_environment(self, config: Dict[str, Any], environment_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    async def _padres_run_trial(self, environment_id: str, parameters: Dict[str, Any], trial_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _mock_setup_environment(self, config_payload: Dict[str, Any], environment_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        print(f"MockPadresServer: Environment '{env_id}' set up with config: {config_payload}")
        return {
            "environment_id": env_id,
            "status": "SUCCESS",
            "details": {"message": f"Mock environment {env_id} configured.", "current_config": config_payload}
        }

    def _mock_run_trial(self, environment_id: str, parameters: Dict[str, Any], trial_id: Optional[str] = None) -> Dict[str, Any]:
        if environment_id not in self.mock_environments:
            return {"error": f"Mock environment {environment_id} not found.", "status": "failure", "trial_id": trial_id}
        ...
        self.mock_trials[tr_id] = {
            ...
            "status": "COMPLETED",
            "results": {"mock_metric": 0.88, "steps": 50, "info": f"Mock trial {tr_id} completed."}
        }
        print(f"MockPadresServer: Trial '{tr_id}' run in env '{environment_id}' with params: {parameters}")
        return {
            "trial_id": tr_id,
            "status": "COMPLETED",
            "environment_id": environment_id,
    ...
    def _mock_get_results(self, trial_id: str) -> Dict[str, Any]:
        if trial_id not in self.mock_trials:
            return {"error": f"Mock trial {trial_id} not found.", "status": "failure", "trial_id": trial_id}
    ...
    def get_status(self) -> dict:
        ...
        if self.mock_mode:
            ...
            client_status = "initialized" if self.http_client else "not_initialized_or_error"
            return {
    ...
    async def async_call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        """
        print(f"PadresMCPServer ('{self.server_name}', mock_mode={self.mock_mode}) async_call_tool: '{tool_name}' with params: {parameters}")
        ...
        if self.mock_mode:
            if tool_name == "setup_environment":
                ...
                return {"error": f"Tool '{tool_name}' not supported in mock mode for PadresMCPServer.", "status": "failure"}
        else: # Live mode, use HTTP client
            if not self.http_client and tool_name != "get_padres_api_status": # Allow status check even if client failed init, API might be down
                 # Re-initialize client if it's None and we are not in mock_mode
                print(f"PadresMCPServer '{self.server_name}': http_client is None. Attempting to re-initialize.")
                self._initialize_client()
                if not self.http_client:
                    return {"error": f"HTTP client for PadresMCPServer '{self.server_name}' is not initialized. Cannot call tool '{tool_name}'.", "status": "failure"}
            ...
            if tool_name == "setup_environment":
                ...
                # Parameters: {"trial_id": str}
                return await self._padres_get_results(trial_id=parameters.get("trial_id"))
                 ...
                 if not self.http_client : # If client never initialized or failed
                    ...
                    print(f"PadresMCPServer '{self.server_name}': http_client is None for status check. Attempting to re-initialize.")
                    self._initialize_client()
                    if not self.http_client:
                         return {"error": f"Cannot get live API status for '{self.server_name}'. HTTP client failed to initialize.", 
                                 "status": "failure", "service_base_url": self.service_base_url}
                 return await self._padres_get_status()
            else:
                return {"error": f"Tool '{tool_name}' not supported in live mode for PadresMCPServer.", "status": "failure"}
    ...
    def call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        if self.mock_mode:
            ...
            if tool_name in ["setup_environment", "run_trial", "get_results", "get_padres_api_status"]:
                 return {"error": f"Tool '{tool_name}' on PadresMCPServer '{self.server_name}' (mock mode) should be called via async_call_tool for consistency.", 
                        "status":"hint", "suggestion": "Use async_call_tool."}
            else:
                 return {"error": f"Tool '{tool_name}' not recognized or not synchronously callable on PadresMCPServer '{self.server_name}'.", "status": "failure"}
        else: # Live mode
            return {"error": f"Tool '{tool_name}' on PadresMCPServer '{self.server_name}' (live mode) must be called via async_call_tool.",
                    "status": "failure", "suggestion": "Use async_call_tool."}
    ...
    async def close_client(self):
        ...
        if self.http_client:
            try:
                ...
                print(f"HTTP AsyncClient closed for PadresMCPServer '{self.server_name}'.")
            except Exception as e:
                print(f"Error closing HTTP client for PadresMCPServer '{self.server_name}': {e}")
            finally:
                self.http_client = None
        else:
            print(f"No active HTTP client to close for PadresMCPServer '{self.server_name}' (mock_mode: {self.mock_mode}).")
...
async def main_test():
    ...
    mock_padres_server = PadresMCPServer(server_name="padres_mock_test", config=mock_config)
    
    print(f"Mock Server Status: {mock_padres_server.get_status()}")
    print(f"Mock Server Status (async): {await mock_padres_server.async_call_tool('get_padres_api_status', {})}")

    env_setup_params_mock = {"config": {"scene": "kitchen_v1", "max_steps": 100}}
    setup_result_mock = await mock_padres_server.async_call_tool("setup_environment", env_setup_params_mock)
    ...
    if mock_env_id:
        trial_params_mock = {"environment_id": mock_env_id, "parameters": {"agent_type": "random", "num_episodes": 1}}
        trial_result_mock = await mock_padres_server.async_call_tool("run_trial", trial_params_mock)
        print(f"Mock Run Trial Result: {json.dumps(trial_result_mock, indent=2)}")
        
        mock_trial_id = trial_result_mock.get("trial_id")
        if mock_trial_id:
            results_mock = await mock_padres_server.async_call_tool("get_results", {"trial_id": mock_trial_id})
    ...
    try:
        live_padres_server = PadresMCPServer(server_name="padres_live_test", config=live_config)
        print(f"Live Server Status (sync): {live_padres_server.get_status()}")

        # Test get_padres_api_status
        api_status = await live_padres_server.async_call_tool("get_padres_api_status", {})
        print(f"Live API Status from service: {json.dumps(api_status, indent=2)}")
        
        if api_status.get("api_status") == "OPERATIONAL": # Check if the mock API is running
            env_setup_params_live = {"config": {"scene": "office_v2", "complexity": "high"}}
            setup_result_live = await live_padres_server.async_call_tool("setup_environment", env_setup_params_live)
            print(f"Live Setup Environment Result: {json.dumps(setup_result_live, indent=2)}")
            live_env_id = setup_result_live.get("environment_id")

            if live_env_id and not setup_result_live.get("error"):
                trial_params_live = {"environment_id": live_env_id, "parameters": {"algorithm": "PPO", "seed": 123}}
                trial_result_live = await live_padres_server.async_call_tool("run_trial", trial_params_live)
                print(f"Live Run Trial Result: {json.dumps(trial_result_live, indent=2)}")
                live_trial_id = trial_result_live.get("trial_id")

                if live_trial_id and not trial_result_live.get("error"):
                    results_live = await live_padres_server.async_call_tool("get_results", {"trial_id": live_trial_id})
                    print(f"Live Get Results: {json.dumps(results_live, indent=2)}")
            else:
                print("Skipping further live tests as environment setup failed or returned an error.")
        else:
            print("Skipping live API interaction tests as mock API status is not OPERATIONAL or fetch failed.")
            print("Ensure padres_container/app/main.py is running on http://localhost:8088.")
