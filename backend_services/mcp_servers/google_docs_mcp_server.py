...
class GoogleDocsMCPServer(BaseMCPServer):
    ...
    def _validate_config(self):
        ...
        if creds_path and not os.path.exists(creds_path):
            raise ValueError(f"Invalid 'credentials_path' for GoogleDocsMCPServer '{self.server_name}': File not found at {creds_path}")
        print(f"  GoogleDocsMCPServer '{self.server_name}' config validation passed. Creds path: {creds_path or 'Not specified (will try ADC)'}")
    ...
    def _initialize_client(self):
        ...
        try:
            if creds_path:
                ...
                print(f"  GoogleDocsMCPServer '{self.server_name}': No credentials_path provided. Attempting Application Default Credentials (ADC).")
                self.credentials, project_id = get_application_default_credentials(scopes=self.REQUIRED_SCOPES)
            ...
            self.docs_service = build('docs', self.DOCS_API_VERSION, credentials=self.credentials, static_discovery=False)
            self.drive_service = build('drive', self.DRIVE_API_VERSION, credentials=self.credentials, static_discovery=False)
            print(f"Google Docs & Drive API clients initialized for '{self.server_name}' using {self.auth_method_used}.")
            ...
            print(f"CRITICAL: DefaultCredentialsError for GoogleDocsMCPServer '{self.server_name}': {dce}. Ensure ADC are configured or provide a service account key.")
            self.docs_service = None
            self.drive_service = None
        except Exception as e:
            print(f"CRITICAL: Error initializing Google API clients for {self.server_name}: {type(e).__name__} - {e}")
            self.docs_service = None
            self.drive_service = None

    def _create_document_from_template(self, template_id: str, title: str, folder_id: str = None, replacements: dict = None) -> dict:
        """
        ...
        print(f"    Template ID: {template_id}, Title: '{title}', Folder ID: {target_folder_id}")
        ...
        return {"status": "success_mock", "document_id": mock_doc_id, "url": f"https://docs.google.com/document/d/{mock_doc_id}/edit"}
    ...
    def _update_document_content(self, document_id: str, requests: list) -> dict:
        ...
        """
        print(f"  {self.server_name}._update_document_content (mock) called for doc '{document_id}'. Num requests: {len(requests)}.")
        # try:
        ...
        #     raise
        return {"status": "success_mock", "document_id": document_id, "updated_elements": len(requests)}
    ...
    def call_tool(self, tool_name: str, parameters: dict) -> any:
        ...
        if tool_name == "create_doc_from_template":
            ...
            if not template_id or not title:
                raise ValueError(f"Missing 'template_id' or 'title' for 'create_doc_from_template' tool in {self.server_name}.")
            ...
            if not document_id or not requests:
                raise ValueError(f"Missing 'document_id' or 'requests' for 'update_document_content' tool in {self.server_name}.")
...
if __name__ == "__main__":
    print("Testing GoogleDocsMCPServer... (Requires ADC or service_account.json and relevant Drive/Docs API enabled)")
    ...
    gcp_project_for_adc = os.environ.get("GOOGLE_CLOUD_PROJECT") # ADC often associated with a project
    ...
    if os.path.exists(creds_file_path):
        ...
        print(f"  Attempting ADC (associated project if shown by SDK: {gcp_project_for_adc}). Ensure you are authenticated.")
    else:
        print("  WARNING: No explicit credentials_path and GOOGLE_CLOUD_PROJECT env var not set. ADC might fail or use unexpected project.")
    ...
    if test_template_id == "YOUR_GOOGLE_DOCS_TEMPLATE_ID_HERE":
        ...
        gdocs_server = GoogleDocsMCPServer(server_name="gdocs_sdk_test", config=test_config)
        if gdocs_server.docs_service and gdocs_server.drive_service:
            ...
            create_params = {
                ...
                "replacements": {"REPORT_TITLE": "Automated Test Report", "MAIN_CONTENT_AREA": "This is the main content.\n- Point 1\n- Point 2"}
            }
            create_result = gdocs_server.call_tool("create_doc_from_template", create_params)
            print(f"Create Doc Result:\n{json.dumps(create_result, indent=2)}")

            if create_result.get("status") == "success" and create_result.get("document_id"):
                new_doc_id = create_result["document_id"]                
                print(f"\n--- Test 2: Update Document Content (Append text) ---")
                update_requests = [
                    {
                        'insertText': {
                            'endOfSegmentLocation': {'segmentId': ''}, # Appends to the end of the body
                            'text': "\n\nAdditional appended section via MCP."
                        }
                    }
                ]
                update_result = gdocs_server.call_tool("update_document_content", {"document_id": new_doc_id, "requests": update_requests})
                print(f"Update Doc Result:\n{json.dumps(update_result, indent=2)}")
        else:
            print("Google Docs/Drive services not initialized. Skipping API call tests.")
