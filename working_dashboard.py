...
class WorkingSocietyDashboard:
    ...
    def create_performance_chart(self, df):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Performance', 'Agent Metrics', 'LLM Usage', 'System Health'),
        ...
        if 'steps_per_second' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['steps_per_second'], 
                                   name='SPS', line=dict(color='green')), row=1, col=1)
        if 'runtime' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['runtime'], 
                                   name='Runtime', line=dict(color='blue')), row=1, col=1)
        
        # Agent metrics
        if 'agent_count' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['agent_count'], 
                                   name='Agents', line=dict(color='red')), row=1, col=2)
        if 'avg_energy' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['avg_energy'], 
                                   name='Avg Energy', line=dict(color='orange')), row=1, col=2)
        if 'avg_happiness' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['avg_happiness'], 
                                   name='Avg Happiness', line=dict(color='purple')), row=1, col=2)
        
        # LLM metrics
        if 'llm_total_requests' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['llm_total_requests'], 
                                   name='LLM Requests', line=dict(color='cyan')), row=2, col=1)
        if 'llm_cache_hit_rate' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['llm_cache_hit_rate'], 
                                   name='Cache Hit Rate', line=dict(color='magenta')), 
                         row=2, col=1, secondary_y=True)
        
        # System health
        if 'total_social_interactions' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['total_social_interactions'], 
                                   name='Interactions', line=dict(color='brown')), row=2, col=2)
        if 'total_objects_created' in df.columns:
            fig.add_trace(go.Scatter(x=steps, y=df['total_objects_created'], 
                                   name='Objects Created', line=dict(color='pink')), row=2, col=2)
        
        fig.update_layout(height=700, showlegend=True, title_text="Society Simulation Metrics")
        return fig
        ...
        if 'steps_per_second' in latest:
            cards.append(
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{latest['steps_per_second']:.1f}", className="card-title"),
        ...
        if 'agent_count' in latest:
            cards.append(
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{int(latest['agent_count'])}", className="card-title"),
        ...
        if 'avg_happiness' in latest:
            cards.append(
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{latest['avg_happiness']:.3f}", className="card-title"),
        ...
        if 'llm_cache_hit_rate' in latest:
            cards.append(
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            html.H4(f"{latest['llm_cache_hit_rate']:.1%}", className="card-title"),
                            html.P("LLM Cache Hit Rate", className="card-text")
                        ])
    ...
    def create_3d_visualization(self, df):
        ...
        fig = go.Figure(data=[go.Scatter3d(
            ...
            text=[f"Society Node {i}<br>{color_title}: {c:.3f}" for i, c in enumerate(colors)],
    ...
    def setup_layout(self):
        ...
        self.app.layout = dbc.Container([
            dbc.Row([
                dbc.Col([
                    html.H1("🌐 Society Simulation Dashboard", className="text-center mb-4"),
                    html.Hr()
                ])
            ]),
                ...
                dbc.Col([
                    dbc.Card([
                        ...
                        dbc.CardBody([
                            ...
                            dbc.Button("🚀 Run Simulation", id="run-sim-btn", color="success", className="me-2"),
    ...
    def setup_callbacks(self):
        ...
        def update_data(refresh_clicks, interval_clicks, sim_clicks):
            ...
            if ctx.triggered and 'run-sim-btn' in ctx.triggered[0]['prop_id']:
                status = dbc.Alert("🚀 Running simulation...", color="info", is_open=True)
    ...
    def run(self, debug=False, port=8051):
        """Run the dashboard"""
        print(f"🌐 Starting Working Society Dashboard...")
        ...
        try:
            ...
            df, categories = self.load_metrics_data()
            print(f"✅ Loaded {len(df)} data rows with categories: {list(categories.keys())}")
