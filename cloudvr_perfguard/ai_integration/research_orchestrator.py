...
class ResearchOrchestrator:
    ...
    def __init__(self, db_manager: DatabaseManager):
        ...
        self.pipeline_configs = {
            ...
            "comprehensive_analysis": {
                "evolution_types": ["visual_cue_discovery", "affordance_scoring", "interaction_predictor"],
            ...
            "full_research_suite": {
                "evolution_types": ["visual_cue_discovery", "affordance_scoring", "interaction_predictor"],
                "paper_types": ["vr_affordance_discovery", "performance_optimization", "user_experience_analysis"],
    ...
    async def run_complete_research_pipeline(
        ...
        """
        try:
            print(f"INFO: Starting complete research pipeline for job {job_id}, type: {pipeline_type}")
            
            # Get pipeline configuration
            config = self.pipeline_configs.get(pipeline_type, self.pipeline_configs["comprehensive_analysis"])
            
            # Update job status
            await self.db_manager.update_job_status(
                job_id=job_id,
                status="ai_research_running",
                progress=10
            )
            ...
            await self.db_manager.update_job_status(job_id=job_id, status="ai_research_running", progress=60)
            ...
            await self.db_manager.update_job_status(job_id=job_id, status="ai_research_running", progress=90)
    ...
    async def _run_evolution_phase(
        ...
        for i, evolution_type in enumerate(evolution_types):
            print(f"INFO: Running evolution type {evolution_type} ({i+1}/{len(evolution_types)})")
            
            # Check time limit
            elapsed = (datetime.utcnow() - start_time).total_seconds()
            if elapsed > max_time:
                print(f"WARNING: Evolution phase time limit reached, skipping remaining types")
                break
            ...
            try:
                ...
                if result["status"] == "success":
                    ...
                    type_functions = await self.funsearch.get_evolved_functions_for_job(job_id)
                    type_functions = [
                        f for f in type_functions 
    ...
    async def _run_paper_generation_phase(
        ...
        for i, paper_type in enumerate(paper_types):
            print(f"INFO: Generating paper type {paper_type} ({i+1}/{len(paper_types)})")
            ...
            try:
                ...
                if result["status"] == "success":
                    ...
                    paper_summaries[paper_type] = {
                        ...
                        "abstract_preview": result.get("abstract", "")[:200] + "..." if result.get("abstract") else ""
                    }
                else:
                    paper_summaries[paper_type] = {
                        "status": "error",
    ...
    def _generate_research_summary(
        ...
        for evo_type, summary in evolution_results["evolution_summaries"].items():
            ...
            summary_parts.append(f"- **{evo_type.replace('_', ' ').title()}**: {status_icon} {summary['status']}")
            if summary["status"] == "success":
                summary_parts.append(f"  - Functions: {summary['functions_evolved']}, Best Score: {summary['best_score']:.3f}")
        ...
        for paper_type, summary in paper_results["paper_summaries"].items():
            status_icon = "✅" if summary["status"] == "success" else "❌"
            summary_parts.append(f"- **{paper_type.replace('_', ' ').title()}**: {status_icon} {summary['status']}")
            if summary["status"] == "success":
                summary_parts.extend([
    ...
    async def run_quick_evolution_only(
        ...
        try:
            print(f"INFO: Running quick evolution for job {job_id}, type: {evolution_type}")
    ...
    async def run_paper_generation_only(
        ...
        try:
            print(f"INFO: Running paper generation for job {job_id}, type: {paper_type}")
            
            # Get existing evolved functions
            evolved_functions = await self.funsearch.get_evolved_functions_for_job(job_id)
            
            if not evolved_functions:
                raise ValueError(f"No evolved functions found for job {job_id}. Run evolution first.")
            
            await self.db_manager.update_job_status(
                job_id=job_id,
                status="paper_generation_running",
                progress=20
            )
            ...
            await self.db_manager.update_job_status(
                job_id=job_id,
                status="paper_generation_completed",
                progress=100
            )
            ...
            await self.db_manager.update_job_status(
                job_id=job_id,
                status="paper_generation_failed",
                error=str(e)
            )
            raise 