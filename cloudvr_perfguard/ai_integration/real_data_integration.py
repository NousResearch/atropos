...
class RealDataResearchPipeline:
    ...
    async def generate_research_from_real_data(
        ...
        print(f"🔬 Starting real data research generation...")
        ...
        results = {
            ...
            "data_count": len(real_data),
            "apps_analyzed": list(set(d["job_metadata"]["app_name"] for d in real_data)),
        ...
        try:
            if research_type in ["comprehensive", "performance_analysis"]:
                ...
                paper_result = await self._generate_performance_paper(research_data, real_data)
                if paper_result:
            ...
            if research_type in ["comprehensive", "optimization_study"]:
                ...
                function_results = await self._generate_optimization_functions(research_data)
            ...
            if research_type == "comprehensive":
                ...
                if len(results["apps_analyzed"]) > 1:
                    comparison_paper = await self._generate_comparison_paper(research_data, real_data)
                    if comparison_paper:
                        results["papers"].append(comparison_paper)
                        results["total_cost"] += comparison_paper.get("generation_cost", 0)
            
            # Calculate research quality
            results["research_quality"] = self._calculate_research_quality(results)
            
            print(f"✅ Real data research completed!")
    ...
    def _convert_real_data_to_research_format(self, real_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _generate_performance_paper(
        ...
        try:
            ...
            if len(research_data["metadata"]["apps_tested"]) > 3:
                apps_list += f" and {len(research_data['metadata']['apps_tested']) - 3} others"
    ...
    async def _generate_optimization_functions(
        ...
        try:
            ...
            for domain in domains:
                try:
                    function_result = self.function_discovery.discover_optimization_function(
                        funsearch_data,
                        domain=domain
                    )
    ...
    async def _generate_comparison_paper(
        ...
        try:
            ...
            paper_result["comparative_context"] = {
                "apps_compared": apps,
                "comparison_metrics": ["fps", "frame_time", "gpu_utilization", "comfort_score"],
                "data_source": "CloudVR-PerfGuard Production Database"
            }
    ...
    def _calculate_research_quality(self, results: Dict[str, Any]) -> float:
        ...
        # Paper quality (40%)
        if results["papers"]:
            avg_paper_quality = sum(p.get("quality_score", 0) for p in results["papers"]) / len(results["papers"])
        ...
        if results["functions"]:
            avg_function_fitness = sum(f.get("fitness_score", 0) for f in results["functions"]) / len(results["functions"])
    ...
    async def run_daily_research(self) -> Dict[str, Any]:
        ...
        apps_with_data = set(job["app_name"] for job in recent_jobs if job["status"] == "completed")
        ...
        try:
            ...
            if len(apps_with_data) > 1:
                ...
                if comparison_research["success"]:
                    ...
                    daily_results["total_cost"] += comparison_research.get("total_cost", 0)
...
async def generate_research_from_real_data(
    ...
    try:
        results = await pipeline.generate_research_from_real_data(app_name, research_type)
        return results
    finally:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        results = await generate_research_from_real_data(
            research_type="comprehensive"
        )
