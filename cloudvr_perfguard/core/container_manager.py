...
class ContainerManager:
    ...
    async def create_container(self, config: Dict[str, Any]) -> str:
        ...
        try:
            print(f"🐳 Creating container {container_id} for {config['platform']} + {config['vr_runtime']}")
    ...
    async def run_vr_app(self, container_id: str, scene_name: str, duration: int) -> Dict[str, Any]:
        """
    ...
    async def _execute_vr_app(self, container, scene_name: str, duration: int) -> Dict[str, Any]:
        ...
        print(f"    🎯 Executing scene '{scene_name}' for {duration}s")
        ...
        reprojected_frames = sum(1 for ft in frame_times if ft > 13.33)  # Frames > 75 FPS
        ...
        print(f"    ✅ Scene execution complete: {len(frame_times)} frames, {dropped_frames} dropped")
