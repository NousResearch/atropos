...
class VRPerformanceTester:
    ...
    def _generate_test_configurations(self, config: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        for gpu_type in config.get("gpu_types", ["T4"]):
            for scene_name in config.get("test_scenes", ["main_menu"]):
                if scene_name not in self.test_scenes:
                    print(f"WARNING: Unknown test scene '{scene_name}', skipping")
                    continue
                ...
                test_config = {
                    ...
                    "test_duration": min(config.get("test_duration_seconds", 60), scene_config["duration"]),
    ...
    async def _run_parallel_tests(self, build_path: str, test_configs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
    ...
    async def _run_single_performance_test(
        ...
        async with semaphore:
            print(f"  🧪 Test {test_num}: {test_config['gpu_type']} + {test_config['scene_name']}")
            
            try:
                # Setup test environment (like AMIEN's setup_environment)
                container_id = await self._setup_test_environment(build_path, test_config)
                
                # Run performance measurement (like AMIEN's execute_action)
                performance_data = await self._measure_performance(container_id, test_config)
                
                # Cleanup
                await self._cleanup_test_environment(container_id)
                
                # Calculate metrics (like AMIEN's calculate_performance_metrics)
                metrics = self._calculate_performance_metrics(performance_data, test_config)
    ...
    async def _setup_test_environment(self, build_path: str, test_config: Dict[str, Any]) -> str:
    ...
    async def _measure_performance(self, container_id: str, test_config: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _aggregate_test_results(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        aggregated = {
            ...
            "overall_avg_frame_time": np.mean([m["avg_frame_time"] for m in all_metrics]),
            "overall_p99_frame_time": np.max([m["p99_frame_time"] for m in all_metrics]),
            ...
            # VR quality
            "overall_comfort_score": np.mean([m["vr_comfort_score"] for m in all_metrics]),
