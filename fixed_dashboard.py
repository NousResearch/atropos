...
data_cache = {"df": pd.DataFrame(), "categories": {}, "last_update": 0}
...
def load_metrics_data(db_path="results/metrics.db"):
    ...
    if time.time() - data_cache["last_update"] < 5:
        return data_cache["df"], data_cache["categories"]
...
def create_simple_charts(df):
    ...
    if 'steps_per_second' in df.columns:
        fig1.add_trace(go.Scatter(x=df['step'], y=df['steps_per_second'], 
                                 name='SPS', line=dict(color='green')))
    if 'agent_count' in df.columns:
        fig1.add_trace(go.Scatter(x=df['step'], y=df['agent_count'], 
                                 name='Agents', line=dict(color='blue')))
    ...
    cards = dbc.Row([
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    html.H4(f"{latest.get('llm_cache_hit_rate', 0):.1%}"),
