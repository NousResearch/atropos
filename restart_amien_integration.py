...
class AMIENIntegrationManager:
    ...
    def __init__(self):
        ...
        print(f"🚀 AMIEN Integration Manager Initialized")
    ...
    def check_ai_scientist(self):
        ...
        return (self.ai_scientist_dir.exists() and 
                (self.ai_scientist_dir / "launch_scientist.py").exists())
    
    def check_funsearch(self):
        """Check if FunSearch is properly set up"""
        return (self.funsearch_dir.exists() and 
                (self.funsearch_dir / "implementation" / "funsearch.py").exists())
    ...
    async def generate_gemini_paper(self, experiment_data, paper_spec):
        ...
        try:
            ...
            successful_experiments = sum(1 for exp in experiment_data if exp['test_results']['success'])
            success_rate = successful_experiments / total_experiments * 100
            
            # Calculate performance statistics
            fps_values = [exp['performance_metrics']['fps_avg'] for exp in experiment_data]
            comfort_scores = [exp['performance_metrics']['comfort_score'] for exp in experiment_data]
...
"""
            ...
            if response and response.text:
                ...
                # Save paper
                paper_file = self.output_dir / f"ai_scientist_paper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
    ...
    async def fallback_paper_generation(self, experiment_data, paper_spec=None):
        ...
        paper_content = f"""# {paper_spec['title']}
...
"""
        ...
        # Save paper
        paper_file = self.output_dir / f"fallback_paper_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        with open(paper_file, 'w') as f:
            f.write(paper_content)
        ...
        try:
            ...
            for exp in experiment_data:
                ...
                features = [
                    ...
                    1.0 if exp['app_name'] == 'VRExplorer' else 0.0  # App type encoding
                ]
...
"""
            ...
            if response and response.text:
                ...
                # Save function
                function_file = self.output_dir / f"vr_optimizer_function_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
...
'''
        ...
        # Save function
        function_file = self.output_dir / f"fallback_optimizer_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
        with open(function_file, 'w') as f:
    ...
    async def run_comprehensive_integration(self):
        ...
        # Save results
        results_file = self.output_dir / f"integration_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    async def generate_integration_report(self, results, experiment_data):
        ...
        report_content = f"""# AMIEN AI Research Integration Report
...
"""
        ...
        # Save report
        report_file = self.output_dir / f"integration_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
