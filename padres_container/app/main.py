...
from spatial_rl_mvp.spatial_env import SpatialEnvironmentMVP, SpatialTask, ObjectState
...
async def startup_event():
    ...
    if 'spatial_rl_mvp.spatial_env' in sys.modules:
        if hasattr(sys.modules['spatial_rl_mvp.spatial_env'], 'notify_visualization_clients'):
            original_notify_visualization_clients = sys.modules['spatial_rl_mvp.spatial_env'].notify_visualization_clients
            sys.modules['spatial_rl_mvp.spatial_env'].notify_visualization_clients = dummy_notify_visualization_clients_func
            print("INFO: spatial_rl_mvp.spatial_env.notify_visualization_clients has been temporarily neutralized during startup.")
        else:
            print("WARN (startup): spatial_rl_mvp.spatial_env.notify_visualization_clients not found for neutralization.")
    else:
        print("WARN (startup): spatial_rl_mvp.spatial_env module not found in sys.modules for websocket neutralization.")
...
async def shutdown_event():
    if spatial_env_instance and hasattr(spatial_env_instance, 'simulator') and spatial_env_instance.simulator:
    ...
    if original_notify_visualization_clients and 'spatial_rl_mvp.spatial_env' in sys.modules:
        if hasattr(sys.modules['spatial_rl_mvp.spatial_env'], 'notify_visualization_clients'):
            sys.modules['spatial_rl_mvp.spatial_env'].notify_visualization_clients = original_notify_visualization_clients
...
async def setup_environment_endpoint():
    ...
    if not spatial_env_instance:
        print("ERROR: /setup_environment called but spatial_env_instance is None. Startup likely failed.")
        raise HTTPException(status_code=500, detail="Spatial environment instance not available. Check server logs for startup errors.")

    # Define a hardcoded task for Day 1
    objects = [
        ObjectState(id="red_cube", type="cube", position=[0.5, 0.0, 0.2], scale=[0.2,0.2,0.2], color_rgba=[1,0,0,1]),
        ObjectState(id="blue_sphere", type="sphere", position=[-0.5, 0.0, 0.2], scale=[0.2,0.2,0.2], color_rgba=[0,0,1,1])
    ]
    task_id = f"hardcoded_task_day1_{uuid.uuid4().hex[:4]}"
    current_default_task = SpatialTask(
        task_id=task_id, description="Move red cube near blue sphere.",
        initial_objects=objects, goal_description="Red cube within 0.3 units of blue sphere.",
        target_object_id="red_cube", reference_object_id="blue_sphere", target_distance=0.3 
    )
    ...
    try:
        ...
        print(f"API CALL: /setup_environment - Task '{task_id}' initialized.")
        return {"message": "Environment initialized with hardcoded task.", "task_id": task_id, "status": "SUCCESS"}
    except Exception as e:
...
async def execute_action_endpoint():
    ...
    if not spatial_env_instance:
        ...
        raise HTTPException(status_code=500, detail="Spatial environment instance not available.")
    if not hasattr(spatial_env_instance, 'current_task') or not spatial_env_instance.current_task:
        raise HTTPException(status_code=400, detail="Environment not initialized with a task. Call /setup_environment first.")
    ...
    action_to_apply = {
        ...
        "target_position": [
            ref_obj_initial_pos[0] + 0.1 if target_obj_id == "red_cube" else ref_obj_initial_pos[0] - 0.1, # Simplistic move towards center
    ...
    try:
        task_id = spatial_env_instance.current_task.task_id
        print(f"API CALL: /execute_action for task '{task_id}' - Applying action: {action_to_apply}")
        ...
        print(f"API CALL: /execute_action for task '{task_id}' - Observation: {obs_msg}")
        
        return {
            "message": "Action executed.", "task_id": task_id,
            "action_applied": action_to_apply, "observation": obs_msg,
            "reward": outcome.get("reward"), "done": outcome.get("done"),
