...
async def run_continuous_discovery(orchestrator: EnhancedResearchOrchestrator, config: dict):
    ...
    while True:
        try:
            ...
            if sleep_time > 0:
                logger.info(f"😴 Sleeping for {sleep_time/3600:.1f} hours until next cycle...")
                await asyncio.sleep(sleep_time)
            else:
                logger.warning(f"⚠️  Cycle took longer than interval ({cycle_duration/3600:.1f}h)")
