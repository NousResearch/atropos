...
class SocietyVisualizer:
    ...
    def _create_legends(self):
        ...
        # Agent type legend
        type_legend_elements = [plt.Line2D([0], [0], marker='o', color='w', 
                                         markerfacecolor=color, markersize=8, label=agent_type)
                              for agent_type, color in self.agent_type_colors.items()]
        
        self.ax_main.legend(handles=type_legend_elements, loc='upper right', 
                           bbox_to_anchor=(1.0, 1.0), title="Agent Types")
    ...
    def _plot_agents(self, world):
        ...
        for agent_type, data in agent_groups.items():
            ...
            alphas = [max(0.3, h) for h in data['happiness']]   # Transparency based on happiness
            
            scatter = self.ax_main.scatter(data['x'], data['y'], 
        ...
        for agent in world.agents:
            ...
            for connected_id, strength in agent.social_connections.items():
                if strength > 0.5:  # Only show strong connections
                    ...
                    connected_agent = next((a for a in world.agents if a.agent_id == connected_id), None)
                    if connected_agent:
                        # Draw line with alpha based on connection strength
                        self.ax_main.plot([agent.position.x, connected_agent.position.x],
                                        [agent.position.y, connected_agent.position.y],
                                        'gray', alpha=strength * 0.5, linewidth=0.5)
                        connection_count += 1
    ...
    def _plot_statistics(self, world, step_count: int):
        ...
        self.stats_history['total_currency'].append(stats['total_resources']['currency'])
        ...
        if len(steps) > 1:
            ...
            self.ax_stats.plot(steps, list(self.stats_history['avg_energy']), 
                             'b-', label='Avg Energy', alpha=0.8)
            self.ax_stats.plot(steps, list(self.stats_history['avg_happiness']), 
                             'g-', label='Avg Happiness', alpha=0.8)
            
            # Social connections (different scale)
            ax2.plot(steps, list(self.stats_history['total_connections']), 
                    'r-', label='Social Connections', alpha=0.8)
...
"""
        ...
        self.ax_stats.text(0.02, 0.02, stats_text, transform=self.ax_stats.transAxes,
...
def run_simulation_with_visualization(num_agents: int = 50, steps: int = 500):
    ...
    print(f"🎬 Starting Visualized Simulation")
