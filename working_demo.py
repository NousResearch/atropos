...
class WorkingAgent(Agent):
    ...
    def __init__(self, unique_id: str, model, position: Position = None):
        ...
        self.personality = random.choice(["extrovert", "introvert", "analytical", "creative"])
    ...
    def _make_decision(self) -> str:
        ...
        if self.energy < 0.3:
            return "rest"
        elif len(nearby_agents) > 0 and self.personality == "extrovert" and random.random() < 0.4:
            return "socialize"
    ...
    def _move_randomly(self):
        ...
        new_x = max(0, min(self.model.space.width, self.position.x + random.uniform(-5, 5)))
        new_y = max(0, min(self.model.space.height, self.position.y + random.uniform(-5, 5)))
        ...
        for other in nearby_agents:
            if hasattr(other, 'social_connections'):
                ...
                if other.unique_id not in self.social_connections:
                    ...
                    self.social_connections[other.unique_id] = min(1.0,
                        self.social_connections[other.unique_id] + 0.05)
    ...
    def __init__(self, num_agents: int = 50, width: int = 100, height: int = 100):
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                "Average_Happiness": lambda m: sum(agent.happiness for agent in m.agent_list) / len(m.agent_list),
                "Total_Connections": lambda m: sum(len(agent.social_connections) for agent in m.agent_list),
                "Average_Age": lambda m: sum(agent.age for agent in m.agent_list) / len(m.agent_list),
                "Total_Currency": lambda m: sum(agent.resources["currency"] for agent in m.agent_list)
            }
        )
...
def run_simulation(num_agents: int = 50, steps: int = 200):
    ...
    print(f"🏘️  Running Society Simulation")
...
def main():
    ...
    for num_agents in [25, 50, 100]:
        ...
        total_connections = sum(len(agent.social_connections) for agent in model.agent_list)
