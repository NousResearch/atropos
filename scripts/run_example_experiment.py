...
async def run_full_experiment(experiment_id="exp_spatial_reasoning_001", config_path=None):
    """
    ...
    research_assistant = ResearchAssistant(
        mcp_manager=mcp_manager,
        config=pipeline_config["research_assistant_config"]
    )
    
    # Initialize Analysis Engine
    analysis_engine = AnalysisEngine(
        mcp_manager=mcp_manager,
        config=pipeline_config["analysis_engine_config"]
    )
    ...
    try:
        ...
        experiment_config = await experiment_orchestrator.fetch_experiment_config(experiment_id)
        experiment_result = await experiment_orchestrator.run_single_experiment(experiment_config)
        print(f"Experiment result: {json.dumps(experiment_result, indent=2)}")
        
        # Step 2: Analyze experimental results
        print("\n2. ANALYZING RESULTS")
        print("===================")
        analysis_result = await research_assistant.analyze_experimental_results([experiment_id])
        ...
        deep_analysis_config = {
            ...
            "query": f"SELECT * FROM trial_results WHERE experiment_id = '{experiment_id}'"
        }
        deep_analysis_result = await analysis_engine.perform_deep_analysis(deep_analysis_config)
        ...
        report_data_sources = []
        if analysis_result and analysis_result.get("status") == "success" and analysis_result.get("storage_info", {}).get("document_id"):
            ra_output_doc_id = analysis_result["storage_info"]["document_id"]
            ra_collection_id = pipeline_config["research_assistant_config"].get("firestore_insights_collection", "research_assistant_outputs")
            ...
            print(f"Warning: Could not get Research Assistant output document ID from: {analysis_result.get('storage_info')}")

        if deep_analysis_result and deep_analysis_result.get("status") == "success" and deep_analysis_result.get("storage_info", {}).get("analysis_id_stored"):
            ae_analysis_id = deep_analysis_result["storage_info"]["analysis_id_stored"]
            ae_config = pipeline_config["analysis_engine_config"]
            ae_table_id = ae_config.get("bigquery_analysis_results_table", "analysis_outputs")
            ...
            bq_project = ae_config.get("bigquery_project_id", pipeline_config.get("gcp_project_id"))
            bq_dataset = ae_config.get("bigquery_dataset_id", pipeline_config.get("bigquery_config", {}).get("dataset_id"))
            ...
            if bq_project and bq_dataset:
                ...
                print(f"Warning: Missing BigQuery project/dataset details for Analysis Engine source. Cannot query AE results.")
        else:
            print(f"Warning: Could not get Analysis Engine stored analysis ID from: {deep_analysis_result.get('storage_info')}")
        ...
        for server_instance in mcp_manager.mcp_servers.values():
            if hasattr(server_instance, "close_client") and asyncio.iscoroutinefunction(server_instance.close_client):
                try:
                    ...
                    print(f"Error closing client for server {server_instance.server_name}: {ex_close}")            
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run a complete example experiment workflow")
    parser.add_argument("--experiment-id", default="exp_spatial_reasoning_001", 
